...
> |---|---|---|
> | ① 全域層 | **本檔** | 每次都讀(自動載入) |
> | ② 規則層 | [`.Codex/rules/*.md`](.Codex/rules/) | 由本檔的觸發列指名時讀 |
> | ③ 模組層 | [`public/js/.AGENTS.md`](public/js/.AGENTS.md)・[`server/AGENTS.md`](server/AGENTS.md)・[`tools/AGENTS.md`](tools/AGENTS.md) | 動到該目錄的檔案時讀 |
...
| `public/js/*.js` | 渲染/FPV/輸入/HUD(檔案地圖見 `public/js/.AGENTS.md`) |
...
### 2.1 單一真相縫索引(**目錄在此,全文住 `.Codex/rules/seams-*.md`**)
...
**動到下列任一主題,MUST 先讀對應那一支的全文**;現役 178 列縫(2026-08-16 實數;舊註的 159 是估數),下表只列主題名。查不到主題時 `grep -rn "<關鍵詞>" .Codex/rules/` —— **MUST NOT 因為目錄裡沒看到就認定沒有規則**。

| 規則檔(= 原 §2.1 分節) | 涵蓋的縫 |
|---|---|
| [`seams-balance.md`](.Codex/rules/seams-balance.md)<br>**§2.1 A 平衡與角色** | 平衡數值・角色機種・推導值・英雄武器與招式解析・傷害衰減・陣營對抗對稱化・目標類別剋制與建築加乘・護盾裝甲分軌剋制・護盾軸配置紀律・對建築 DPS 收斂・三軸預算(範圍/機動/射程)・攻擊範圍收斂・射速壓縮與連發演出・機體移速壓縮・開火中位移懲罰・圖鑑六角能力圖・波次編制與節奏・陣營小兵強化・八軌階梯(價格 + 戰鬥分數門檻)・戰鬥分數・攻堅順序(劇情戰役鎖血)・劇情階段對話・劇情畫面標記・環境標籤・對進戰模型・前線交戰模型 |
| [`seams-abilities.md`](.Codex/rules/seams-abilities.md)<br>**§2.1 B 招式 / 大招 / 載具** | 招式啟動手勢・招式載具的發射點與槽位 CD 帶・大招載具遞送・自身強化型輔助機隊・純自身型大招補償・機種絕招預算・三種載具形式(無人機/變形者/機甲)・絕招載具 HP 校準與爆風面積計價・飛行動力學(受擊掉高 / 爬升動力) |
| [`seams-weapons.md`](.Codex/rules/seams-weapons.md)<br>**§2.1 C 武器判定 / 彈道** | AoE 與彈道分類・「打得到嗎」判定・範圍光暈名冊・扇形錐緣・閃避範圍與維持 DPS 補償・高地壓制・爆風超壓帶・射程界・引爆 = 碰撞・射程球心 = 擊發位置・出膛初速與飛行時間・榴彈火控・導引頭機動・射程閘門容差・高度制空射程・地形稜線遮蔽・貫穿演出・彈匣惰性補彈・機體高度與半徑 |
| [`seams-bots.md`](.Codex/rules/seams-bots.md)<br>**§2.1 D 電腦玩家(bot)** | 操作節奏・視野・戰術(選敵/撤退/打帶跑)・定位分類與策略・學習策略・碰撞量體與實體碰撞 |
| [`seams-terrain.md`](.Codex/rules/seams-terrain.md)<br>**§2.1 E 地形 / 結構 / 通行** | 迷你地圖・劇情戰役地圖與 NPC BOSS・砲塔佈局規則・兵線導航規則・離線工具的地形圖資剖面・**線工切面(§0-a 可行性樁)**・隧道與地下道剖面・隧道頂板・道路塗裝寬與結構接合・明隧道判定・道路路基整平・地形坡度移動・經緯度→世界公尺(含地圖主方位)・戰場世界方框與抓取範圍・道路 16 方向量化・立體結構建置範圍・飛行體貼地渲染基準・世界高度上限・世界邊界(環 + 緩衝空間)・邊界牆型錄與緩衝布景・巨岩表面落點・圓形腳印落底・攀爬路線・可通行性 |
| [`seams-render.md`](.Codex/rules/seams-render.md)<br>**§2.1 F 表現層 / 畫面** | 時間流逝與太陽月亮影子・**賽璐璐明暗階梯 / 學派**・**只掛學派不掛演出的材質**・後製管線・描邊寬度・世界曲面・景深模糊・空氣透視・勾線資訊緩衝・**outlineContribution**・**表面群組**・3D LUT 調色・地貌不出接縫・**勾線的雜訊斷筆**・**掠射抑制項恰一項**・**地貌分區墨線**・**霧範圍 ≡ 勾線淡出**・**畫面轉場(斜向 wipe)**・**轉場的呼叫端**・**物件出現(dissolve)**・建構期讓步・畫面表現旋鈕・陰影偏色・地表屬性場・風化屬性場・軟性物質・陣風包絡・海浪・**岸邊泡沫與水面倒影塊**・**玩家位移擾動**・**植被擾動源的餵入**・國旗・**落花 / 落葉粒子**・**葉片卡冠層與整棵樹的剪影**・**剪影優先(巨岩 / 石堆 / 遠景背景)**・**立體結構的線工授權**・零件級細節抖動・表現層資源生命週期・英雄機體建模・FPV 座艙・變形者的變形過程・步態關節曲線・交戰姿態・跳躍分級・**幀率無關阻尼**・**動畫權重向量**・共用視覺入口 |
| [`seams-world.md`](.Codex/rules/seams-world.md)<br>**§2.1 G 世界內容** | 都市計畫・**載具 / 擺件型錄**・**真凹處與可視角**・**鳥群(野生動物)**・樹冠羞避・地貌拼圖的顏色與花紋・地被貼合抬升・陸域地貌認養地形三角形・農田田埂與田塊對齊・農牧地表四季・地質排列・聚落場與建物來源信任階梯・場地地貌宣告 vs 圖資實測・語意化地標・世界文字・在地文字語料・AI 零件庫消費・建物零件庫・整棟量體輪廓剖面・平整垂直牆面板・平面整平與去噪封底・img→3D 自動入庫與撤下・採集迴圈啟停與圖檔三態・人眼判決字彙與封存區・來源帳的鍵・重跑順位・鏡像貼補・角色機體檔案格式 |
| [`seams-ui-net.md`](.Codex/rules/seams-ui-net.md)<br>**§2.1 H HUD / 輸入 / UI / 連線** | 連線機制・路網中繼・區網同時多路徑・操作方式・戰場選單開關・視野鎖定・觀戰相機・商店掃貨與預約・受擊濺血提示・異常狀態致盲白幕・蓄力跳水平移速・GUI 說明與懸浮提示・分段按鈕樣式・NPC 建築機種圖示・**觸控「一次點擊」的判定**・**視窗尺寸定案(旋轉 debounce)**・**頁面級觸控硬化(裝置 vs 版型)**・**音效層級(地點床 / 移動床 / 事件音 / BGM 階梯)**・決定性亂數 |
### 2.2 狀態鍵與迴圈粒度
...
## 3. 絕對反模式 A 表(**索引在此,全文住 [`.Codex/rules/antipatterns.md`](.Codex/rules/antipatterns.md)**)
...
**離線稽核(~60 支)與資產管線指令的完整清單住 [`.Codex/rules/verification.md`](.Codex/rules/verification.md) 的「5.1(續)」段**;`--break-*` = 反向驗證(見 §0 原則 9)。一律不需伺服器/瀏覽器/網路,例外(㋓ 需外網或真瀏覽器)在該檔內標明。
...
- ㋗ **「逐項不動」不等於逐字元 diff** —— 三處輸出天生有雜訊,拿 diff 當判準會得到假紅字:①`audit_touch_layout`/`audit_touch_gesture`/`audit_ui_layout` 的框線數字有 run-to-run 抖動(pristine 自己也抖)②`npm test` 有 5 處輸出隨機(PIN / 掉落現金 / TC 稀有度 / 爆風尾數 / 障礙種類)③`audit_traverse --json` 的 `cells` 欄非決定性(`BattleSim` 建構期以 `Math.random()` 擺第三方碉堡)。判準一律是「**通過數 + 失敗案例集合**」;全文與逐項數字見 [`verification.md`](.Codex/rules/verification.md) §5.5-0。


### 5.5 改了什麼 → MUST 跑什麼(**全文住 [`.Codex/rules/verification.md`](.Codex/rules/verification.md)**)

判準通則(§5.5-0)+ 147 列對照表(改動 → 驗證)。**動手改完、宣稱完成之前 MUST 打開那一支對號入座**;查不到你動的東西時回 §5.4 ㋔ 的相鄰稽核通則,MUST NOT 當成「這一項不用驗」。

### 5.6 AI 退化量測(bot 改動專屬)

判準、樣本數與取 base 的手法住同一支 [`verification.md`](.Codex/rules/verification.md)。一句話:**MUST NOT 拿 `npm run sim` 的勝負旗標**當訊號(它天生飽和),改看 `繞行%`/`engage%` 這類結構性指標。

---

## 6. 退場清單(**全文住 [`.Codex/rules/retired.md`](.Codex/rules/retired.md);MUST NOT 復辟**)

> 33 條**已經移除**的機制。看到「這裡好像少了一塊」而想補回去之前,**MUST 先開那一支** —— 移除本身就是定案(§0 原則 10)。

現役退場條目涵蓋:**格點驅動的岸邊泡沫片**・**四份手寫車 / 四份貨櫃 / 兩份列車**・**`ent._moveSpd` 第二份速度推導**・爆炸戰鬥部飛到射程界原地引爆・座艙手繪 builder 十支・舊版英雄建模七支・單樹變形者・NPC BOSS 防禦面永不升級・對白掛在整階推平上・劇情預置兵線取較小者・「爆炸傷害納入閃避 = 整組變弱」・有爆風就不可閃・bal ③ 十分鐘升滿・逐機種擊殺分數表・八軌階梯舊常數・小兵強化的 hp/賞金 ×cu・機種絕招三個入口・常駐護衛機外觀・重砲模式與巨砲・建築加成・榴彈錐形瞄準輔助・爆炸型豁免名冊內容・逐武器手寫 move/slowF・逐階手寫 BAR_MAX・舊 `signage.js`・自由字串 `lore.proto`・飛行取景 `standing`・舊載具 HP 係數・極音速終端追擊(死碼但定案留著)・「物件最高高度 = 2 倍砲塔高」・光暈分幀預算與 TTL 快取・航跡長 `b.dist`・bots 直寫座標與本地 BUY_ORDER・機種分派表・巨岩隨機 `rotation.y`。

---

## 附:其餘情境層與文件

**第 ② 層 — `.Codex/rules/`(由本檔觸發列指名時讀)**
...
| 文件 | 內容 |
|---|---|
| [`public/js/.AGENTS.md`](public/js/.AGENTS.md) | 客戶端 60 支模組:逐檔職責地圖 + 模組級地雷。**刻意不叫 `AGENTS.md`**(134KB,不該自動載入);動 `public/js/**` 時 MUST 手動開 |
| [`server/AGENTS.md`](server/AGENTS.md) | 權威層四支:傳輸 / 房間 / 模擬 / bot 的邊界與三機制約束 |
| [`tools/AGENTS.md`](tools/AGENTS.md) | 122 支離線工具的家族地圖與寫稽核的紀律 |
...
| `docs/ai3d_runbook.md` | img→3D 資產管線:§0 定案、§1~§4 現況與佇列、§5 逐節結論索引(錨點 `§5aj-C` 等被 25 支工具引用,**MUST NOT 改號**)。方法論住 `.Codex/skills/photo-to-3d-pipeline` 等三支 |
...
| `docs/anime_style_plan.md` | **日系動漫畫面整合計畫(下一輪畫面工作看這一份)**;技術本體拆在 `.Codex/skills/` 九支 |
...
> 2026-07-18 前的逐日檔案庫 `Codex-orig0718.md` 已於 commit `3629979` 刪除,內容住 git 歷史(`git show 3629979^:Codex-orig0718.md`)。
