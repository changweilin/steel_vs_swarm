...
2. **Zero extra randomness**: swapping geometry MUST NOT consume an extra `rnd()`. One extra draw shifts
   the whole map's vegetation/building/hazard layout sequence (AGENTS.md §2.3), and on screen it just reads as
...
4. **One extra `rnd()`** → the whole map's layout shifts → scenes differ across clients (AGENTS.md §2.3 / A4).
