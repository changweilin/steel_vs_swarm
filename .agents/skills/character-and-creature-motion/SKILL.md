...
| The no-people rule, its two accidental breaches and how they were caught | sakura — `NEXT.md` "Rules that must not be broken"; `AGENTS.md` trap table, the `addGlass` row |
| Prop-scale rules for small creatures (the crows) | sakura — `AGENTS.md` trap table, "A prop under 0.3 m will read as a dot" |
