...
table in `CLAUDE.md`); `messenger.abeto.co` (one-mesh terrain with shader masks, wipe
...
by flood fill. Its `CLAUDE.md` trap table is the single densest source of seam bugs
...
| **The seam trap table** — coplanar coin tosses, junction paving, kerb splits, unowned thresholds, platform overlap, the reference-plane bug | sakura — `CLAUDE.md`, "Traps that have already bitten" |
| Build order as a dependency; "a district returns its planting" | sakura — `CLAUDE.md` "Conventions"; `src/world/index.js` `districts` array |
