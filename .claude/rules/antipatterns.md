...
| A40 | Character and body file schemas live in exactly one module with identical generated key sets and order; body prototypes are structured layers derived from visuals; outbound texts assemble in exactly one place; content modules keep zero imports. | — (已退場；`audit_codex` 隨 2D 生圖對照台退役) |
