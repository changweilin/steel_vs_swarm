...
node tools/audit_leaf_card.mjs       # 葉片卡冠層 / 整棵樹的剪影(張數推導 / 外廓 ≤ 保險絲冠幅 / 零共享 rnd
                                     #  / 佈局只讀 p.g / 無 MRT 必退回)
#   ±--break-count(張數改逐型手寫)/--break-fuse(包絡改讀庫幾何)/--break-partgate(移除 v1 自然零件資格閘)
...
node tools/audit_pedestrian_plan.mjs # 人行天橋 / 地下與車站入口 / 沿路鐵道主題廊道
...
node tools/audit_bridge_crossing.mjs # 橋交會去重 + 橋墩帽梁不得蓋過橋面
...
| 世界邊界 / 邊界障礙型錄 / 最小緩衝裙 | `audit_world_edge` ±`--break-lap`/`--break-buffer`/`--break-fit`/`--break-face`/`--break-run`/`--break-slope`/`--break-boxh`/`--break-bias`/`--break-rock-season`/`--break-wet`/`--break-motion` + `audit_slope_move`(坡度門檻是共用的那兩條線) + `audit_siteplan`(`placeBoundary` 的沙箱要多注入 `edgeWallDeepM`)+ **裙 MUST 擺在 terrain.js 開挖/射線/打洞三支稽核抽原文的標題之外**(落在裡面那些沙箱會一起執行而 `ReferenceError`)+ `audit_cel_pipeline`/`audit_soft_stroke`/`audit_gpu_lifecycle`/`audit_world_curve`(envMat、共用風時鐘與水面分段 MUST 逐位元不動)+ `audit_client_syntax`(㋖)+ `audit_net_modes`/`audit_solo_boot`(新增客戶端模組)+ `audit_beacons`(`mergeGeos` 多一個選用參數,不給 = 逐位元同舊制)+ **`shot_scene` 的 `edge_wall`(㋓:起伏帶自然障礙密度與四季岩色只能由實拍驗收)** + ㋔ + ㋒ |
...
| **頁面級觸控硬化 / 裝置 vs 版型旗標**(`style.css` 的 `body.touch-dev` 區塊與 `body.touch-dev #game`、`#roomPin/#roomUrls` 選字窄豁免、根層 `text-size-adjust`、`body.touch-ui` 的 `--tl-*` 與安全區、`mobile.installTouchUI()` 的 `touch-dev` 掛載、`index.html` 的 viewport meta) | `audit_ctrl_mode` **Ⅹ** ±**五支** `--break-viewport`/`--break-textadj`/`--break-touchdev`/`--break-touchact`/`--break-meta-select`(每一支 MUST 對應紅字;`--break-touchdev` 下 Ⅹ⑤ **MUST 仍綠** —— 兩欄同時對才代表旗標拆開了)+ `audit_touch_layout` / `audit_touch_gesture` / `audit_ui_layout` / `audit_gyro` MUST **逐項不動**(判準見 ㋗-1:框線數字有 run-to-run 抖動,`audit_touch_layout` 的 **8/60 既有紅 MUST 維持在 8**)+ `audit_client_syntax`(㋖)+ `npm run audit:net` / `audit_solo_boot` + ㋒ + **㋕ 真機**:①房主鎖「限定滑鼠鍵盤」時在手機上仍**捏合不動、下拉不刷新、長按不選字**(這正是改制前壞掉的那一格,而它在離線這端只表現成選擇器字串不同)②iPhone 橫握 HUD 下帶比例與直握一致(text-size-adjust 的唯一驗收面)③有觸控螢幕的 Windows 筆電仍吃 `#game{touch-action:none}` 與頁面級 `user-select:none`,但 PIN / 區網網址可長按選取並複製 |
...
| 語意化地標 | `audit_beacons` ±`--break-extent`/`--break-pad`/`--break-order` + `audit_object_joints`/`audit_gpu_lifecycle` + ㋒ |
...
| 行人天橋／地下與車站入口／沿道路鐵道主題廊道(`pedestrian.js` / `biomes.js` 接線 / OSM entrances) | `audit_pedestrian_plan` ±`--break-underground`/`--break-footbridge`/`--break-adjacent` + `audit_road_grid`/`audit_road_joint`/`audit_bridge_crossing`/`audit_water_skirt`/`audit_osm_relay`/`audit_world_text` + `audit_client_syntax`；新增 OSM 欄位與查詢須重跑 `measure_osm_relay`；**`audit_traverse`** 驗人行橋端點與一般道路主網不斷；`shot_scene --live --only road_tangle` 與入口／天橋固定鏡位做人眼核對。純表現入口與廊道附屬件 MUST 零共享 `rnd()`、不得新增 blockers。|
...
| 橋交會去重(`dedupeCrossingBridges`)/ 橋墩帽梁(`capM`)/ 馬路橫切繞行(`skirtWaterClips`)/ 橋上砲塔墩座(`planTowerBridgePads`/`TOWER_PAD_AXIS`/`TOWER_BASE_R`) | `audit_bridge_crossing` ±`--break-cap-sink`(優先度 兵線 > 鐵路 > 大馬路 > 小馬路;鐵路容差 `gap=0` 只認真交叉;水平帽梁下掛 ≥0.8m,不得在坡段蓋過橋面)/ `audit_water_skirt`(斜交對稱車行路 MUST 建橋不繞；步道不因泡水影像自動造橋，但明示 `bridge=yes` 由 `audit_pedestrian_plan` 驗成人行天橋)/ `audit_bridge_tower_pad`(沿橋軸與水側走位帶 ≥ 基座 + 8m;`TOWER_BASE_R` 不變) |
