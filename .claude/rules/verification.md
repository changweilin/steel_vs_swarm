...
#   ±--break-prune/--break-drift/--break-dense/--break-relax/--break-rotbox/--break-rotover
...
| 橋交會去重(`dedupeCrossingBridges`)/ 橋墩帽梁(`capM`)/ 馬路橫切繞行(`skirtWaterClips`/`PED_HW`)/ 橋上砲塔墩座(`planTowerBridgePads`/`TOWER_PAD_AXIS`/`TOWER_BASE_R`) | `audit_bridge_crossing` ±`--break-cap-sink`(優先度 兵線 > 鐵路 > 大馬路 > 小馬路;鐵路容差 `gap=0` 只認真交叉;水平帽梁下掛 ≥0.8m,不得在坡段蓋過橋面)/ `audit_water_skirt`(斜交對稱穿越 MUST 建橋不繞;步道一律不進橋樑管線)/ `audit_bridge_tower_pad`(沿橋軸與水側走位帶 ≥ 基座 + 8m;`TOWER_BASE_R` 不變) |
