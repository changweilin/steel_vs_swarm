...
for (const [venue, byL] of Object.entries(VENUE_LANES)) {
  for (const [L, entry] of Object.entries(byL)) {
    ...
    let worstRB = 0, worstRF = 0, minStack = Infinity, resid = 0, oppFront = 0;
    ...
    rows.push({ venue, L, worstRB, worstRF, oppFront, minStack, resid, stackBad: minStack < STACK - 1 });
  }
}

console.log(`規則 #4 稽核(per-lane)— SEP(≤80% 門檻)=${Math.round(SEP)}m,疊塔門檻 <${STACK}m,R=${R}m\n`);
console.log('場地        L  敵我前塔  後↔堡  後↔前  最近塔距  殘餘>80%');
...
for (const r of rows) {
  ...
  console.log(
    ...
    `  ${`${Math.round(r.worstRF)}%`.padStart(5)}  ${`${Math.round(r.minStack)}m`.padStart(7)}   ${String(r.resid).padStart(4)}  ${flag}`);
}
console.log('─'.repeat(72));
