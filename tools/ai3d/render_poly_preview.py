#!/usr/bin/env python3
"""把 v6 model.json 軟體光柵化成固定三視圖，供 LLM 與來源圖複核。"""
...
def render_view(draw, vertices, faces, panel_x, panel_w, height, yaw, label):
...
def main():
    parser = argparse.ArgumentParser(description="渲染 v6 model.json 三視圖")
    ...
    panel_w = width // 3
    for index, (yaw, label) in enumerate(((math.radians(-35), "FRONT 3/4"), (math.radians(90), "SIDE"), (math.radians(180), "REAR"))):
        render_view(draw, vertices, faces, panel_w * index, panel_w, height, yaw, label)
        if index:
