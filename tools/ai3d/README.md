...
| `object_classification_inventory.mjs` | 把 corpus + 母照片 + YOLO26 schema-v2 證據綁成同一穩定列；`npm run classify:objects` 產生清單與類別樹 |
| `object_category_extensions.json` | 現有功能類型容納不了時，由父流程登錄新類別的單一擴充縫 |
| `object_category_tree.mjs` | 產生 `family/existing|extended/objectType` 樹狀目錄；證據不足與損壞來源另行隔離 |
| `audit_object_classification.mjs` | 母照片覆蓋、YOLO26 同列配對與功能替換正反例；`npm run audit:classification` |
| `audit_object_category_tree.mjs` | 類別擴充、樹狀目錄覆蓋與非外觀分類反向稽核；`npm run audit:category-tree` |
