...
//
// ---- 接縫紀律:緣石那一列(2026-08-16 併入,`docs/anime_style_plan.md` ④-4;純註解,零斷言改動)----
// `UND.COPE` 帶(Ⅱ-b:自牆線 hw 鋪到 hw + COPE)是**全專案唯一的緣石**,而參考專案記錄過的
// 那一族接縫陷阱裡緣石有兩條 symptom,兩條在本專案都成立:
//
//  ㋐ **車道經過的地方要斷緣石。** 症狀:**停車場 / 側路 / 場地出入口橫著一道緣石或草皮**,
//     車開不過去、走過去會被抬起來。⚠ 這一條在本專案是**結構上量不到**的:
//     `COPE` 帶**純視覺、不進 `blockers` 也不進碰撞** ⇒ **`audit_traverse` 的泛洪永遠看不到它**
//     (泛洪只認碰撞與站立面)。也就是說「緣石橫在入口上」**沒有任何離線稽核會紅**,
//     只在真機上看得出來(㋕)。今天本專案的地下道引道兩側沒有橫向出入口 ⇒ 這一條**還沒有**
//     可以驗的對象;真的長出「側路匯入引道」那種幾何時,MUST 在本支補一條「緣石帶遇到橫向
//     車道要斷開」的斷言,**MUST NOT 指望通行稽核**。
//  ㋑ **緣石是「兩個高度在同一條線上相接」的典型。** 同族規則的一句是「兩塊同高度的板子,
//     一塊**抵住**另一塊的邊線、絕不重疊」;而 Ⅱ-b 的 `COPE ≥ CUT_W + 1` 是它的另一面 ——
//     緣石帶**必須蓋過**垂直路塹過渡帶與地形網格拉伸殘坡。兩條方向相反而不矛盾:
