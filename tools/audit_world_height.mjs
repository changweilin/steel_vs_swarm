...
const LANDMARK_COL = runTable(bioSrc, 'LANDMARK_COL', ['TOWER_BUILDINGS'], [TOWER_BUILDINGS]);
...
console.log('\nⅤ 單一縫:天花板只有一份、消費端數得出來');
{
  ...
  t('物件夾制的每一個生成點都經 data.js 的縫(神木×2 / 巨岩×2 / 地標×1 / 共用物件×1)',
    count(bioCode, /objScaleFit\(/g) === 6 && /objScaleFit\(1, sampledSize\[1\], 1\)/.test(bioCode), `${count(bioCode, /objScaleFit\(/g)} 處`);
