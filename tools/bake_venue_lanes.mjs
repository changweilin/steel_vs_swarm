...
const ANCHORS_ALL = {
  ...
  madrid: [[40.43784, -3.68925], [40.43784, -3.68560]],
  // 地下道(②)第二張圖:桃園國際機場航廈區。探測(2026-08-02)成洞 5 條 ——
  // 航站南路 覆蓋 47m @25.07614,121.23270、航站北路 覆蓋 228m @25.07791,121.23111。
  // **兩錨都夾在航站北路那條洞上**(第一輪教訓):首輪把南路擺前面、放任 PREFER_TUNNEL
  // 自己挑,結果它挑了一條**更長但建不起來**的無名 service tunnel way —— 兵線踩上去 238m、
  // `underpassPlan` 放棄 ⇒ 仍是平街,只拿到 ⑥ 穿越橋下。PREFER_TUNNEL 比的是「踩在 tunnel
  // way 上的長度」,那是**圖資**長度,不問執行期建不建得出來 ⇒ 錨點要自己把兵線鎖在
  // 探測證實成洞的那條上。錨點取 228m 覆蓋段中點 @25.07791,121.23111 東西各退 ~150m,
  // 洞才會落在兩堡中間(引道 ~128 遊戲公尺 ×2 + 覆蓋段要一起塞進 L1 圖界)。
  taoyuan: [[25.07791, 121.22962], [25.07791, 121.23260]],
...
const PREFER_BRIDGE = new Set(['parkave', 'london', 'berlin', 'chicago']);
...
const BEARING_SECTORS = {
  ...
  madrid: [[[60, 120]], [[240, 300]]],   // María de Molina 東西向:西錨往東 / 東錨往西
  // 航站南路/北路都是東西向(航廈 ↔ 停機坪聯絡道):西錨往東 / 東錨往西,
  // 兩堡才會一南一北分踞滑行道兩側,兵線非鑽下穿道不可。
  taoyuan: [[[60, 120]], [[240, 300]]],
