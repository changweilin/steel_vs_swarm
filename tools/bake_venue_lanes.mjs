...
const ANCHORS_ALL = {
  ...
  // 陸上高架橋(③)測試場地:柏林 華沙大街跨東站調車場。橋下是鐵道不是水 ⇒ 純陸域;
  // 左右數百公尺內沒有第二個跨越點 ⇒ 最短路徑非上橋不可(parkave 失敗的原因正是「等長替代
  // 路線太多」,曼哈頓格柵怎麼繞都一樣長)。方位角夾南北 = 沿華沙大街的橋軸。
  berlin: [[52.50300, 13.44870], [52.50760, 13.44890]],
  // 地下道(②)測試場地:馬德里 卡斯提亞大道沿線車行地下道群。地形全平 ⇒ 深度只能來自「挖」,
  // 正是 underpassPlan 的適用面;PREFER_TUNNEL 讓選線優先踩上 tunnel way。
  madrid: [[40.44560, -3.69120], [40.43350, -3.68960]],
  // 水上高架橋(⑨)測試場地:芝加哥河兩岸。河面僅數十公尺寬、南北向幹道一律以可通車的
  // 開合橋跨河 ⇒ 兩堡分踞兩岸時兵線必然踩上橋面(與 london 的「單座長橋」互為對照組)。
  chicago: [[41.88780, -87.62430], [41.88980, -87.62420]],
...
const PREFER_TUNNEL = new Set(['civicblvd', 'taroko', 'madrid']);
