...
const server = createServer(async (req, res) => {
  ...
  const file = path === '/' ? new URL('./geology_preview.html', import.meta.url)
    : /^\/js\/[a-zA-Z0-9_-]+\.js$/.test(path) ? new URL(path.slice(4), root) : null;
  if (!file) { res.writeHead(404).end(); return; }
  try {
    const content = await readFile(fileURLToPath(file));
    res.writeHead(200, { 'Content-Type': path === '/' ? 'text/html; charset=utf-8' : 'text/javascript; charset=utf-8', 'Cache-Control': 'no-store' });
    res.end(content);
