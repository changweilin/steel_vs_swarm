...
import { mat, outlineW, dim, bx, cyl, rbz, feather, jetFlame, segLimb } from '/public/js/geo3d.js';
import { podWeapon, fitToHeight } from '/public/js/models.js';
 ...
 *       |'heli' 三旋翼突擊機(**無尾桁無翼**:雙腿後外撇打直與軀幹成 Y 字,機首桅 + 雙腿末端
 *       共三具旋翼;2026-07-11 重構,見下方 `F === 'heli'` 區塊)|'tilt' 傾轉旋翼母艦
 ...
 */
// 實作住 `geo3d.js`(本檔頂部 import);上面那段契約說明留在這裡,呼叫端全在本檔一帶。
...
// ---- 舊版名冊(機體台「舊版」對照分頁吃這一份)--------------------------------
// 鍵 = 角色 id;`kind` = 這台在**舊制**走的是哪一支建構器(與新版的鷹架分類無關)。
export const LEGACY_BUILDERS = {
  robotProto: (side, vis, ch) => buildRobotMech(side, vis, ch),
  robotBeast: (side, vis, ch) => buildBeastMech(side, vis, ch),
  robotBiped: (side, vis, ch) => buildBipedBeast(side, vis, ch),
  droneRotor: (side, vis, ch) => buildDrone(side, vis, ch),
  droneFixed: (side, vis, ch) => buildFixedWing(side, vis, ch),
  droneAvian: (side, vis, ch) => buildAvianDrone(side, vis, ch),
  morph: (side, vis, ch) => buildMorphMech(side, vis, ch),
};

/** 舊制 `makeUnit` 的分流規則(逐字保留:機體台要看到的就是**當年那一台**)。 */
export function legacyKindOf(ch) {
  const c = CHARACTERS[ch];
  if (!c) return null;
  const vis = c.visual || {};
  const k = c.kind;
  if (k === 'morph') return 'morph';
  if (k === 'robot') return vis.form === 'beast' ? 'robotBeast' : vis.form === 'biped' ? 'robotBiped' : 'robotProto';
  if (k === 'drone') return vis.form === 'avian' ? 'droneAvian' : vis.form === 'fixed' ? 'droneFixed' : 'droneRotor';
  return null;
}

/**
 * 依角色 id 鍛一台**舊版**機體(機體台對照用)。
 * 收尾逐字照抄舊制 `makeUnit` 的程序生成分支:塗裝 → fitToHeight → 描邊。
 * @returns { group, rig } | null
 */
export function buildLegacyUnit(ch, { height = null } = {}) {
  const c = CHARACTERS[ch];
  const key = legacyKindOf(ch);
  if (!c || !key) return null;
  const vis = c.visual || {};
  const side = c.side || 'STEEL';
  const target = height ?? heroTargetH(c.kind, ch);
  const built = LEGACY_BUILDERS[key](side, vis, ch);
  const tone = (key === 'robotBeast' || c.kind === 'drone' || c.kind === 'morph') ? 'dark' : 'light';
  if (vis) paintUnit(built, vis, side, tone);
  fitToHeight(built, target);
  outlinify(built, outlineW(target));
  if (built.userData.rig) built.userData.rig.s = built.scale.x;
  return { group: built, rig: built.userData.rig || null, spin: built.userData.spin || null };
}
