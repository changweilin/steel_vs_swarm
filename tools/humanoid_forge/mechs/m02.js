...
//   讓開頸/頭的凝視擺動空間。尾 = chainF 收分節鏈 + 背脊板。圖騰紅黃帶紋 / 鏽蝕補丁 = paint 層。
// 2026-08-12 v3 依暴龍正模骨架照調姿(★ 仍管機械外觀,骨架照管姿態比例):
//   ①脊線壓平近水平 —— 胸艙轉近水平(rot.x 1.42)+ 背甲蓋板放平成脊背線(髖最高、向肩微下斜),
//     sz 剪切讓深胸落在肩後、腹線向髖上收;②頭前伸 —— 頭幾何整組經靜態中介 Group 前移
//     (hh z+1.35),頭頂 ≈ 背線高、短粗頸接在肩前;③尾加長加粗 —— 11 節、r0 0.46、
//     尾根抬到脊線高(hips 0.42)近水平微垂(tailRoot −0.12);④腿改趾行深屈 ——
//     legBase −0.28 股前傾 + 膝 0.62 深屈 + 踝 −0.32 補償(足底放平、踝落髖正下方)。
...
export default {
  ...
  neckAt: [0, 0.86, 0.72],                                      // 頸樞軸(胸艙前上緣;頭前伸的兩段凝視穩定)
  ...
  doc: [
    ...
    ['頸(短粗前伸)', 'cylF 頸芯斜插(rot.x 1.3 頂進頭底)+latheF 三環收分沿頸軸+cablesF 喉管 ×3+背側鱗板 ×2 —— 頭接在肩前(骨架照)'],
    ['軀幹(水平脊線)', 'tboxF 近水平深胸艙(rot.x 1.42+sz 剪切:深胸落肩後、腹線向髖上收)+latheF 外凸肋甲環 ×3+prismF 胸甲 ×2+頸根領環+腹龍骨+cablesF 腹管+側百葉'],
    ...
    ['重尾(chainF 十一節配重)', '十一節收分節鏈(≈半個體長,尾根抬到脊線高、近水平微垂)+節環+背脊板 ×5(索引遞減)+coneF 尾梢錐'],
  ],
  head(c, h) {
    const { PAL, accent } = c;
    // 靜態中介 Group:頭幾何整組前移(骨架照:頭由短粗頸接在肩前、頭頂 ≈ 背線高)——
    // 樞軸(凝視穩定)不動,只推幾何;mount 的 jawG/tg 吃同一組偏移(−0.13/+1.48 系)。
    const hh = new THREE.Group();
    hh.position.set(0, 0.05, 1.35);
    h.add(hh);
    ...
    const vent = latheF(hh, [[0.06, 0], [0.09, 0.10], [0.125, 0.21]], 10, 0, 0.07, -0.30, GUNMETAL, { metalness: 0.8 });
    vent.rotation.x = -Math.PI / 2;
  },
  neck(c, nk) {
    const { PAL } = c;
    // 短粗前伸頸(骨架照:頭接在肩前、頸近水平微昂)—— 樞軸在胸艙前上緣(neckAt 0.86/0.72),
    // 頸芯斜插(rot.x 1.3 = 頂端朝前)自胸艙頂進頭底(頭殼 hh 前移後的後緣),零斷縫。
    const core = cylF(nk, 0.24, 0.30, 0.62, 10, 0, 0.10, 0.30, PAL.dark, { metalness: 0.65 });
    core.rotation.x = 1.3;
    // 分節頸甲環 ×3(latheF 收分沿頸軸;★ 圖頸部 2~3 圈裝甲帶 —— 圖騰紋歸 paint 層)
    const rings = [[0.063, 0.165, 0.33, PAL.main], [0.113, 0.348, 0.30, PAL.mid], [0.164, 0.531, 0.27, PAL.main]];
    ...
    // 喉側管束(cablesF 一條一件,沿頸下緣伸向顎根)+ 背側鱗板 ×2(頸背)
    cablesF(nk, { p0: [0, -0.18, 0.12], p1: [0, -0.08, 0.58], k: 3, r: 0.03, sag: 0.03, spread: 0.065 }, GUNMETAL, { metalness: 0.75 });
    for (const [py, pz] of [[0.39, 0.20], [0.41, 0.40]]) {
  ...
  chest(c, ch, d) {
    const { PAL, accent } = c;
    // 近水平深胸艙(rot.x 1.42 ≈ 體軸仰 8°;sz 剪切把前端面往下推 = 深胸落在肩後、
    // 腹線向髖上收 —— 骨架照的胸腔剖影),樞軸不動只推幾何
    const body = tboxF(ch, { w0: 1.0, d0: 1.02, w1: 1.15, d1: 1.18, h: 1.55, sz: 0.35 }, 0, 0.58, 0.38, PAL.main, { metalness: 0.55 });
    body.rotation.x = 1.42;
    // 外凸肋甲環 ×3(latheF 鼓環貼體軸;★ 圖軀幹一圈圈裝甲帶 —— 環心壓在體軸下緣,凸在腹側)
    const hoops = [[0.325, -0.115, 0.70, PAL.mid], [0.38, 0.38, 0.70, PAL.main], [0.44, 0.875, 0.66, PAL.mid]];
    ...
    // 左右胸甲板(prismF 六角,貼前端剪切面)+ 頸根領環(胸艙前上緣)
    for (const sx of [-1, 1]) {
      const pec = prismF(ch, [[-0.17, -0.24], [0.17, -0.24], [0.28, 0], [0.17, 0.22], [-0.17, 0.22], [-0.28, 0]]
        .map(([x, y]) => [sx * x, y]), 0.10, sx * 0.28, 0.45, 1.26, PAL.lite, { metalness: 0.55 });
      pec.rotation.x = -0.15;
      // 側百葉(散熱開口)
      bxF(ch, 0.06, 0.26, 0.44, sx * 0.56, 0.38, 0.30, INK, { metalness: 0.7 }).rotation.y = sx * 0.1;
    }
    const collar = latheF(ch, [[0.38, -0.06], [0.44, -0.02], [0.44, 0.02], [0.38, 0.06]], 12, 0, 0.90, 0.85, PAL.mid, { metalness: 0.6 });
    collar.rotation.x = 1.30;
    collar.scale.z = 1.1;
    // 腹龍骨(深胸下緣近水平楔)+ 腹側管束(沿腹線)
    const keel = tboxF(ch, { w0: 0.55, d0: 0.45, w1: 0.45, d1: 0.35, h: 0.50 }, 0, -0.14, 0.85, PAL.dark, { metalness: 0.65 });
    keel.rotation.x = 1.5;
    cablesF(ch, { p0: [0, -0.26, 1.10], p1: [0, -0.14, 0.30], k: 3, r: 0.03, sag: 0.03, spread: 0.09 }, IRON, { metalness: 0.8 });
    // ── 背甲大蓋板(放平 = 脊背線:髖最高、向肩微下斜,肩後覆至尾根;前緣讓開頸/頭)──
    const slab = prismF(ch, [
      [0.50, 0.98], [0.05, 1.28], [-0.80, 1.38], [-1.55, 1.30], [-1.92, 1.06], [-1.72, 0.84], [-0.70, 0.80], [0.22, 0.84],
    ], 1.45, 0, 0, 0, PAL.mid, { metalness: 0.6 });
    slab.rotation.y = -Math.PI / 2;
    const cap = tboxF(ch, { w0: 1.00, d0: 0.88, w1: 0.86, d1: 0.72, h: 0.08 }, 0, 1.42, -0.80, PAL.main, { metalness: 0.55 });
    cap.rotation.x = 0.02;
    // 前斜面散熱百葉 ×4(INK 橫柵,索引沿斜面爬升;斜面向後升 ⇒ 負仰角)
    for (let i = 0; i < 4; i++)
      bxF(ch, 0.78, 0.04, 0.15, 0, 1.04 + i * 0.075, 0.40 - i * 0.11, INK, { metalness: 0.7 }).rotation.x = -0.59;
  ...
  pelvis(c, hips) {
    ...
    const tr = latheF(hips, [[0.34, -0.06], [0.40, -0.02], [0.40, 0.02], [0.34, 0.06]], 10, 0, 0.42, -0.90, IRON, { metalness: 0.8 });
  ...
  armUp(c, a, d) {
    const { PAL } = c;
    // 球肩(sphF)+ 亮色楔臂殼 + 活塞:短前臂幾何整組前置(+z 0.45)貼胸前 —— ★ 圖短臂在胸前深屈
    // (v3 胸艙拉長前伸 ⇒ 前置量 +0.15 跟著胸前緣走,armFore/mount 爪同步)
    sphF(a, 0.15, 0, 0.02, 0.45, PAL.mid, { metalness: 0.65 });
    tboxF(a, { w0: 0.20, d0: 0.22, w1: 0.15, d1: 0.17, h: d.len * 0.94 }, 0, -d.len * 0.5 - 0.02, 0.45, PAL.lite, { metalness: 0.55 });
    hydCyl(a, 0.026, d.len * 0.5, c.sx * 0.12, -d.len * 0.55, 0.39, -0.06, PISTON);
  },
  armFore(c, a, d) {
    const { PAL } = c;
    // 肘盤(latheF)+ 前臂細殼(root 依肘屈角補償貼回上臂端)+ 活塞
    const ed = latheF(a, [[0.035, -0.045], [0.11, -0.036], [0.13, 0], [0.11, 0.036], [0.035, 0.045]], 10, c.sx * 0.12, -0.01, 0.43, PAL.mid, { metalness: 0.75 });
    ed.rotation.z = Math.PI / 2;
    tboxF(a, { w0: 0.15, d0: 0.17, w1: 0.11, d1: 0.13, h: d.len * 0.94 }, 0, -d.len * 0.56, 0.41, PAL.lite, { metalness: 0.6 });
    hydCyl(a, 0.022, d.len * 0.45, -c.sx * 0.085, -d.len * 0.5, 0.35, -0.05, PISTON);
  },
  mount(c, F) {
    const { PAL, accent } = c;
    // ── 顎鉸 + 下顎(真樞軸:heavyPivot 充能張顎 = 射擊姿勢;rest 常開 0.30 露砲)──
    const jawG = new THREE.Group();
    jawG.position.set(0, -0.13, 1.48);                          // 吃 head hh 前移偏移(+0.05/+1.35)
    ...
    tg.position.set(0, -0.29, 1.68);                            // 吃 head hh 前移偏移(+0.05/+1.35)
    F.head.add(tg);
    ...
    for (const [g, sx] of [[F.handL, -1], [F.handR, 1]]) {
      const cap = latheF(g, [[0.055, -0.045], [0.095, -0.022], [0.095, 0.022], [0.055, 0.045]], 10, 0, -0.02, 0.39, PAL.deep, { metalness: 0.7 });
      cap.rotation.x = 0.2;
      for (let i = -1; i <= 1; i++) {
        const s1 = tboxF(g, { w0: 0.055, d0: 0.06, w1: 0.04, d1: 0.048, h: 0.19 }, i * 0.08, -0.11, 0.52, CLAWSTEEL, { metalness: 0.7 });
        s1.rotation.x = Math.PI / 2 + 0.55 + i * 0.06;
        const s2 = finF(g, { len: 0.21, w0: 0.058, w1: 0.014, t: 0.05, sweep: 0.04, camber: 0.035 }, i * 0.08, -0.17, 0.62, CLAWSTEEL, { metalness: 0.7 });
  ...
  extra(c, F, rig) {
    const { PAL } = c;
    // 重尾(chainF 十一節收分,≈半個體長)—— whipTail 逐幀「覆寫」節樞軸旋轉 ⇒ 基礎姿勢 MUST 住幾何:
    // 掛在近水平微垂的尾根 Group 上(whipTail 摸不到它;骨架照:尾自髖向後近水平延伸、與前半身配平),
    // 尾根抬到脊線高(hips 0.42)接在背甲蓋板尾端之下。節間分隔 = 深色窄環(INK,v2 實測定案)。
    const tailRoot = new THREE.Group();
    tailRoot.position.set(0, 0.42, -1.00);
    tailRoot.rotation.x = -0.12;
    F.hips.add(tailRoot);
    const { segs, tip } = chainF(tailRoot, {
      n: 11, y: 0, z: 0,
      len0: 0.62, len1: 0.26, r0: 0.50, r1: 0.12,
      rot0: -0.03, rotD: 0.006, ring: false, seg: 10,
      drawSeg: (t, i, { r, len }) => {
        const sep = cylF(t, r * 1.02, r * 1.02, r * 0.18, 10, 0, 0, -0.015, INK, { metalness: 0.7 });
        sep.rotation.x = Math.PI / 2;
        if (i < 5) tboxF(t, { w0: r * 0.55, d0: len * 0.5, w1: r * 0.3, d1: len * 0.32, h: 0.13 - i * 0.02 },
