// ============ t05 逐機零件檔(dev-only;獸型雙足 biped + grounded/tuckArms)============
// ── t05「仿生鶴」原型機(ostrich 鴕鳥/鶴):長腿貼地跑、分節長頸、右翼藏飛彈管、左翼藏線圈步槍 ──
// 2D 定案圖:public/assets/cyberpunk_art/mechs/t05_{static,moving,heavy}.png(★ = t05_moving)
// 幾何語彙一律取自 ../geo.js;MUST NOT 在本檔自建 BufferGeometry。
// 2026-08-12 多面體改寫(照 ★ 圖逐部位):
//   頭 = prismF 楔形顱殼(後掠冠稜一體)+ 鉤喙楔台;頸 = 貝茲曲線取樣 8 節 latheF 裝甲袖套
//   (基粗梢細、節間束環 —— 鶴頸分節,索引遞變零亂數);軀幹 = prismF 側剖面鳥身主殼
//   + 側甲板 ×2 + 背鞍板 + 腹部波紋內構鼓 + 尾下雙噴口;收翼 = 覆羽板 + finF 長刃初級
//   飛羽一片一件;腿 = 收分股殼 + latheF 髖盤螺栓 + 細長脛 + 二趾分節爪;尾羽 = fanF 六片。
// 白鶴美學:素色 solid(PAL 白銀階)+ 局部冷焰藍(ICE)發光;塗裝圖紋歸 paint 層。
import * as THREE from 'three';
import {
  matF, dimF, bxF, cylF, sphF, coneF, torusF, tboxF, prismF, latheF, finF, fanF, chainF, cablesF,
  hydCyl, sinew, seg2, IRON, GUNMETAL, COAL, INK, BONE, BRASS,
} from '../geo.js';

// 本機專用色:冷焰藍(★ 圖的眼/噴口/線圈輝光)
const ICE = 0x66d8ff;

// 二次貝茲(長頸取樣;純索引決定性,零亂數)
const bz = (p0, c1, p1, t) => p0.map((v, k) => (1 - t) * (1 - t) * v + 2 * (1 - t) * t * c1[k] + t * t * p1[k]);
const bzT = (p0, c1, p1, t) => p0.map((v, k) => 2 * (1 - t) * (c1[k] - v) + 2 * t * (p1[k] - c1[k]));
// 頸曲線:neckG 原點(胸腔 [0,0,0.18];neckAt y MUST 取 0 —— locomotion :481 以 chest 框的
// headY0 直接覆寫 head.position.y,neckAt 抬高多少頭就懸空多少)→ 頭樞軸(neck 框 [0,2.28,-0.14])
const NK0 = [0, 0.52, 0.14], NKC = [0, 1.48, 0.58], NK1 = [0, 2.28, -0.14];

export default {
  label: '仿生鶴(t05 機甲・鴕鳥)', hue: 0xf2f2f2, height: 6.0,
  prop: { hips: 0.58, legSplay: 0.07, thigh: 0.42, shin: 0.5, shoulderY: 0.80, shoulderX: 0.09, upperArm: 0.05, foreArm: 0.1, head: 0.96, girth: 0.8 },
  gait: { strideF: 1.5, bob: 0.1, sway: 0.06, top: 7, legBase: -0.16, armBase: 0.35 },
  pose: { knee: { base: 0.3, k: 0.72, d: 0.15 }, ankle: { base: -0.28, k: -0.44, d: 0.55 }, elbow: { base: -1.1, k: -0.1, d: 0.3 } },
  neckAt: [0, 0, 0.18],                                          // 長頸樞軸(y=0:見上方 NK0 註)
  moveSig: { poise: 0.50, idleF: 1.42, idleA: 0.60, launch: 0.16, spool: 0.18, brake: 0.22, settle: 1.55 },
  castSig: { omni: 'dance', dir: 'kick' },
  doc: [
    ['楔形小頭 + 冠稜', 'prismF 側剖顱殼(後掠冠稜一體)+ tboxF 鉤喙(sz 下垂)+ 冷焰藍眼 ×2 + latheF 感測圓盤/torusF 亮環 ×2'],
    ['分節長鶴頸', '貝茲曲線取樣 8 節 latheF 裝甲袖套(基粗梢細)+ 節間束環 ×3 + 頸根出殼領環'],
    ['鳥形軀幹', 'prismF 側剖主殼(深龍骨胸→尾尖)+ prismF 大側甲 ×2 + tboxF 背鞍板 + latheF 腹部波紋內構鼓 + 尾下 latheF 噴口 ×2(ICE 光碟)'],
    ['收翼(tuckArms)', 'armUp:prismF 覆羽長刃板 + fanF 覆羽 ×3;armFore:finF 長刃初級飛羽一片一件 ×5(索引遞減長/遞增後掠外展)'],
    ['翼下武裝', '左翼線圈步槍(tboxF 電容匣 + 加速管 + ICE 線圈環 ×3 + 槍口環)/右翼 latheF 飛彈管 ×3 + 彈尖 + 掛架'],
    ['長腿 ×2(貼地跑)', 'tboxF 收分股殼 + latheF 髖盤/輪轂/螺栓 ×4 + hydCyl 亮腱 + 膝噴口;細長脛楔台 + 前稜 + 亮腱桿雙套環 + 踝噴口'],
    ['二趾足', '踝座 + 斜蹠節 + 二趾各 2 節楔台 + coneF 爪尖 + 後距爪'],
    ['尾羽(fanF)', 'fanF 六片一片一件(edgeF 遞減)指向後上 + latheF 尾根環'],
  ],
  head(c, h) {
    const { PAL } = c;
    // 楔形顱殼:側剖面稜柱(鉤喙基→眉→後掠冠稜一體;rotation.y=-π/2 使剖面 +x → +z 前向)
    const skull = [
      [0.05, -0.12], [0.23, -0.06], [0.20, 0.07], [0.05, 0.15],
      [-0.50, 0.35], [-0.25, 0.03], [-0.12, -0.075],
    ];
    const sk = prismF(h, skull, 0.25, 0, 0.02, 0.0, PAL.main, { metalness: 0.5 });
    sk.rotation.y = -Math.PI / 2;
    // 顱頂中稜(冠稜的立體補強:窄薄稜柱疊在顱中線)
    const ridge = [[0.13, 0.10], [-0.07, 0.17], [-0.45, 0.36], [-0.22, 0.125], [0.0, 0.055]];
    const rg = prismF(h, ridge, 0.06, 0, 0.03, 0.0, PAL.lite, { metalness: 0.55 });
    rg.rotation.y = -Math.PI / 2;
    // 鉤喙(細長楔台;sz>0 → 旋轉後梢端下垂 = 鶴喙微鉤)+ 深色喙尖
    const beak = tboxF(h, { w0: 0.12, d0: 0.10, w1: 0.028, d1: 0.025, h: 0.42, sz: 0.055 }, 0, -0.03, 0.40, PAL.lite, { metalness: 0.55 });
    beak.rotation.x = Math.PI / 2 + 0.10;
    const tip = tboxF(h, { w0: 0.03, d0: 0.028, w1: 0.013, d1: 0.013, h: 0.08, sz: 0.024 }, 0, -0.095, 0.62, INK, { metalness: 0.4 });
    tip.rotation.x = Math.PI / 2 + 0.24;
    for (const sx of [-1, 1]) {
      // 冷焰藍眼(★ 圖圓形鏡頭眼)
      sphF(h, 0.055, sx * 0.115, 0.05, 0.17, ICE, { emissive: ICE, emissiveIntensity: 1.6 });
      // 頰側感測圓盤(latheF 側向盤)+ 藍亮環(細環掛 noOutline)
      const puck = latheF(h, [[0.025, -0.026], [0.075, -0.02], [0.085, 0], [0.075, 0.02], [0.025, 0.026]], 10, sx * 0.14, 0.015, -0.03, PAL.mid, { metalness: 0.7 });
      puck.rotation.z = Math.PI / 2;
      const ring = torusF(h, 0.058, 0.013, sx * 0.165, 0.015, -0.03, ICE, { emissive: ICE, emissiveIntensity: 1.0 });
      ring.rotation.y = Math.PI / 2;
      ring.userData.noOutline = true;
    }
  },
  neck(c, nk) {
    const { PAL } = c;
    // 分節鶴頸:貝茲曲線取樣 8 節 latheF 裝甲袖套(基粗梢細;逐節沿切線定向)
    const N = 8;
    for (let i = 0; i < N; i++) {
      const t = (i + 0.5) / N;
      const [px, py, pz] = bz(NK0, NKC, NK1, t);
      const [, ty, tz] = bzT(NK0, NKC, NK1, t);
      const th = Math.atan2(tz, ty);
      const r = 0.15 - 0.055 * t;
      const seg = latheF(nk, [[r * 0.84, -0.17], [r, -0.05], [r * 0.97, 0.06], [r * 0.80, 0.17]], 8, px, py, pz, PAL.main, { metalness: 0.5 });
      seg.rotation.x = th;
    }
    // 節間束環(每兩節一圈;深色 = ★ 圖的節縫陰影)
    for (const i of [2, 4, 6]) {
      const t = i / N;
      const [px, py, pz] = bz(NK0, NKC, NK1, t);
      const [, ty, tz] = bzT(NK0, NKC, NK1, t);
      const r = 0.15 - 0.055 * t;
      const ring = latheF(nk, [[r * 0.86, -0.032], [r * 1.05, 0], [r * 0.86, 0.032]], 8, px, py, pz, PAL.deep, { metalness: 0.7 });
      ring.rotation.x = Math.atan2(tz, ty);
    }
    // 頸根出殼領環(頸自軀幹殼鑽出處)
    const tc = 0.3;
    const [cx, cy, cz] = bz(NK0, NKC, NK1, tc);
    const [, cty, ctz] = bzT(NK0, NKC, NK1, tc);
    const col = latheF(nk, [[0.155, -0.06], [0.195, -0.01], [0.185, 0.06], [0.145, 0.11]], 10, cx, cy, cz, PAL.mid, { metalness: 0.6 });
    col.rotation.x = Math.atan2(ctz, cty);
  },
  chest(c, ch, d) {
    const { PAL, G } = c;
    // 鳥形軀幹主殼:側剖面稜柱(深龍骨胸 + 背拱 + 後收尾尖;rotation.y=-π/2 → +x 前向)
    const hullP = [
      [-1.15, 0.22], [-0.62, -0.14], [0.28, -0.42], [0.82, -0.12],
      [1.00, 0.28], [0.70, 0.58], [-0.28, 0.66], [-0.88, 0.46],
    ];
    const hull = prismF(ch, hullP, 0.84, 0, 0.52, 0.02, PAL.main, { metalness: 0.5 });
    hull.rotation.y = -Math.PI / 2;
    // 大側甲面 ×2(★ 圖側腹的大稜面甲板)
    const flankP = [[-0.60, 0.02], [-0.20, -0.30], [0.34, -0.32], [0.68, 0.0], [0.38, 0.27], [-0.23, 0.32]];
    for (const sx of [-1, 1]) {
      const fl = prismF(ch, flankP, 0.07, sx * 0.44, 0.47, 0.06, PAL.lite, { metalness: 0.5 });
      fl.rotation.y = -Math.PI / 2;
    }
    // 背鞍板(tboxF 收分;頂面後移 = 後掠)+ 前肩領板
    tboxF(ch, { w0: 0.5, d0: 1.05, w1: 0.3, d1: 0.62, h: 0.2, sz: -0.14 }, 0, 1.14, -0.10, PAL.mid, { metalness: 0.55 });
    const cl2 = tboxF(ch, { w0: 0.4, d0: 0.46, w1: 0.28, d1: 0.32, h: 0.16 }, 0, 1.05, 0.56, PAL.mid, { metalness: 0.55 });
    cl2.rotation.x = 0.35;
    // 腹部波紋內構鼓(latheF 交替半徑肋;軀幹甲縫間外露的深色機構)
    const rib = [];
    for (let k = 0; k <= 8; k++) rib.push([k % 2 ? 0.24 : 0.20, -0.36 + k * 0.09]);
    const drum = latheF(ch, rib, 10, 0, 0.26, -0.34, GUNMETAL, { metalness: 0.75 });
    drum.rotation.x = Math.PI / 2;
    // 胸前冷焰藍識別燈(龍骨上緣)
    const lamp = bxF(ch, 0.2, 0.06, 0.04, 0, 0.74, 0.97, ICE, { emissive: ICE, emissiveIntensity: 1.0 });
    lamp.rotation.x = -0.4;
    // 尾下雙噴口(latheF 喇叭口朝後下 + ICE 光碟;★ 圖尾下藍焰的源頭)
    for (const sx of [-1, 1]) {
      const noz = latheF(ch, [[0.055, 0], [0.10, 0.06], [0.082, 0.15], [0.115, 0.24]], 10, sx * 0.20, 0.26, -1.02, COAL, { metalness: 0.8 });
      noz.rotation.x = -1.95;
      const gl = cylF(ch, 0.06, 0.06, 0.02, 8, sx * 0.20, 0.17, -1.24, ICE, { emissive: ICE, emissiveIntensity: 1.4 });
      gl.rotation.x = -1.95;
    }
  },
  pelvis(c, hips) {
    const { PAL, G } = c;
    // 骨盆束腰(主殼下的窄腰甲)+ 腹下冷焰藍帶
    tboxF(hips, { w0: 0.46 * G, d0: 0.52 * G, w1: 0.42 * G, d1: 0.46 * G, h: 0.26 }, 0, -0.02, 0, PAL.mid, { metalness: 0.55 });
    bxF(hips, 0.2, 0.04, 0.3, 0, -0.16, 0.04, dimF(ICE, 0.8), { emissive: ICE, emissiveIntensity: 0.6 });
  },
  thigh(c, l, d) {
    const { PAL, G } = c;
    // 收分股殼(髖寬膝窄的雞腿量體;tboxF w0/d0 = 底面 = 膝端)
    tboxF(l, { w0: 0.18, d0: 0.26, w1: 0.40, d1: 0.58, h: d.len * 0.92, sz: 0.03 }, 0, -d.len * 0.44, 0.04, PAL.main, { metalness: 0.5 });
    // 髖側大圓盤(★ 圖髖部的大圓輪轂)+ 轂心 + 螺栓 ×4(索引均分)
    const hd = latheF(l, [[0.03, -0.055], [0.24, -0.045], [0.28, 0], [0.24, 0.045], [0.03, 0.055]], 12, c.sx * 0.28, -0.08, 0.05, PAL.lite, { metalness: 0.6 });
    hd.rotation.z = Math.PI / 2;
    const hub = cylF(l, 0.085, 0.085, 0.055, 10, c.sx * 0.34, -0.08, 0.05, PAL.deep, { metalness: 0.8 });
    hub.rotation.z = Math.PI / 2;
    for (let k = 0; k < 4; k++) {
      const a = Math.PI / 4 + k * Math.PI / 2;
      const bolt = cylF(l, 0.024, 0.024, 0.045, 6, c.sx * 0.32, -0.08 + Math.cos(a) * 0.17, 0.05 + Math.sin(a) * 0.17, IRON, { metalness: 0.85 });
      bolt.rotation.z = Math.PI / 2;
    }
    // 膝前護板 + 後側亮腱缸(hydCyl ICE 芯)
    tboxF(l, { w0: 0.14, d0: 0.06, w1: 0.18, d1: 0.08, h: 0.26 }, 0, -d.len * 0.86, 0.16, PAL.lite, { metalness: 0.55 });
    hydCyl(l, 0.03, d.len * 0.5, -c.sx * 0.08, -d.len * 0.55, -0.20, 0.08, ICE);
    // 膝後噴口(★ 圖膝部藍焰)+ ICE 光碟
    const kn = latheF(l, [[0.035, 0], [0.06, 0.05], [0.05, 0.10], [0.07, 0.16]], 8, 0, -d.len * 0.92, -0.22, COAL, { metalness: 0.8 });
    kn.rotation.x = -1.75;
    const kg = cylF(l, 0.038, 0.038, 0.015, 8, 0, -d.len * 0.92 - 0.03, -0.37, ICE, { emissive: ICE, emissiveIntensity: 1.2 });
    kg.rotation.x = -1.75;
  },
  shin(c, l, d) {
    const { PAL, G } = c;
    // 膝球關節 + 膝側小圓盤
    sphF(l, 0.13, 0, 0.02, -0.02, PAL.deep, { metalness: 0.7 });
    const kd = latheF(l, [[0.02, -0.026], [0.085, -0.02], [0.095, 0], [0.085, 0.02], [0.02, 0.026]], 10, c.sx * 0.12, 0.0, -0.02, PAL.lite, { metalness: 0.6 });
    kd.rotation.z = Math.PI / 2;
    // 細長脛(踝端更細的窄楔台 = 鶴腿招牌)+ 前緣稜條
    tboxF(l, { w0: 0.075, d0: 0.11, w1: 0.125, d1: 0.19, h: d.len }, 0, -d.len * 0.5, 0, PAL.mid, { metalness: 0.6 });
    tboxF(l, { w0: 0.05, d0: 0.06, w1: 0.024, d1: 0.05, h: d.len * 0.68 }, 0, -d.len * 0.46, 0.10, PAL.lite, { metalness: 0.55 });
    // 後緣儲能腱桿(亮桿 + 上下端套環;models.js 鴕鳥腱的多面體版)
    cylF(l, 0.02, 0.02, d.len * 0.7, 6, 0, -d.len * 0.44, -0.13, 0xd8dde2, { metalness: 0.9 });
    for (const oy of [-d.len * 0.10, -d.len * 0.78])
      cylF(l, 0.042, 0.042, 0.05, 6, 0, oy, -0.13, COAL, { metalness: 0.8 });
    // 踝後噴口(★ 圖踝部藍焰)+ ICE 光碟
    const an = latheF(l, [[0.03, 0], [0.052, 0.045], [0.044, 0.09], [0.06, 0.14]], 8, 0, -d.len * 0.80, -0.15, COAL, { metalness: 0.8 });
    an.rotation.x = -1.7;
    const ag = cylF(l, 0.033, 0.033, 0.015, 8, 0, -d.len * 0.80 - 0.02, -0.28, ICE, { emissive: ICE, emissiveIntensity: 1.2 });
    ag.rotation.x = -1.7;
  },
  foot(c, l, d) {
    const { PAL } = c;
    // 踝座 + 斜蹠節(短跗蹠向前下斜)
    tboxF(l, { w0: 0.14, d0: 0.16, w1: 0.10, d1: 0.12, h: 0.14 }, 0, -0.10, 0.0, PAL.deep, { metalness: 0.6 });
    const mt = tboxF(l, { w0: 0.08, d0: 0.08, w1: 0.12, d1: 0.10, h: 0.30 }, 0, -0.17, 0.12, PAL.mid, { metalness: 0.6 });
    mt.rotation.x = 1.15;
    // 二趾(鴕鳥腳):每趾 2 節楔台 + coneF 爪尖(rotation.z 外撇)
    for (const [ox, rz] of [[-0.055, 0.12], [0.06, -0.16]]) {
      const s1 = tboxF(l, { w0: 0.065, d0: 0.07, w1: 0.05, d1: 0.055, h: 0.22 }, ox, -0.235, 0.30, PAL.lite, { metalness: 0.6 });
      s1.rotation.x = Math.PI / 2 + 0.10;
      s1.rotation.z = rz;
      const s2 = tboxF(l, { w0: 0.048, d0: 0.05, w1: 0.034, d1: 0.04, h: 0.16, sz: 0.03 }, ox + rz * -0.35, -0.27, 0.48, PAL.lite, { metalness: 0.6 });
      s2.rotation.x = Math.PI / 2 + 0.42;
      s2.rotation.z = rz;
      const cw = coneF(l, 0.034, 0.14, 6, ox + rz * -0.55, -0.285, 0.60, 0xd8dde2, { metalness: 0.75 });
      cw.rotation.x = Math.PI / 2 + 0.62;
      cw.rotation.z = rz;
    }
    // 後距爪(向後下的短爪)
    const sp = coneF(l, 0.035, 0.13, 6, 0, -0.24, -0.13, 0xd8dde2, { metalness: 0.75 });
    sp.rotation.x = -(Math.PI / 2 + 0.5);
  },
  armUp(c, a, d) {
    const { PAL } = c;
    // 肩節圓頂(latheF)
    latheF(a, [[0.14, -0.02], [0.17, 0.03], [0.13, 0.10], [0.0001, 0.15]], 12, 0, 0.02, 0, PAL.main, { metalness: 0.5 });
    // 覆羽主板(prismF 側剖長刃板:自肩沿體側向後掠過軀幹 = ★ 圖疊在背上那片大翼板)
    const covP = [[0.30, -0.10], [0.42, 0.06], [0.10, 0.16], [-0.95, 0.30], [-1.35, 0.16], [-0.60, 0.02]];
    const pl = prismF(a, covP, 0.07, c.sx * 0.05, -d.len * 0.55, -0.05, PAL.main, { metalness: 0.5 });
    pl.rotation.y = -Math.PI / 2;
    pl.rotation.z = c.sx * 0.14;                                  // 板面外傾貼體側
    // 覆羽小扇(fanF 三片一片一件;沿覆羽板下緣指向後下)
    const cov = fanF(a, { n: 3, arc: 0.45, len: 0.6, edgeF: 0.7, gap: 0.022, fin: { w0: 0.15, w1: 0.06, t: 0.03, sweep: 0.10, camber: 0.03 } }, c.sx * 0.06, -d.len * 0.8, -0.35, PAL.lite, { metalness: 0.5 });
    cov.g.rotation.x = -2.35;
  },
  armFore(c, a, d) {
    const { PAL } = c;
    // 肘節環(latheF 側向盤)+ 腕背小甲
    const el = latheF(a, [[0.02, -0.024], [0.075, -0.018], [0.085, 0], [0.075, 0.018], [0.02, 0.024]], 10, c.sx * 0.06, 0.0, 0, PAL.lite, { metalness: 0.6 });
    el.rotation.z = Math.PI / 2;
    tboxF(a, { w0: 0.07, d0: 0.24, w1: 0.09, d1: 0.30, h: d.len * 0.8 }, c.sx * 0.01, -d.len * 0.45, -0.04, PAL.mid, { metalness: 0.55 });
    // 長刃初級飛羽 ×5(finF 一片一件;索引遞減長度、遞增後掠/橫展 = ★ 圖收翼疊列長刃,
    // 近水平向後掠過尾羽;armFore 靜姿世界傾角 ≈ -0.75 ⇒ rotation.x -0.55 起 ≈ 世界後方微上)
    for (let i = 0; i < 5; i++) {
      const f = finF(a, { len: 1.75 - 0.17 * i, w0: 0.20, w1: 0.055, t: 0.038, sweep: 0.20 + 0.04 * i, camber: 0.03 },
        c.sx * (0.02 + 0.02 * i), -0.10 - 0.10 * i, -0.02 - 0.02 * i, i % 2 ? PAL.main : PAL.lite, { metalness: 0.5 });
      f.rotation.z = -c.sx * (0.06 + 0.09 * i);                  // 負號:+y 經 Rz(+θ) 倒向 −x ⇒ 外展要取 −sx
      f.rotation.x = -0.55 - 0.06 * i;
    }
  },
  mount(c, F) {
    const { PAL } = c;
    // 翼下武裝各包一個反傾 Group:armFore 靜姿世界傾角 ≈ -0.75(armBase 0.35 + 肘 -1.1)
    // ⇒ group.rotation.x = +0.75 把砲管扳回世界水平指前(+z)
    const mkWg = (parent, x) => {
      const wg = new THREE.Group();
      wg.position.set(x, -0.30, 0);
      wg.rotation.x = 0.75;
      parent.add(wg);
      return wg;
    };
    // 左翼線圈步槍(輕):電容匣 + 加速管 + ICE 線圈環 ×3 + 槍口環(藏覆羽下)
    const wgL = mkWg(F.foreL, -0.05);
    const box = tboxF(wgL, { w0: 0.16, d0: 0.44, w1: 0.13, d1: 0.36, h: 0.20 }, 0, -0.02, -0.15, GUNMETAL, { metalness: 0.8 });
    const rail = cylF(wgL, 0.030, 0.037, 1.20, 6, 0, -0.10, 0.40, 0x111418, { metalness: 0.85 });
    rail.rotation.x = Math.PI / 2;
    const lN = [box, rail];
    for (let k = 0; k < 3; k++) {
      const cr = cylF(wgL, 0.068, 0.068, 0.05, 8, 0, -0.10, 0.12 + 0.24 * k, ICE, { emissive: ICE, emissiveIntensity: 0.7 });
      cr.rotation.x = Math.PI / 2;
      lN.push(cr);
    }
    const lMuz = cylF(wgL, 0.05, 0.05, 0.05, 8, 0, -0.10, 1.02, ICE, { emissive: ICE, emissiveIntensity: 1.2 });
    lMuz.rotation.x = Math.PI / 2;
    lN.push(lMuz);
    // 右翼飛彈管 ×3(重):latheF 管身(前唇內收)+ 彈尖 ICE + 掛架(斜列藏覆羽下)
    const wgR = mkWg(F.foreR, 0.05);
    const rack = bxF(wgR, 0.06, 0.36, 0.30, 0.04, -0.10, -0.20, IRON, { metalness: 0.8 });
    const hN = [rack];
    const tips = [];
    for (let k = 0; k < 3; k++) {
      const tube = latheF(wgR, [[0.050, -0.26], [0.058, -0.22], [0.058, 0.20], [0.046, 0.26]], 8,
        0.02 + 0.015 * k, 0.02 - 0.12 * k, -0.04 + 0.05 * k, PAL.deep, { metalness: 0.7 });
      tube.rotation.x = Math.PI / 2;
      const tp = cylF(wgR, 0.042, 0.042, 0.04, 8, 0.02 + 0.015 * k, 0.02 - 0.12 * k, 0.23 + 0.05 * k, ICE, { emissive: ICE, emissiveIntensity: 1.3 });
      tp.rotation.x = Math.PI / 2;
      hN.push(tube, tp);
      tips.push(tp);
    }
    const hMuz = tips[0];
    return {
      gunR: null, gunL: null,
      muzzles: { light: { n: lMuz, r: 0.05 }, heavy: { n: hMuz, r: 0.06 } },
      lightGlowM: [lMuz], heavyGlowM: tips, heavyPivot: [],
      weap: { light: 'N', heavy: 'N' },
      hvy: { chest: 0.04 },
      aimPose: null,
      wpn: { light: { nodes: lN, ref: wgL, muz: lMuz, fwd: 'z' }, heavy: { nodes: hN, ref: wgR, muz: hMuz, fwd: 'z' } },
    };
  },
  extra(c, F, rig) {
    const { PAL } = c;
    // 尾羽扇(fanF 六片一片一件;指向後上、橫向張扇 = ★ 圖尾羽)+ 尾根環
    const fan = fanF(F.chest, { n: 6, arc: 0.95, len: 1.05, edgeF: 0.6, gap: 0.024, fin: { w0: 0.17, w1: 0.055, t: 0.038, sweep: 0.14, camber: 0.04 } }, 0, 0.72, -1.06, PAL.lite, { metalness: 0.45 });
    fan.g.rotation.x = -1.40;
    const root = latheF(F.chest, [[0.11, -0.07], [0.155, 0], [0.12, 0.08]], 10, 0, 0.70, -0.98, PAL.mid, { metalness: 0.6 });
    root.rotation.x = -1.40;
    rig.grounded = 1;      // 貼地跑:沒有騰空相、高速浮沉收斂
    rig.tuckArms = 1;      // 收翼蓄勢:前肢不擺大臂
    rig.leanF = 0.35;      // 水平體軸:加速改抬尾配平
    rig.tailUp = 0.16;
  },
};
