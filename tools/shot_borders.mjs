...
].join('\n') + '\nreturn { BORDER_KINDS, borderKindOf, CARPET };')();
...
const report = await page.evaluate(async ({ W, H, ONLY, SUBZ_OBJ, SEEDS }) => {
...
for (const b of report.builds) {
  console.log(`  ${b.zonePair.padEnd(15)} cells=${String(b.cells ?? '-').padStart(5)}` +
              ` border=${String(b.borderPts ?? '-').padStart(6)} 新組合=${String(b.found ?? '-').padStart(3)}` +
              (b.subs ? `  [${b.subs}]` : '') + (b.error ? `  ✗ ${b.error}` : ''));
}
if (report.errors.length) console.log('例外:', report.errors);

// ---- 落檔 + 期望值核對 ----
const got = new Map(report.shots.map((s) => [s.key, s]));
let noBorder = 0, missing = 0;
const rows = [];
for (const p of wantPairs) {
  ...
  // 期望有分界線的組合,畫面裡就必須真的看得到那一種(kindOk);無界組合本來就沒有,
  // 款式兩屬者期望值不可信 ⇒ 不判
  if (p.kind && !p.ambiguous && !s.kindOk) noBorder++;
  rows.push({ ...p, zoneA: SUBZ.get(p.subA), zoneB: SUBZ.get(p.subB),
              shot: `${p.subA}__${p.subB}.jpg`, kindSeen: s.kindSeen, kindOk: s.kindOk,
...
if (noBorder) {   // 只列「有圖卻沒框到線」的;沒出圖的已在上一行單獨列
  console.log('未見預期分界線:', rows.filter((r) => r.shot && r.kind && !r.ambiguous && !r.kindOk)
