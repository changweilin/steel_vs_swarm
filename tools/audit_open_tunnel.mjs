...
// 退出碼:0 = 全綠;1 = 有紅字
//
// ---- 接縫紀律:共面硬幣拋(2026-08-16 併入,`docs/anime_style_plan.md` ④-4;純註解,零斷言改動)----
// 明隧道是全專案**共面面最多**的一族:牆背貼著岩體、矮牆貼著柱腳、頂板貼著側坡、
// collar 貼著洞口斷面。參考專案為此付過學費,結論值得逐字記在這裡:
//
//  ㋐ **同一個高度上的兩個共面面是硬幣拋,不是圖層。** 渲染器會**任意**贏其中一個,
//     而且逐機位不同。判別法只有一句,但它是決定性的:
//     **「一次材質改動如果產生逐像素完全相同的畫面,那絕不是細微的材質問題 —— 是那個面根本沒被畫。」**
//     ⇒ 調了顏色 / 亮度 / 貼圖而 `shot_scene` 的 md5 一格不變時,MUST NOT 推論成「改得太細看不出來」,
//     那是「這個面被另一個面蓋掉了」的**唯一訊號**。
//  ㋑ **三條處方**(哪一條適用看幾何):兩塊同高度的板 ⇒ 一塊**抵住**另一塊的邊線、絕不重疊;
//     被一塊板封頂的量體 ⇒ 量體**縮 0.10 m**、由板子封;掛在洞口上方的東西 ⇒ 離牆面 **≥ 0.10 m**。
//     本支的 Ⅰ/Ⅱ 已經在守「牆背 vs 岩體」那一格的**厚度**(`WALL_MIN`/`NEAR_W`/`OUT_W` 三條件),
//     而共面那一格**沒有斷言** —— 它量不到「這一面有沒有被畫出來」,只有實拍看得到(㋓ `shot_tunnels`)。
//  ㋒ **透明可穿透處是具名例外,不是漏洞。** 柱間(`gal` 遮罩 → slab 第 7 欄 → 伺服器
//     `tunnelSideExit`/`_slabSep` 放行)是刻意的:那裡**看得穿也走得過**。這與 A6b 的
//     「擋得住砲火的面同時 MUST 是站立面」不衝突 —— 兩條規則問的都是「這一面到底存不存在」,
//     而柱間的答案是「不存在」,兩端同判(Ⅳ 在守)。
