...
const TS_NAME = CLOUD ? null : magicDnsName();
...
function ensureCert() {
  ...
  const san = ['DNS:localhost', 'IP:127.0.0.1', ...lanIps().map((ip) => `IP:${ip}`), ...(TS_NAME ? [`DNS:${TS_NAME}`] : [])].join(',');
  try {
...
httpServer.listen(PORT, HOST, () => {
  ...
  if (CLOUD) {
    ...
    for (const a of addrs.filter((x) => x.kind === 'tailscale')) console.log(`  Tailscale:${proto}://${a.ip}:${PORT}`);
    if (TS_NAME) console.log(`  MagicDNS: ${proto}://${TS_NAME}:${PORT}`);
    for (const a of addrs.filter((x) => x.kind === 'lan')) console.log(`  區網:  ${proto}://${a.ip}:${PORT}`);
