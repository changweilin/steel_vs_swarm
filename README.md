...
| **X / Y** | 防守招式 / 攻擊招式(冷卻與就緒直接顯示在鈕面) |
| **L / R** | 填彈 / 狙擊模式(短按切換,**長按 = 機種專屬絕招**) |
| **ZL / ZR** | 下降(飛行機種・觀戰)/ 切換僚機視野(無人機) |
| **HOME / ⊟ / ◫** | 戰場選單(等同 ESC)/ 商店 / 小地圖放大 |
| **轉視角 · Look** | 畫面空處拖曳;**陀螺儀預設關閉**,按十字鍵下的「陀螺」鈕打開後轉動手機即轉動準星(兩者可疊加;開過一次就記住) |
...
**全螢幕不鎖方向** —— 進了全螢幕照樣轉手機切換直式 / 橫式,版型即時跟著換。橫式的**升級工坊**還會把
升級列兩兩左右併列(直式維持單欄),一屏就掃得完整份八軌清單。
