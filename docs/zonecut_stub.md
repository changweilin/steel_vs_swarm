...
1. **同意啟用 `surfaceId = f(zone)`。** 驗收面 3 要的「草↔岩、乾↔濕出線」讓
   ...
   兩者不再衝突：拼圖接縫已退場，真正地貌換手才出線。正式七類 id 取
   `{0, 4, 8, 12, 16, 20, 24}/255`。
2. **不沿用 `'!'` / `null` 語意。** `'!'` 升格為正式 `cliff` 分區；`null` 改以逐面
   影像分類 fallback 補成合法分區，不再留白露出衛星底圖。計畫的次款預算表原只有
...
3. **線分級與面積下限定案為 `road rank ≤ 3` / `areaMinF = 0.0004`。**
4. **同意補 A42。** `venue_field.llToWorld` 改直接呼叫 `data.llToXZ`，結構足跡與執行期
   共用主方位投影，不再保留 pre-A42 手抄。
