...
| 植物原型 | 結構規則與邊界 |
| --- | --- |
| 闊葉、白樺 | 側枝接幹、錯落葉簇與淺色樹皮；一般散布依氣候選種 (`forestBirch`)。 |
| 針葉四款、竹林 | 尖冠／層冠、竹節／多稈，由 spire、tiers、bamboo 骨架抽樣。 |
| 枯木、雷擊木、倒木 | 枯木為相接側枝與破損頂梢，雷擊木帶焦黑裂梢；倒木為橫向幹與沿幹碰撞柱（28° 起排除）。 |
| 肉質植物／灌木 | 基生蓮座葉結構，依氣候選種。 |
| 岩壁松 | 依實測壁面入岩彎根，冠部向上，冠與上幹直接使用 `cliffPine` 生成器。 |
| 坡度限制 | 45° 起須標註 `steep: true`，85° 起略過；通過適生權重判定。 |
| 特殊物 | 蘑菇、鳥巢、岩石不列為植物種類，由專屬管線處理。 |
