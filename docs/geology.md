...
寬高區間僅為預設抽樣範圍；自然地質可用正有限數 `width`、`depth`、`height`（公尺）覆寫。
寬、深及面積沒有美術上限，可指定極大範圍或狹長邊界足跡；`strike` 控制水平朝向。
研究台提供相同的寬深高輸入，留空沿用抽樣。既有戰場名岩仍遵守其放置包絡。

岩土主體最高點相對基底的高度，除以未旋轉足跡的水平對角線 `hypot(width, depth)`，
不得超過型錄的 `maxHeightDiagonalRatio`。抽樣高度可因此低於原高度區間下限；
網格生成後再對包含粗糙度的實際峰高等比壓低，維持平台與水池的相對形狀。
此限制不包含植被碎屑、噴煙／水柱，亦不把承載地形的海拔當作露頭高度。

| 類型 | 最高點／水平對角線上限 |
| --- | --- |
| 河灘、溫泉、天然噴泉基底、間歇泉基底 | 0.06 |
| 地熱地表 | 0.08 |
| 珊瑚礁、土石流 | 0.12 |
| 沙丘、泥火山、隕石坑 | 0.15 |
| 偃塞湖 | 0.18 |
| 土堆、火山口、冰磧丘 | 0.20 |
| 走山、惡地 | 0.25 |
| 島礁、孤立岩丘、崩塌地、噴發火山主體 | 0.30 |
| 山巒、大理岩露頭 | 0.35 |
| 花崗岩塊、砂岩台地、節理岩堆 | 0.40 |
| 峭壁、溶蝕峰、玄武岩柱、刃脊、尖峰、砂岩塔峰、花崗岩塔群、礫岩圓頂峰 | 保留原生陡峭比例 |
