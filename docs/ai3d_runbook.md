...
| **冠層造型路線 — 瓣化(v5~v7 + 降級鏈)vs 簡單幾何版** | **PROTOTYPED 2026-08-07~08**(§5w~§5z;**名冊與檔位待使用者定**)| 使用者手稿「冠層莢化放射 + 樹幹迴轉」→ 逐瓣誤差 → 逐叢瓣化 → 五顆降級旋鈕 → **簡單幾何版**(旁瓣佈局照舊、葉冠整組換基本體:針葉多角錐 / 闊葉橢球或多面體 / 幹枝圓台多面柱 / 尖端細錐;茂密度一顆旋鈕;拿掉葉冠 = 枯木)。佈局收成三個縫(`trunk_cut`/`clump_metrics`/`plan_lobes`),抽出前後 v7 **逐位元相同**。針葉:簡單幾何 `mid` **1,138 面**同時過成長額度(13.3%)與 tree 族 `kind_factor`(2,000),而 v7 的目標是 2,949(只過前者);闊葉主旋鈕是 `--major` 不是茂密度。列圖 `out_simple/sheet_*.png` 五張。**零節點出貨、儲存庫只有 runbook 改動**;原型住 study clone `simple_tree.py`/`canopy_petals.py`/`sheet_simple.py` |
...
額度怎麼分仍是名冊那一輪的事;②**定場圖仍未補**(§5y-h 的未跑①原樣
成立):這一輪全部是黏土側視 720²,遊戲內尺寸 + 賽璐璐 + 勾線的定場圖還沒跑,而**勾線對
基本體是加分**(硬邊多、稜線清楚)還是**扣分**(平面大、線條稀)沒有量過;③**入庫縫未動**:
簡單幾何版是逐株程序生成的**純資料件**,它其實不必走 `img→3D` 的零件庫路線 —— 可以直接
變成 `VEG_DEFS` 的零件表(這是與 v6/v7 最大的差別,也是它最省的地方),要不要這樣做是
使用者的決定;④`--major` 只在兩張闊葉語料上掃過(同 §5y-b 的未跑①);⑤三個新旋鈕
...
(遊戲端是逐株位置雜湊,不是這裡的單一參數);⑨`--lonely .02` 在針葉上清掉了 48 顆裡的 21 顆
(44%)—— 圖上成立,但這個比例偏高,換語料 MUST 重看;⑩淨幹 `--bole .25` 只驗過三張語料,
