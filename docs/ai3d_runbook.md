...
1. **那四顆仍未重生成**,而下一步不是再掃一次語料,是**等使用者放圖**(§5ar ④ 的 `inbox/`)。
   ...
   斜拍、背景乾淨(→ ac_a)。
   **2026-08-14 追加(使用者定案「重生節點」,§5bd-f)**:`chimney_a` 也進這張清單 ——
   ④一支磚砌煙囪 / 通風管,**基座與管身的交界不要有橫向的強陰影線**(SF3D 會把那道陰影
   讀成一道真的凹槽,現役那顆基座上就有一道)。同理 ① 那一張要避開「柱腳之間有深陰影」的
   取景(現役 `mass_a` 柱腳之間那道縫就是這樣來的)。⇒ **這幾處不再排任何刀**。
...
## 5bb. 第四輪:小區塊併入角度最接近的鄰居(2026-08-14)

> 使用者原句:「建築外部,法線角夾角小的**相鄰**平面合併,**相對周邊面積過小**且**角度差異
> 沒有過大**的區塊,與**角度最接近的鄰居**合併,最後收斂成多面柱體 / 錐台 / 角錐 / 圓柱 /
> 圓台 / 圓錐 / 等幾何多面體構成」

### 5bb-a. 前三輪漏了什麼

第一句「法線角夾角小的相鄰平面合併」前三輪已經做了(`_plane_groups`)。**第二句沒有。**
面積佔比在 `min_f` 之下的群是**整塊被略過**的 —— ㋖ 的殘料吸附只救得到「不屬於任何群的
**孤立頂點**」,救不到「自成一小群」的碎塊。實測第三輪出貨的五顆節點:

| | mass_a | mass_b | mass_c | masslow_a | masslow_b |
|---|---|---|---|---|---|
| 分群數 | 651 | 567 | 348 | 225 | 358 |
| 碎鱗率(群數 ÷ 三角形) | 0.223 | 0.196 | 0.120 | 0.078 | 0.123 |
| 面積真的落在大平面上 | 62.7% | 68.3% | 80.5% | 94.0% | 83.8% |

蓋掉九成五面積只要 14~48 片平面,而網格上仍有 225~651 群 —— 那不是多面體,是**一層碎鱗**。
第三句(收斂成那六種幾何體)因此是**這一輪的驗收語**,不是另一個演算法。

### 5bb-b. ㋗:四條紀律

| | 規則 | 為什麼是這樣 |
|---|---|---|
| 1 | 「相對周邊面積過小」= 這塊的面積 ≤ `small_f` × **那個鄰居**的面積 | **這一行就是使用者列的圓柱 / 圓台 / 圓錐的保護**:曲面體的側面每一片都一樣大 ⇒ 比值恆 1 ⇒ 結構上併不掉。拿「佔全體的比例」當判據的話,一根 36 面的圓柱每片只佔 2%,整根會被抹平,而每一條既有斷言照樣全綠 |
| 2 | 「相鄰」= **共邊**(`_face_adj`,邊的鍵經 `rep` 正規化) | 分群本來就不看連通性(那對「是不是同一面牆」是對的),但「碎屑該併給誰」只能由拓樸回答。⚠ `--node` 那條路的 `_planarize` 吃的是還沒焊過的三角形湯 ⇒ 拿原始頂點索引當邊的鍵,每片面自成孤島、㋗ 安靜地什麼都沒併 |
| 3 | 角度上界 `merge_deg` ∈ (`deg`, 45°) | 下界 = 分群容差(在那之內本來就併掉了);上界 45° = 兩面直角牆之間的**倒角**,那是特徵不是雜訊 |
| 4 | **併得過去才併** | 整塊的頂點都要能在累計位移上限之內落到那個平面上。少了這一條就是「拉一半」——一塊本來 30° 的碎屑被拉成 5°,原本不算「法線角小的相鄰對」的它從此算(實測小角佔比反而由 11.0% 升到 19.6%),而畫面上是半平不平 |

併的方向是**小的貼上大的**:被併的那一塊**不進**大群的 `refit` —— 反過來就是讓碎屑把一面
好牆拉歪,那是第一輪 ㋐ 那個坑的另一種寫法。

### 5bb-c. 新的驗收尺:`parts_src.solidConverge`

**兩欄缺一不可**,因為各自都有一種騙得過去的壞法:

- `scales` = 分群數 ÷ 三角形數(碎鱗密度)。單看它,把整顆抹成一顆大球也很「不碎」。
- `onPlane` = 真的貼在某一片**夠大**的平面上的面積佔比。單看它,一層碎鱗每一片都貼在
  自己那一小群上,佔比一樣漂亮。

JS 這一側的平面分群同輪收成 **`wallpanel.planeGroups` 一份**(`wallPanels` 與 `solidConverge`
同吃;`wallPanels` 的行為逐位元不變 —— 重構前後 `intake_parts` 都是 353 / 0)。

### 5bb-d. 結果(Blender 實跑,從**原始** base 起跑)

| | mass_a | mass_b | mass_c | masslow_a | masslow_b |
|---|---|---|---|---|---|
| 碎鱗率 | 0.223→**0.152** | 0.196→**0.123** | 0.120→**0.093** | 0.078→**0.052** | 0.123→**0.068** |
| 貼平面 | 62.7→**74.4%** | 68.3→**82.0%** | 80.5→**83.8%** | 94.0→**97.2%** | 83.8→**91.3%** |
| 牆平整 | 74.6→**80.9%** | 88.8→**94.9%** | 94.7→**95.7%** | 96.8→**99.1%** | 90.7→**94.9%** |
| 小角 | 17.8→19.5% | 7.7→**7.1%** | 5.6→7.4% | 1.2→**0.5%** | 2.9→**1.6%** |
| ㋗ 併入次數 | 1064 | 574 | 548 | 743 | 255 |

⚠ **小角那一欄可以上升**(mass_a / mass_c)。它數的是相鄰近垂直面夾角落在 (0.5°, 12°] 的
面積佔比,而 ㋗ 把本來夾角**大於** 12° 的碎屑拉進那個帶裡。表面確實更平(牆平整 +6.3pp /
貼平面 +11.7pp / 碎鱗率 −32%),是這把尺量不到那一段 —— 同第三輪那條「p90 可以上升」。

`merge_deg` 掃描(Blender 實跑;碎鱗率 / 貼平面):

| merge_deg | mass_a | mass_b | mass_c | masslow_a | masslow_b |
|---|---|---|---|---|---|
| 25 | .188 / 70.5 | .156 / 77.3 | .109 / 81.8 | .062 / 96.1 | .096 / 87.5 |
| 30 | .169 / 69.1 | .139 / 80.1 | .105 / 83.1 | .058 / 96.3 | .084 / 89.3 |
| **40** | **.152 / 74.4** | **.123 / 82.0** | **.093 / 83.8** | **.052 / 97.2** | .068 / 91.3 |
| 45 | .159 / 72.6 | .127 / **83.3** | .099 / 83.5 | .056 / 97.1 | **.060 / 93.0** |

40 在五顆裡四顆碎鱗率最低;45 沒有一致的額外收益而且踩在倒角那條線上 ⇒ 取 **40**。

### 5bb-e. 曲面體保護的**行為直測**

拿 Blender 生的四顆標準幾何體(24 面圓柱 / 24 面圓錐 / 24 面圓台 / 8 面稜柱)餵 `--replanar`:

```
PLANAR cyl24 : 16 趟、末趟整平 26 群、小區塊併入鄰居 0 次(實得最大位移 0.0000)
PLANAR cone24: 16 趟、末趟整平 25 群、小區塊併入鄰居 0 次(實得最大位移 0.0000)
PLANAR frus24: 16 趟、末趟整平 26 群、小區塊併入鄰居 0 次(實得最大位移 0.0000)
PLANAR prism8: 16 趟、末趟整平 10 群、小區塊併入鄰居 0 次(實得最大位移 0.0000)
```

使用者列的那六種幾何體**一個頂點都沒動** —— 這是紀律 1 的結構保證,不是參數調出來的。

### 5bb-f. 重烤與名冊

**兩趟**(同第三輪):①`--replanar` + 舊帶寬 → 量佔比;②從**原始** base 重跑一次把量到的值
烤進 `--uvbands`。新帶寬 `mass` 0.115 / 0.223(舊 0.128 / 0.275)、`masslow` 0.193 / 0.222
(舊 0.203 / 0.259)—— **素牆帶整體縮小 = 真的不平整的牆變少了**(碎屑併進大平面之後那些面
從「近垂直但不平整」轉回窗牆帶)。`BLD_LIB` 第三格(剖面連第五欄)五顆全部重量。

`sign_flat_min` **不動**:分布位移到 … 0.189 |↕| 0.341 …,0.320 仍落在同一個語意空檔之內,
合格的段一格未變(12 / 20)。空檔的幾何中點雖然變成 0.254,取它或取 0.320 挑出來的是
**同一批段** ⇒ 改一個「換了也一樣」的門檻,只會讓下一個人以為那次改動有意義。

### 5bb-g. 驗收

`intake_parts` **363 / 0**(新增收斂兩條 × 五顆);反向驗證 = 把**第四輪之前**的 `building.glb`
餵給它 ⇒ **紅 16 條**(含新的「已收斂成多面體」/「面積真的落在平面上」在 mass_a·mass_b 上各一,
其餘是帶寬與剖面宣告)。`audit_siteplan` **253 / 0**,新增 `--break-merge` 紅 **5** 條;
既有四支 `--break-flat`/`--break-prof`/`--break-glass`/`--break-roof` 各紅 3 / 4 / 2 / 2。

⚠ **同輪修掉一個假綠**:`--break-roof` 的第一個替換綁死了 `masslow: { roof: 0.203, …}` 的
**現值** ⇒ 重量帶寬之後它是**靜默 no-op**,紅字由 2 條變成 1 條而「壞版」根本沒被造出來
(CLAUDE.md §5.4 ㋑ 的同一條)。改成不綁現值的正規式。同輪也把 ⑥-d 行為直測裡寫死的
`roof: 0.128, plain: 0.275` 改成從量測檔讀。

其餘全綠:`auto_intake` 154 / 0・`beacons` 68 / 0・`object_joints` 21611 接合 0 異常・
`open_tunnel` 163 / 0・`underpass` 161 / 0・`road_joint` 86 / 0・`world_text` 57 / 0・
`world_edge` 156 / 0・`ground_tile`/`qc`/`seam`/`enclave`・`client_syntax` 119・`solo_boot`。
`data.js` / `sim.js` / `server/` **一行未改** ⇒ `npm run bal` / `npm test` 結構上不受影響(㋒)。

### 5bb-h. 未做(㋓ / ㋕)

1. **`shot_facades` 與 `shot_scene` 的 `mass_near`/`masslow_near` 沒補**(需要網路圖資):
   素牆帶由 0.308 縮回 0.223 之後那幾棟塔樓的立面讀起來如何,只有定場照看得到。
2. **真機貼牆走一圈**:招牌落點的**合格段沒變**,但剖面本身重量過(`hw`/`hd` 逐段有位移)
   ⇒ 牌面位置會小幅移動。
3. `audit_traverse`(需要網路)未跑 —— 外廓逐位元還原、幾何只在殼裡動,地面層通行寬理應不變。
4. **mass_a 仍是最不平的一顆**(牆 80.9%、碎鱗率 0.152,兩項都是五顆裡最差)。它本來就在
   重生佇列(§5as-f);這一輪把它從「碎鱗」推到「多面體」的邊緣,但沒有把它變成好節點。

## 5bc. 第五輪:合併之前先去噪(2026-08-14)

> 使用者原句:「處理合併平面前,隨機凹凸不平的面先弄平整」

### 5bc-a. 為什麼順序就是重點

`_plane_groups` 的入群條件是「法線角 ≤ `deg` **且** 平面偏移 ≤ `off`」,而 img→3D 的網格表面
帶著隨機起伏:**一面本來是平的牆,逐片面的法線在 ±5° 之間亂跳** ⇒ 貪心分群走到一半就被
一片跳出容差的面切斷,同一面牆因此裂成好幾群。第四輪之後五顆節點仍有 151~443 群,其中
一大塊不是「真的有那麼多面」,是**分群被雜訊切開**。

⇒ 先抹雜訊、再合併。倒過來做不行:群一旦切碎了,後面每一步都是在碎片上做事。

### 5bc-b. 手法:雙邊法線濾波 + 頂點回推(**不是**一般的平滑)

Laplacian / Blender 的 Smooth 修改器會把**真的稜線**一起磨掉 —— 那正是第三輪 ㋔ 花一整輪
修回來的東西。雙邊濾波的值域項 `exp(−‖Δn‖² / 2σr²)` 讓夾角大的鄰居**權重趨近 0**,稜線
兩側互不影響,只有「同一面上的隨機起伏」被平均掉;頂點回推走 Sun 2007 的
`v += (1/|F(v)|) Σ n_f (n_f·(c_f − v))`(法線先濾好,位置再追上去)。

| 參數 | 值 | 從哪裡來 |
|---|---|---|
| `dn_deg`(值域門檻,σr = 2·sin(deg/2)) | 4° | **量到的雜訊本身**(整平前二面角中位數 3.3~4.3°),且 MUST < `flat_deg` 6° —— 平整那條線被去噪抹掉的話,「真的貼在平面上」那道閘就變成恆真 |
| `dn_ss`(空間項) | 1.0 | 相鄰面重心距平均的**倍率**(尺度自帶,不必手寫絕對值) |
| `dn_iter` / `dn_vert` | 6 / 3 | 見 5bc-d |

位移**共用同一份累計夾制**(對原始位置 ≤ `off_f` × 跨距)⇒「整顆離原始掃描最多跑多遠」
仍然只有一個數字。實測去噪只花掉 9~13%(0.0026~0.0036 對上限 0.0285)⇒ 合併那一段的
餘裕沒有變窄。

### 5bc-c. 曲面體保護仍是結構性的

同 §5bb-e 的四顆標準幾何體再跑一次:

```
PLANAR cyl24 : 去噪 6×3 趟(位移 0.0000)… 小區塊併入鄰居 0 次
PLANAR frus24: 去噪 6×3 趟(位移 0.0000)… 小區塊併入鄰居 0 次
PLANAR prism8: 去噪 6×3 趟(位移 0.0000)… 小區塊併入鄰居 0 次
PLANAR cone24: 去噪 6×3 趟(位移 0.0002)… 小區塊併入鄰居 0 次   ← 上限的 0.7%
```

24 面圓柱相鄰面差 15° ⇒ 值域權重 `exp(−6.99) ≈ 0.0009`,濾出來的法線等於它自己。

### 5bc-d. 效果:**孤立**量測是單調的,**接上合併之後**是混沌的

只跑去噪、不跑合併(JS 原型,0 → 10 趟):

| | mass_a | mass_b | mass_c | masslow_a | masslow_b |
|---|---|---|---|---|---|
| 小角 | 53.5→**41.9%** | 63.0→**49.9%** | 55.7→**38.0%** | 52.6→**38.5%** | 64.1→**44.4%** |
| 牆平整 | 53.9→**58.2%** | 61.5→**70.3%** | 78.9→**81.2%** | 90.2→**93.9%** | 73.8→**84.6%** |
| 貼平面 | 46.0→**50.1%** | 46.2→**53.8%** | 59.7→**64.2%** | 83.9→**88.1%** | 64.9→**71.5%** |

**逐項單調** —— 這一欄才是使用者那句話直接量得到的東西。接上合併之後(Blender 實跑):

| | mass_a | mass_b | mass_c | masslow_a | masslow_b |
|---|---|---|---|---|---|
| 碎鱗率 | .152→**.147** | .123→**.115** | .093→.099 | .052→**.049** | .068→.070 |
| 貼平面 | 74.4→**75.2%** | 82.0→**83.9%** | 83.8→82.8% | 97.2→97.2% | 91.3→**91.4%** |
| 小角 | 19.5→**16.7%** | 7.1→**6.9%** | 7.4→**7.1%** | 0.5→0.5% | 1.6→2.1% |
| 牆平整 | 80.9→**82.2%** | 94.9→**96.3%** | 95.7→94.5% | 99.1→98.8% | 94.9→94.7% |

⚠ **下游是混沌的,而這一條要記著**。`dn_iter` 3 / 6 / 10 三個 Blender 實跑的碎鱗率平均是
.0978 / **.0960** / .0998、貼平面 85.22 / **86.12** / 85.70 —— 但**逐顆互有勝負**
(mass_b 在 3 與 10 都比不做還差、在 6 最好)。差異落在貪心分群的離散跳動裡,不是訊號。
⇒ 取 6 的理由是**平均最好** + **孤立量測單調而 6 之後每趟增益減半**;
MUST NOT 把它當成調出來的最佳點,更 **MUST NOT 逐顆挑參數**。

mass_c 小幅退步(碎鱗 .093→.099、貼平面 83.8→82.8)而它的小角改善(7.4→7.1)——
那顆是錐頂節點,去噪把錐面稍微抹圓、分群跟著換了一種切法。

順帶把第四輪留下的那個 caveat 收掉了:**mass_a 的小角 19.5% → 16.7%**,已經低於第三輪
出貨的 17.8%。

### 5bc-e. 重烤與驗收

同樣**兩趟**。新帶寬 `mass` 0.115 / **0.215**、`masslow` **0.192 / 0.219**;`BLD_LIB` 剖面
五顆重量。`sign_flat_min` **仍不動**(0.320 落在新分布的語意空檔 0.190 ↔ 0.353 之內,
合格段仍是 12 / 20)。

`intake_parts` **363 / 0**;反向 = 餵第五輪之前的 GLB ⇒ 紅 8 條(帶寬 + 剖面宣告)。
⚠ 收斂那兩條**不會**在這個反向上紅 —— ㋘ 是 ㋗ 的細修,不是新的地板;它們的負對照是
第三輪那一份(§5bb-g 的 16 條)。`audit_siteplan` **258 / 0**,新增 `--break-denoise` 紅 **5** 條
(參數同一份 / 排在分群之前 / 門檻 < 平整門檻 / 是雙邊濾波 / 共用同一份位移預算);
既有 13 支 break 全數照舊。其餘離線稽核 + `auto_intake` + `audit:net` 全綠;
`data.js` / `sim.js` / `server/` 一行未改。

### 5bc-f. 未做(㋓ / ㋕)

同 §5bb-h 的 1~3(定場照 / 真機貼牆 / `audit_traverse` 都需要網路圖資)。另加:
**去噪是純 Python 的 O(趟數 × 面數 × 鄰居)迴圈**,五顆節點約多花 3 分鐘 —— 只在離線
匯出時跑,執行期一格未動,但重生一整批節點時要把它算進去。

## 5bd. 第六輪:使用者圈的「破洞」量出來不是洞(2026-08-14)

> 使用者原句(附三張圈起來的截圖):「這些地方有破洞,可以透過讓腳本透過演算法自動補上嗎」

### 5bd-a. 先量:三處圈起來的**都不是破洞**
...
| masslow_b | **0** | 0.0% |
| mass_c | **0** | 0.0% |
| mass_a | 28(7 個迴圈,最低點在 y=0.268 = **上半部**) | 0.9% |
| mass_b | 25 | 0.0% |
| ac_a | **167**(23 個迴圈) | 1.3% |

封閉的網格不可能有洞。真正的情況是 **img→3D 的底面內凹成一顆穹頂**:朝下的面裡落在
「離底 0.12 × 跨距」之內的,masslow_a 只有 **6%** ⇒ 整顆量體其實靠一圈**扇貝狀的毛邊**
站在地上,本體的底面吊在半空中。消費端把 yMin 貼齊地面 ⇒ 建物與地面之間留著一圈縫,
從低角度看進去就是使用者圈的那一塊黑。由下往上實拍一張(`node_sheet` 改機位)也看得到:
底面是**鼓起來的封閉曲面**,不是黑洞。

⇒ 要補的不是洞,是**給它一個平的底**。

### 5bd-b. 三次嘗試,前兩次都會靜默弄壞別的地方

| 做法 | 結果 |
|---|---|
| ①**法線泛洪**(從底緣沿「朝下的面」擴散) | 卡在牆上 —— 穹頂與底緣之間隔著近垂直的面,masslow_a 只泛洪到 821 片裡的 30 片 |
| ②**由下往上看得見的全部壓到基準面** | 三處圈起來的**全部修好**,但**上方 90% 的小角佔比 mass_a 19.0% → 43.6%**:中空殼的內頂棚可以高到 0.9 × 跨距,把它拉到底 = 那個高度的外牆被一起扯垮。⚠ 外廓與面數兩條斷言**照樣全綠**,只表現成「牆看起來變皺了」 |
| ③**只壓底緣帶**(`SEAL_RIM_F` = 0.05 × 跨距) | **採用**。扇貝狀毛邊消掉,而平整度指標**逐項持平或更好** |

判據是「**由下往上的第一個命中且朝下**」,不是法線也不是高度:簷口 / 陽台 / 退縮階的下緣
被地面層的量體擋住 ⇒ 第一個命中不是它 ⇒ 一格不動。

**第三道夾制:破口太多就不封底**(`SEAL_OPEN_MAX` = 0.076)。選片只在封閉網格上成立 ——
`ac_a`(破口佔面數 **0.599**)封底之後側面長出一排黑色尖楔(射線從破口鑽進去打到對面的
內面,那一片被拉下來),而外廓與面數兩條斷言照樣全綠。門檻兩側有 **60× 空隙**
(mass_a 0.0096 / mass_b 0.0087 vs ac_a 0.599)⇒ 取幾何中點;ac_a 因此**整顆跳過**。

### 5bd-c. 結果(Blender 實跑;㋘+㋗ → 再加 ㋙)

| | mass_a | mass_b | mass_c | masslow_a | masslow_b |
|---|---|---|---|---|---|
| 碎鱗率 | .147→**.122** | .115→.118 | .099→**.098** | .049→.049 | .070→**.065** |
| 貼平面 | 75.2→**80.0%** | 83.9→81.4% | 82.8→**83.8%** | 97.2→96.9% | 91.4→**91.9%** |
| 小角 | 16.7→**11.4%** | 6.9→9.3% | 7.1→**5.1%** | 0.5→1.4% | 2.1→**2.1%** |
| 牆平整 | 82.2→**85.5%** | 96.3→92.7% | 94.5→**95.2%** | 99.1→98.1% | 94.9→94.5% |

上方 90% 的小角佔比:19.0→**13.1** / 7.2→9.8 / 8.5→**6.0** / 0.5→0.7 / 2.2→**2.0%**
(對照做法 ② 是 43.6 / 27.0 / 23.2 / 7.4 / 3.2%)。

### 5bd-d. 同輪修掉的一個**構造性假紅**

封底之後 `intake_parts` 紅了一條:masslow_b 的「傾斜面收在素牆帶內」。追下去是**一片**
面積佔 **0.10%** 的面 —— 貪心分群是**順序相依**的(匯出端走 Blender 的面序、量測端走 GLB
的三角形序),落在門檻邊上的個位數面本來就會兩邊不同調。舊版驗的是 v 的**極值** ⇒
一片面就能讓它紅字,而這道閘要擋的是「一整片斜屋頂被印上窗」。
⇒ 判據改成**面積佔比** `tiltOutA`,容差 `band_edge_tol = 0.005`(實測那一片是 0.00102,
真的整片印錯是幾十個百分點 —— 兩者差兩個數量級,這不是把閘調鬆)。

### 5bd-e. 驗收

`intake_parts` **363 / 0**。`audit_siteplan` **263 / 0**,新增 `--break-seal` 紅 **4** 條
(兩道夾制 / 破口上限 / 只壓底緣帶 / 只收朝下的命中 / 排在整平之前);
既有 14 支 break 全數照舊。其餘離線稽核 + `auto_intake` + `audit:net` 全綠;
`data.js` / `sim.js` / `server/` 一行未改。四面黏土對照已拍(`node_sheet --ref`)。

### 5bd-f. **沒有修好的、而且是使用者的決定**

`chimney_a` 基座上那道橫向凹槽(離底約 0.10 × 跨距)與 `mass_a` 柱腳之間那道縫,
**都在底緣帶之上** ⇒ 這一輪碰不到它們。它們不是破洞,是重建出來的**形狀**
(照片上的陰影線被 SF3D 讀成一道真的凹槽)。要消掉它們只有兩條路,都不便宜:

1. **形態學閉運算**(偵測窄口深凹並架橋)—— 做法 ② 已經量到代價:同一批頂點下拉會把
   外牆一起扯垮。要做就得換成 voxel remesh,那會**動到三角形預算與拓樸**,
   而且會一併磨掉這幾輪好不容易保住的退縮階與稜線。
2. **重生節點** —— `chimney_a` 與 `ac_a` **本來就在重生佇列**(§5as-f);`mass_a` 是
   五顆裡最不平的一顆(§5bb-h ④)。這一條不需要新的刀,只需要一張乾淨的來源照片。

### 5bd-g. 改制:**凹陷深度上限**(同日,使用者「調整作法」)

> 使用者原句:「如果遇到深度超過該方向寬度 20% 時拉到 20% 的位置,如果是類似甜甜圈這樣
> 穿過去則不用處理」

㋙ 從「封底」擴充成一條**通用的凹陷深度上限**,六個方向逐一由外向內打射線:
首個命中的**內凹量**超過 `CAVITY_F`(0.20)× 該方向寬度 ⇒ 把那一片拉回 20% 的位置。

**三個一開始寫錯、而且錯得很安靜的地方**(都是實跑量出來的):

| | 錯法 | 症狀 |
|---|---|---|
| ㋐ | 深度從**包圍盒**量 | 一根圓柱從側面看,靠邊那幾條射線要走到圓心才碰到面 ⇒ 讀成「深度 **1.000 × 寬**」,誤拉 80 個頂點把圓柱削成方柱。⇒ 深度 MUST 相對於**凸包**(周圍表面)量,凸體的內凹量恆為 **0.000** |
| ㋑ | 「穿過去」逐點往側向打射線 | 甜甜圈的孔在側向被整個環圍住 ⇒ 判成死凹槽,誤拉 **281** 個頂點把它填成一塊圓餅。⇒ 判定 MUST 是**區塊級**:候選格連成區塊,只要碰得到一條**穿過去**的射線就整塊放行 |
| ㋒ | 六個方向一視同仁 | 由上往下那一向會把 masslow_b 的**屋頂**拉出一道對角摺痕 —— 屋頂天井 / 退縮 / 女兒牆與屋脊之間的凹是**建築本身**。⇒ `CAVITY_FROM_TOP = False` |

**行為直測**(四顆標準幾何體 + 三顆合成件,全部 Blender 實跑):

```
cyl24 / cone24 / frus24 / prism8  拉回 0 個頂點(最深的內凹 0.000 × 寬)
donut  拉回 0 個頂點(量到 0.495 × 寬的內凹,但區塊碰得到穿過去的射線 ⇒ 整塊放行)
arch   拉回 0 個頂點(橫向穿過去的通道)
blind  拉回 53 個頂點(0.556 × 寬 ⇒ 拉回 0.20)
```

**代價**(對照只做底緣帶那一版;碎鱗率 / 貼平面 / 小角):
mass_a .122→.160 / 80.0→74.9 / 11.4→21.2、mass_b .118→.103 / 81.4→82.6 / 9.3→17.7、
mass_c .098→.104 / 83.8→79.3 / 5.1→13.2、masslow_a .049→**.036** / 96.9→96.2 / 1.4→8.9、
masslow_b .065→**.055** / 91.9→**93.0** / 2.1→**1.3**。
把深凹拉出來會在凹口拉出斜面 ⇒ 小角那一欄普遍上升,那是這條規則的**直接代價**不是實作漏了什麼;
碎鱗率與貼平面在 masslow 兩顆上反而更好(整片深腔被拉平之後分群少了一大截)。

### 5bd-h. `CAVITY_F` 是掃出來的,不是 0.20(使用者「深度幫我找一個比較好的數值」)

**先看分布**(乾跑 `--basefill-dry` 印的直方圖:內凹量 ÷ 該方向寬度,5% 一箱,六顆平均):

```
0-5%  5-10 10-15 15-20 20-25 25-30 30-35 35-40 40-45 45-50 50-55 …
.559  .139  .060  .032  .0335 .018  .0086 .019  .011  .022  .013
                                     ↑ 谷底
```

**雙峰**:淺浮雕(建築本身的凹凸)自 0 一路衰減,深空腔另成一群(35~50% 又抬起來,
masslow_a 的中空內腔更集中在 45% 以上),兩群之間的谷底在 **30~35%**。

**再掃成本**(Blender 實跑五顆,平均 碎鱗率 / 貼平面 / 小角):

| `CAVITY_F` | 碎鱗率 | 貼平面 | 小角 |
|---|---|---|---|
| 0.20(使用者提的) | .0917 | 85.2% | 12.5% |
| **0.25** | **.0863** | **85.4%** | **9.5%** |
| 0.325 | .0899 | 83.2% | 10.9% |
| 0.40 | .0882 | 84.4% | 9.5% |
| 0.55 | .0930 | 85.6% | 7.6% |

⇒ **取 0.25**:它**三項全部優於 0.20**,而且是掃描的內部最佳點。
0.55 的小角最低是因為它幾乎什麼都不做(碎鱗率反而最差,那是「不處理」的極限)。
直方圖的谷底(30~35%)與成本最佳點(25%)不一致 —— 兩者相距一箱,而 0.25/0.325/0.40 的
差距落在貪心分群的離散跳動裡(同 `dn_iter` 那條警語)⇒ 取值 MUST 看**平均**,
**MUST NOT 逐顆挑**。畫面上 0.25 與 0.20 幾乎一致(四面黏土對照已拍),修正全部保留。

**同輪又抓到一個靜默 no-op**:`--break-seal` 的「凸包基準」那條字面替換只換第一處,而
**乾跑區塊有一模一樣的一行** ⇒ 紅字由 6 條掉成 5 條,而壞版根本沒被造出來。改成全域替換
(CLAUDE.md §5.4 ㋑ 的同一條,這一輪第二次踩到)。

⚠ `chimney_a` 基座那道凹槽與 `mass_a` 柱腳之間那道縫**變淺了但沒有消失** —— 這條規則的
語意就是「把深度夾到 20%」,不是把它填掉。要完全消失仍然只有重生節點那一條路。

**2026-08-14 使用者定案:走第 2 條(重生節點)。** ⇒ 形態學閉運算 / voxel remesh 那一案
**不採用,MUST NOT 自行復辟** —— 它會動到三角形預算與拓樸,並磨掉這幾輪保住的退縮階與稜線,
而換回來的只是兩道陰影線。要修就修**來源照片**:選片時避開「基座與管身交界的橫向強陰影」
與「柱腳之間的深陰影」(已寫進 §5as-f 的放圖清單第 ④ 條)。
