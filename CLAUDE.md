...
| 明隧道(`tunnelWallProfile`/`TUN.*` 系/`gal` 遮罩上傳/sim `tunnelSideExit`・`_slabSep` 隧道分支) | `audit_open_tunnel.mjs`(77 項;**深埋隧道 MUST 逐點同舊制且 gal=0 伺服器逐位元不變**、幾何 `hw`/segs 不動、柱列不進 `cols`、Ⅳ 以真 BattleSim 直測兩端可穿透與 z 鏡射側別)+ `audit_underpass.mjs`(山體/地下道不變式) |
