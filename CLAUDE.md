...
| `public/js/data.js` | 共用常數 UNITS/WEAPONS/ECON/GAME/HAZARDS — **伺服器直接 import 這支客戶端檔**;所有平衡數值只准住這裡 |
...
### 狀態管理與資料流
- 平衡數值(射程/傷害/經濟/波次)**MUST** 只改 `data.js`;**MUST NOT** 在 sim.js/game.js 硬編碼。
...
- `[2026-07-03 #INC-104]` **武器射程與 e2e 耦合**:e2e 多處從 y=250 高空垂直射擊,所有武器 `range` 刻意 > 200(`range*1.25 > 250`);塔 SAM `range: 240` 刻意 < 250(高空探測機不被鎖);e2e `fakeBattleConfig` 用 1600m×L(留防空安全邊界)。改任一數值 **MUST** 同步重驗 e2e,**MUST NOT** 單方面「調平衡」。
