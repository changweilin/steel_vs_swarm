...
- **FPV 座艙與視野(2026-07-12 起)**:第一人稱看出去的東西 = **自己這台機體的結構**,依 `visual` 分派(`game.js`:`_cockAvian`/`_cockFixed`/`_cockRotor`/`_cockProto`/`_cockSkull`/`_cockNeck`/`_buildMorphCockpit`)。**MUST NOT** 退回「無人機一套、機甲一套」的通用座艙。
  - **先決定艙位,再決定看得到什麼(`SEAT`)**:仿生體的艙位由**行進體態**決定 —— 體軸偏直立(人形機甲/猩猩/袋鼠/章魚/狼/吸血鬼/悟空/亞特拉斯)= **頭部艙** `_cockSkull`(看出去是自己的眉骨/頰骨/吻部/獠牙);體軸偏水平(獵犬/暴龍/鴕鳥/劍龍/人馬/迅猛龍/黑豹/巨象/犀金龜)= **頸部艙** `_cockNeck`(看出去先看到自己的**頸背與頭顱**,嘴砲從那顆頭的口中前伸)。無人機:擬態獸看得到自己拍動的翼與口器、定翼機是罩框+機鼻(零式隔著旋轉螺旋槳)、旋翼機是機架與旋翼。
