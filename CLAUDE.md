...
| 閃避範圍 | `EVASION` + `evadable()`;擲骰 `sim._dodges()`;爆風那一半住 `sim._blast`(逐目標) | 2026-08-11 使用者定案「爆炸傷害就算沒擊中原先的目標,也會造成範圍傷害(閃避率各自計算)」⇒ **輕武器直射 + 一切爆炸傷害**(爆炸型重武器 / 攻擊招式 / 三種載具戰鬥部 / NPC 肩射火箭)全吃閃避;扇形與貫穿**依機制豁免**(錐/圓柱一次掃整排,沒有「這一發瞄的是誰」)。判據 MUST 寫成排除法(`aoeClass` 不是 fan/line) —— 列舉「`id === 'light'` 或有 `r`」會在 NPC 那半靜默失效(小兵武器 def 沒有 `id`)。爆風 MUST 逐目標 `continue`,**MUST NOT 整發 `return`**(一台閃 = 全隊免傷,而傷害數字一個都不會跳);自損(`friendly`)刻意不擲。四個消費端(sim / duel / lanesim / balance)MUST 全吃。稽核 `audit_weapon_gate` ⅩⅢ |
...
| 閃避範圍(`evadable`)/ 爆風逐目標擲骰 / NPC 爆炸型武器 | `audit_weapon_gate` ⅩⅢ + **反向驗證三支**(拿掉爆風擲骰 / 把 `continue` 改成 `return` / NPC 分支改回單體直擊,三支 MUST 各自紅字)+ **`npm run bal` 全綠**(①④⑤⑦ 都會位移:AoE 對機動機體的期望輸出整組 ×(1−p);⑦e 的對局長度最敏感)+ `npm test`(**MUST 先重啟伺服器**)+ `audit_speed_comp`(閃避門檻同一條速度軸)+ ㋔ |
