...
| 平整垂直牆面板(切法 + 窗格對齊) | **`public/js/wallpanel.js`**(`PANEL`/`wallPanels()`/`splitByPanel()`/`panelGridUV()`/`panelCells()`);消費端 `biomes.alignedGeo()`(執行期窗格對齊)+ `parts_src.flatWalls()`(離線量測);門檻 `MASS.PANEL` = `tri_budget planar_spec` | 2026-08-13 使用者「平面區域太小的話不渲染窗戶,窗戶會被裁切掉的時候也不渲染」。本檔**零 import** ⇒ 遊戲端與離線工具吃同一份面板定義。格數 `round` 不 `floor`、round 成 0 的面板整片改吃素牆帶、跨面板共用頂點先拆、對齊只換 uv。詳見 A46 ⑧;稽核 `audit_siteplan` ⑥-d(含行為直測) |
| 平面整平與「這面牆平不平」 | 刀 `normalize_parts.py` 的 `PLANAR_*`+`_plane_groups()`+`_planarize()`+`--replanar`;尺 `parts_src.mjs` 的 `meshFaces()`/`flatWalls()`(分群轉呼 `wallpanel.wallPanels`)/`wallFlatness()`;規格 `tri_budget.json families.building.planar_spec`;消費端 `--uvbands` 的窗牆帶資格 + `nodeProfile` 第五欄 | 2026-08-13 使用者「建築外部不平整的多塊法線角小的平面牆合併平整」。**刀與尺刻意各寫一份**(抄同一份程式碼只驗得到「我抄對了」);參數只有 `planar_spec` 一份,匯出端與量測端同吃。分群 MUST 同時看法線角與平面偏移、位移上限 MUST 與合併容差是同一個數字且夾**累計**量、MUST 只動位置不動拓樸。驗收尺 = 「相鄰近垂直面夾角落在 (0.5°, `deg`] 的面積佔比」(實測 52.6~64.1% → 6.9~31.2%)。詳見 A46 ⑥;稽核 `audit_siteplan` Ⅴ ±`--break-flat` + `intake_parts` |
...
| **平面整平 / 窗牆帶的平整條件 / 招牌落點的平整條件 / 窗格輪廓 / 窗格貼齊面板**(`normalize_parts.py` 的 `PLANAR_*`·`_plane_groups`·`_planarize`·`--replanar`·`--uvbands` 後兩欄 / **`public/js/wallpanel.js` 全檔** / `parts_src` 的 `meshFaces`·`flatWalls`·`wallFlatness` / `tri_budget` 的 `planar_spec`·`sign_flat_min` / `MASS.UVB.FLAT_*`·`SIGN_FLAT_MIN`·`PANEL` / `bldFaceList` / `biomes.alignedGeo`·`pitchGrid` / `facadeTex` 的 `snap`·`pane`·`wrapS`·`FACADE_PX.FRAME`·`ANISO`) | `audit_siteplan` Ⅴ ±**`--break-flat`**(MUST 紅 3 條)+ **⑥-d 的行為直測**(真的切一次面板、跑四種網格:格數恆為 ≥1 整數、u 跨距恆為 1/cols 的整數倍、v 恆收在窗牆帶內、共用頂點真的會出現)+ `audit_client_syntax`/`audit_solo_boot`/`audit:net`(新增客戶端模組 `wallpanel.js`)+ **`intake_parts`**(從成品 GLB 重量平整度;反向驗證 = 把**整平前**的 `building.glb` 餵給它,MUST 紅 22 條,其中「已合併整平」與「真的平整的 ≥ 60%」各節點都要紅)+ **重烤 MUST 兩趟**(帶寬是「該群面積佔比」⇒ 先 `--replanar` 拿到整平後的幾何、量出佔比,再從**原始** base 重跑一次把量到的值烤進 `--uvbands`;拿整平後的 GLB 再整平一次 = 這一顆被推了兩倍)+ 名冊三處 MUST 同步(`BLD_LIB` 第三格連第五欄、`MASS.UVB`、`tri_budget`)+ `audit_auto_intake` + `audit_object_joints --seeds 8` + `audit_beacons` + `audit_client_syntax`(㋖)+ **`npm run bal` / `npm test` MUST 逐項不動**(`data.js`/`sim.js` 一行未改)+ **㋓ `shot_facades`**(窗框與硬邊只有貼圖排面看得到;量化判準 = 窗邊「過渡 texel」佔比 MUST 從 22~30% 掉到個位數)+ **㋓ `shot_scene` 的 `mass_near`/`masslow_near`**(素牆帶長大之後那幾棟塔還讀不讀得出是辦公樓)+ **㋕ 貼著塔走一圈**(招牌不再浮在尖塔前面) |
