...
- **站立表面 = 地形 ∪ 高架橋面(2026-07-12 起)**:橋樑/地下道是**可通行的結構物**,不是佈景。
  ...
  - 地下道走**開挖式(cut-and-cover)**:地形是高程場、沒有「地下」可鑽 ⇒ 路面仍貼地,兩側立擋土牆 + 上方橫樑 + 山體端門洞。**MUST NOT** 退回「隧道整條不畫」。
  - 橋面/地下道路寬夾在 `PASS_W`(16 遊戲公尺)以上 —— NPC 與玩家並肩通過不互相卡住。
- **兵線指引 = 3D 箭頭,不是線(2026-07-12 起)**:`game.js _initLanes()` 沿兵線佈扁平箭鏃(InstancedMesh,指向敵方主堡):直線段每 55m 一支、**航向變化 > 0.22 rad 的轉角一定有一支**、任兩支間距 ≥ 26m(轉角優先)。箭身壓扁 + 抬頭 24°(從後方跟上來的玩家看到的是箭的上表面;立體角錐從背面只會變成菱形)。**MUST NOT** 退回連續折線。
