...
- **機甲蓄力跳躍 + 無敵幀(2026-07-16;`data.js CJUMP/IFRAME`)**:`kind:'robot'` 長按 Space 蓄力(重心下沉/減速與 morph 共用 `this.charge`)→ 放開彈射高跳,騰空低重力(×`GRAV_F` 0.45)+ 低阻力滑行 = 太空漫步;蓄力不足 = 普通小跳(**tap 跳改為放開才觸發**)。**morph 的長按 Space 已被變形彈射佔用,MUST NOT 幫 morph 加蓄力跳**。無敵幀:蓄力跳「中段」(升速衰減過半)與 morph 變形(launch/land)時客戶端送 `{t:'iframe'}` → `sim.heroIframe` 驗 20s CD 後給 1s 免傷(`h.invUntil`,`_damage` 早退 + `_applyCC` 免疫;時長/CD 夾在伺服器,客戶端只能決定何時用);無人機一律被拒。高跳越過 `GAME.AA_MIN_ALT` 照樣吃塔 SAM(y 判定規則不變,刻意風險)。
...
**全角色施法/跳躍稽核**(動 stepCastPose/stepJumpPose/CAST_SIG 後 MUST 跑):同上範式,對 32 角各設 `ent.castFx = {t0, slot, dir}`(skill/ult × 全向/定向四組合)跑到自清、再餵 `ent.heroY` 拋物線(morph 頂點壓 1.0m —— 超過 1.2m 是變形彈射,不屬跳躍)跑兩輪,檢查:①全程無 NaN ②castFx 到時自清 ③靜止後包圍盒高度回歸基線 ±20%(**aerial 免驗高度** —— 擬態獸翼靜止仍慢拍,包圍盒隨相位變動是常態)。抓的就是「動了步態不重賦值的通道 → 動作結束永久歪掉」這類 bug。
