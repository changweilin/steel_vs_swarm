...
| **橋交會去重**(`biomes.js dedupeCrossingBridges`/`polylinesMeet`/`CROSS_GAP` + buildBiomes 呼叫點)| `node tools/audit_bridge_crossing.mjs`(8 項離線直測,**執行 biomes.js 原文**):兩橋幾何相交/交會 → 低優先者**整條剔除**,優先度 兵線補橋(`laneWetPieces`)>大馬路(`roadWidth` 大)>小馬路;平行不交會兩條都留(平行堆疊歸 `dedupeParallelBridges`)/ 等寬 → 長者保留 / 換順序仍依 rank。**含立體交叉**(使用者定案,與舊 dropLaneBridges「立體交叉保留」相反)。MUST 排在 `roadInput` 定案前(三消費端同一份)。**改完 MUST 反向驗證**:把 `polylinesMeet` 改恆 false ⇒「交會偵測失效」對照組紅字。**已知缺口**:真圖資的視覺確認待真機(倫敦 `scen:['underBridge']` 若該真橋與兵線補橋交叉會被剔除 —— 依使用者定案「兵線恆勝」,browser 冒煙複核)|
| **馬路橫切繞行**(`biomes.js skirtWaterClips`/`SKIRT_NEAR\|OPEN\|MAX\|STEP\|CROSS_MIN` + `splitWaterPieces` 的 `!inclSwamp` 呼叫)| `node tools/audit_water_skirt.mjs`(8 項離線直測,**執行 biomes.js 原文** + 合成地形):判定 = 垂向雙側量到乾地距 lo/hi —— **貼邊橫切 = 不對稱**(一側近岸 `lo<SKIRT_NEAR`、另一側開放水域 `hi≥SKIRT_OPEN`)MUST 內部貼邊頂點推到乾地繞行;**橫跨對岸 = 對稱/兩側皆近**(`lo≈hi` 恆非貼邊 ⇒ **含斜交穿越**)MUST 建橋不繞。直角穿越 + **45° 斜交 40m 河**(`SKIRT_OPEN>SKIRT_NEAR` 才保證斜交對稱穿越不被誤繞 —— 複審抓到的回歸)+ 連續橫跨型弧長 ≥ `SKIRT_CROSS_MIN` 才判整段建橋。**只作用真 OSM 道路**(`!inclSwamp`;兵線/離線備援/真 bridge=yes way 不繞)。**改完 MUST 反向驗證**:①`skirtWaterClips` 改 no-op ⇒「貼邊繞行」紅字 ②去掉「另一側開放」條件(回舊版單看近岸)⇒「斜交穿越」紅字。**已知缺口**:真圖資的繞行路徑視覺待真機(跨水橋 vs 邊角橫切)|
| **橋上砲塔墩座台**(`biomes.js planTowerBridgePads`(純幾何單一縫)/`TOWER_PAD_AXIS`/`TOWER_PAD_R`/`TOWER_BASE_R`;`buildTowerBridgePads` 只建 mesh)| `node tools/audit_bridge_tower_pad.mjs`(19 項離線直測,**執行 biomes.js 原文** + `makeDeckIndex`):砲塔沿橋軸走位帶 ≥ 基座半徑 + 8m(舊 `TOWER_PAD_R=10.5` 僅 ~7.2m ⇒ 掉橋)、貼基座繞行整圈無掉點、`TOWER_BASE_R` 不變(碰撞/LOS/平衡不動)、視覺板沿橋軸半寬 = 站立面 = `TOWER_PAD_AXIS`、寬橋內砲塔不加外伸台仍可繞行。**改完 MUST 反向驗證**:`TOWER_PAD_AXIS` 寫回舊值(10.5)⇒「砲塔可繞行」對照組紅字。**已知缺口**:倫敦跨河橋真機冒煙(繞砲塔左右側不掉橋)|
