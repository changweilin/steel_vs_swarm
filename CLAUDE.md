...
| 觀戰相機(`SPEC_CAM`/`specViewNext`/`camSmoothF`/`camAngleStep`/`wrapPi`・`game._updateSpectator`/`_specSetView`/`_specCycleView`/`_specFollow`/`_specHud`/`_specRoster`・觀戰鍵位 `_onKey` 的 F・Q/E 與 `_cmd` 的 special・swap・dive・玩家資訊面板 `main.renderSpecUpg`/`hud.self` 的 spec 分支/`#specUpg`/`body.spectating`) | `audit_spectator_cam.mjs`(90 項;Ⅰ 視角序單一縫 + `_specView` 恰兩個寫入點 + game.js 無本地常數表、Ⅱ **平滑幀率無關直測**(30/60/240fps 追隨 1 秒差 < 0.5m 且 = 解析解)+ 角度最短路徑/不過衝/無穩態誤差、Ⅲ 以真品 `_updateSpectator` 原文行為直測:三顆升降鍵(含 Ctrl)與 `MOVE_MPS`・`BOOST_F` 推導、下降地板夾制、四種視角的相機解、只有第一人稱藏機體且換視角/退場都還原、小位移平滑而 > `SNAP_M` 直接貼上、第三人稱不鑽地、Ⅳ 鍵位與派發單一縫 + help.js 四種視角名稱齊全、Ⅴ 面板欄位全來自快照 + 八軌吃 `ECON.UPGRADES` + 面板 DOM 只有一塊)+ `audit_ctrl_mode.mjs` Ⅶ(觸控鈕位/鈕面字/版型)+ `audit_touch_layout.mjs` + `audit_ui_layout.mjs` + `npm test`(㋒ 純表現層,MUST 仍全綠)+ 真機冒煙(上帝視角按住 C 或 Ctrl 一路降到地面就停住;F 連按四下走完四種視角並回到上帝視角;第三人稱看得到自己跟的那台機體、鏡頭平順不抖;第一人稱視角跟著該玩家轉頭;跟隨中左下角顯示該玩家裝甲/護盾/電力/武器/招式冷卻/金錢/擊殺與八軌升級、離開跟隨即收起) |
