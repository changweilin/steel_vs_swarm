...
const {renderer,scene,pipeline}=window.forestReview;
const {stepCelWind,disposeTree}=await import('/public/js/toon.js');
const trees=scene.children.filter(m=>m.isInstancedMesh);
const triangles=trees.reduce((sum,m)=>sum+m.geometry.index.count/3,0);
const bendMatches=trees.every((m,i)=>i%2===0 || ['span','flex','lag','rate'].every(key=>m.material.userData.celOpts.soft[key]===trees[i-1].material.userData.celOpts.soft[key]));
pipeline.render();const before=renderer.domElement.toDataURL();
for(let i=0;i<32;i++)stepCelWind(.25);pipeline.render();const moved=before!==renderer.domElement.toDataURL();
const memoryBefore=renderer.info.memory.geometries;
for(const m of trees){scene.remove(m);disposeTree(m);}pipeline.render();
return {draws:trees.length,triangles,bendMatches,moved,released:memoryBefore-renderer.info.memory.geometries};
});
console.log(JSON.stringify(audit));
await browser.close();
if(errors.length || audit.draws!==10 || !audit.bendMatches || !audit.moved || audit.released!==10)process.exitCode=1;