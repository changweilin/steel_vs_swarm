...
for (let seed = 0; seed < 80; seed++) {
  for (const kind of ['house', 'ranch']) for (const size of [[26, 12, 20], [12, 20, 18], [40, 8, 14]]) {
    ...
    const roof = rows.filter(p => p.role === 'roof-slope'), gables = rows.filter(p => p.role === 'house-gable');
    const body = partBox(rows.find(p => p.role === 'building-body'));
    assert.equal(roof.length, 2);
    assert.equal(gables.length, 4);
    const pitch = Math.abs(roof[0].r[0]), spec = ENVIRONMENT_STRUCTURE_PARAMETERS.houseRoof;
    assert(pitch >= spec.pitch[0] && pitch <= spec.pitch[1]);
    for (const gable of gables) {
      close(partBox(gable).y0, body.y1, 'gable rests on wall');
      const vertices = gable.g[1].vertices;
      for (let i = 0; i < vertices.length; i += 3) {
        const v = world(gable, vertices.slice(i, i + 3));
        const ridgeZ = (body.z0 + body.z1) / 2;
        const upperY = body.y1 + ((body.z1 - body.z0) / 2 - Math.abs(v[2] - ridgeZ)) * Math.tan(pitch);
        if (Math.abs(v[1] - upperY) > 1e-7) continue; // Lower centre vertices are inside the gable.
        const slab = roof.find(p => (p.p[2] - ridgeZ) * (gable.p[2] - ridgeZ) > 0);
        const local = mat3Apply(mat3Transpose(mat3FromEulerXYZ(slab.r)), v.map((n, k) => n - slab.p[k]))
          .map((n, k) => n / (slab.s?.[k] || 1));
        close(local[1], -slab.g[2] / 2, 'gable upper edge touches roof underside');
      }
    }
    ...
    variants.add(posts.length + ':' + pitch);
  }
}
assert(variants.size > 30, 'seeded roofs and fences vary');
console.log('PASS: 80 seeds × 3 envelopes of closed house gables, supported ranch fences, braced gates and grounded silos.');
