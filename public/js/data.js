...
export const WEAPON_CONE_TAN = SQRT3;         // 60° 圓錐母線斜率 tan(60°) = √3 ≈ 1.732
export const WEAPON_SPHERE_R_F = SQRT3 / 2;    // 上段球體半徑比例 r = d * √3 / 2 ≈ 0.866 d
export const WEAPON_TRANS_Y_F = SQRT3 / 4;     // 上段球體與中下段圓錐相切過渡高度 dy = d * √3 / 4 ≈ 0.433 d

/**
 * 給定平面射程 d (即原本武器 def.range) 與目標相對射手垂直高差 dy = y_target - y_shooter，
 * 計算該垂直高度下允許的最大水平有效射程 rho_max (單一真相縫)：
 *   - dy > d * √3 / 2 (超出球頂)：0
 *   - d * √3 / 4 <= dy <= d * √3 / 2 (上段球體)：√( (d * √3 / 2)^2 - dy^2 )
 *   - dy < d * √3 / 4 (中下段 60° 圓錐體)：d - dy / √3  (向下俯射 dy < 0 時射程自然展開)
 */
export function weaponMaxHoriz(d, dy) {
  if (d <= 0) return 0;
  const yTrans = d * WEAPON_TRANS_Y_F;
  if (dy >= yTrans) {
    const r = d * WEAPON_SPHERE_R_F;
    if (dy >= r) return 0;
    return Math.sqrt(r * r - dy * dy);
  }
  return Math.max(0, d - dy / SQRT3);
}

/**
 * 判定目標點 (dx, dz, dy) 是否落在以 (0,0,0) 為射擊點、平面射程 d 的射程包絡內。
 * hitR: 目標水平碰撞量體半徑（量到表面，surf = max(0, hypot(dx, dz) - hitR)）。
 * mul: 額外倍率（如網路寬容 RANGE_TOL、buff 等）。
 */
export function inWeaponRange(d, dx, dz, dy, hitR = 0, mul = 1) {
  const effD = d * mul;
  if (effD <= 0) return false;
  if (dy > effD * WEAPON_SPHERE_R_F) return false;
  const maxH = weaponMaxHoriz(effD, dy);
  const horiz = Math.hypot(dx, dz);
  const surf = Math.max(0, horiz - hitR);
  return surf <= maxH;
}
...
export const CHARACTERS = {
  ...
  s01: {
    ...
    skill: { name: '巴哈賦格：共鳴護盾', fx: 'buff', target: 'self', spRestore: [60, 90, 120], shieldExpand: true,
      mul: { dmgTaken: [0.8, 0.75, 0.7] }, dur: [6, 7, 8], cd: [20, 18, 16], mp: [35, 40, 45], desc: '奏響防護協奏：同調機體力場頻率，直接補充磁力並在防守時大幅擴大護盾面積' },
  ...
  s02: {
    ...
    skill: { name: '神爐回火：磁力重鑄', fx: 'buff', target: 'self', spRestore: [60, 90, 120], shieldDefBoost: [0.6, 0.5, 0.4],
      dur: [6, 7, 8], cd: [18, 16, 14], mp: [35, 40, 45], desc: '全功率開啟應急納米回火：直接充盈磁力護盾，並在防守時大幅提高護盾傷害減免效果' },
  ...
  s03: {
    ...
    skill: { name: '天海長歌：電磁屏障', fx: 'buff', target: 'self', spRestore: [50, 75, 100], shieldExpand: true, shieldDefBoost: [0.65, 0.55, 0.45],
      dur: [6, 7, 8], cd: [18, 16, 14], mp: [35, 40, 45], desc: '展開長耳微波電磁屏障：直接補充磁力並擴大防守護盾面積，提高傷害減免效果' },
  ...
  s04: {
    ...
    skill: { name: '修羅狂瀾：護盾衝撞', fx: 'shield_bash', shieldBash: true, imp: 28, dmg: [60, 80, 105], r: 12,
      mul: { speed: [1.25, 1.35, 1.45], dmgTaken: [0.75, 0.7, 0.65] },
      dur: [5, 6, 7], cd: [18, 16, 14], mp: [30, 35, 40], desc: '修羅突進：防守姿態下持護盾高速衝撞前方，擊退並震暈沿途敵軍，期間大幅減免承受傷害' },
  ...
  s05: {
    ...
    skill: { name: '納米修復：磁力再生', fx: 'buff', target: 'self', spRegenHit: true, spRestore: [45, 65, 90],
      dur: [6, 7, 8], cd: [18, 16, 14], mp: [35, 40, 45], desc: '啟動應急奈米修復網：直接修復磁力，且在時效內即便受到敵火攻擊亦不中斷磁力護盾回充' },
  ...
  s06: {
    ...
    skill: { name: '聖光穹頂：絕對庇佑', fx: 'intercept', r: [140, 170, 200], intercept: true, shieldDefBoost: [0.55, 0.45, 0.35],
      dur: [6, 7, 8], cd: [20, 18, 16], mp: [35, 40, 45], desc: '展開神聖攔截力場：粉碎半徑內來襲飛彈，並在防守時大幅提高護盾減免效果' },
  ...
  s07: {
    ...
    skill: { name: '非歐折射：護盾偏折拓撲', fx: 'buff', target: 'self', shieldBash: true, shieldDefBoost: [0.55, 0.45, 0.35],
      dur: [6, 7, 8], cd: [18, 16, 14], mp: [35, 40, 45], desc: '幾何護盾拓撲偏折：在防守時大幅提高護盾減免效果，並賦予護盾衝撞擊退能力' },
  ...
  s08: {
    ...
    skill: { name: '聖城鐘聲：聖域庇護', fx: 'heal', target: 'self', heal: [120, 160, 200], sp: true, spRestore: [60, 85, 110], shieldExpand: true,
      dur: 6, cd: [22, 20, 18], mp: [40, 45, 50], desc: '敲響聖城大鐘：直接修復機體裝甲與磁力，並在防守時擴大護盾保護面積' },
  ...
  s09: {
    ...
    skill: { name: '獵手敏捷：防守躍遷', fx: 'buff', target: 'self', defJump: 2, mul: { speed: [1.2, 1.3, 1.4] }, spRestore: [40, 60, 80],
      dur: [6, 7, 8], cd: [18, 16, 14], mp: [35, 40, 45], desc: '獵場本能爆發：立即補充磁力與加速，防守姿態下追加連續大跳躍次數' },
  ...
  s10: {
    ...
    skill: { name: '始祖威儀：防守衝撞', fx: 'shield_bash', shieldBash: true, imp: 26, dmg: [55, 75, 100], r: 12, spRestore: [40, 60, 80],
      shieldDefBoost: [0.6, 0.5, 0.4], dur: [5, 6, 7], cd: [18, 16, 14], mp: [35, 40, 45], desc: '始祖巨翼衝鋒：持護盾強勢衝撞前方擊退敵軍，立即補充磁力並提高護盾傷害減免' },
  ...
  s11: {
    ...
    skill: { name: '時序重調：神木巨盾', fx: 'buff', target: 'self', spRestore: [60, 90, 120], shieldDefBoost: [0.55, 0.45, 0.35],
      dur: [6, 7, 8], cd: [20, 18, 16], mp: [35, 40, 45], desc: '格拉蘇蒂重組力場：直接補充磁力，大幅提高護盾傷害減免效果' },
  ...
  s12: {
    ...
    skill: { name: '星穹誓約：奇點護盾', fx: 'buff', target: 'self', spRestore: [60, 90, 120], shieldExpand: true, shieldDefBoost: [0.6, 0.5, 0.4],
      dur: [6, 7, 8], cd: [20, 18, 16], mp: [35, 40, 45], desc: '奇點引力透鏡護盾：直接充盈磁力，防守時大幅擴大護盾面積並提高減傷效果' },
  ...
  t01: {
    ...
    skill: { name: '霜狼巨盾：雪原衝撞', fx: 'shield_bash', shieldBash: true, imp: 28, dmg: [65, 90, 120], r: 12,
      mul: { speed: [1.2, 1.3, 1.4], dmgTaken: [0.75, 0.7, 0.65] },
      dur: [5, 6, 7], cd: [20, 18, 16], mp: [35, 40, 45], desc: '霜狼巨盾極地衝鋒：防守姿態下持重盾強勢衝撞，擊退並震暈沿途敵軍，減免承受傷害' },
  ...
  t02: {
    ...
    skill: { name: '神經超載：防守大跳躍加速', fx: 'buff', target: 'self', defJump: 2, mul: { speed: [1.3, 1.4, 1.5] }, spRestore: [40, 60, 80],
      dur: [6, 7, 8], cd: [18, 16, 14], mp: [30, 35, 40], desc: '超頻神經訊號：立即補充磁力並大幅提高移速，防守姿態下追加連續大跳躍次數' },
  ...
  t03: {
    ...
    skill: { name: '不屈鐵壁：防護擴張衝撞', fx: 'shield_bash', shieldBash: true, imp: 30, dmg: [70, 95, 125], r: 12, shieldExpand: true,
      spRestore: [40, 60, 80], dur: [5, 6, 7], cd: [20, 18, 16], mp: [35, 40, 45], desc: '熔岩巨盾狂暴突進：防守姿態下持盾衝撞擊退敵機，並擴大護盾面積且補充磁力' },
  ...
  t04: {
    ...
    skill: { name: '全息干涉：幻影偽裝', fx: 'decoy_beacon', spRestore: [50, 75, 100], shieldDefBoost: [0.6, 0.5, 0.4],
      dur: [6, 7, 8], cd: [18, 16, 14], mp: [35, 40, 45], desc: '投擲全息防護信標：干擾敵方火控，立即補充磁力並在防守時提高護盾減傷效果' },
  ...
  t05: {
    ...
    skill: { name: '九轉玄功：磁力應力自癒', fx: 'heal', target: 'self', heal: [120, 160, 200], sp: true, spRestore: [60, 90, 120], spRegenHit: true,
      dur: [6, 7, 8], cd: [20, 18, 16], mp: [35, 40, 45], desc: '仿生應力自癒：立即修復機體與磁力，且時效內受擊仍可持續回充磁力' },
  ...
  t06: {
    ...
    skill: { name: '筋斗騰雲：防守筋斗躍遷', fx: 'buff', target: 'self', defJump: 3, mul: { speed: [1.25, 1.35, 1.45] }, spRestore: [40, 60, 80],
      dur: [6, 7, 8], cd: [18, 16, 14], mp: [35, 40, 45], desc: '騰雲駕霧身法：立即補充磁力並大幅加速，防守姿態下追加多次連續大跳躍' },
  ...
  t07: {
    ...
    skill: { name: '幽冥翔影：擬態無聲遁形', fx: 'stealth', spRestore: [40, 60, 80], mul: { speed: [1.2, 1.3, 1.4] },
      dur: [5, 6, 7], cd: [20, 18, 16], mp: [35, 40, 45], desc: '光學擬態完全隱形：進入無聲遁形狀態並補充磁力，大幅提升移動速度' },
  ...
  t08: {
    ...
    skill: { name: '破滅神曲：斷音防護結界', fx: 'emp', r: 16, dur: [3.5, 4.0, 4.5], spRestore: [40, 60, 80], shieldDefBoost: [0.6, 0.5, 0.4],
      cd: [20, 18, 16], mp: [35, 40, 45], desc: '天籟斷音結界：強烈干擾周圍敵機武器使其離線，並立即補充磁力與提高護盾減傷' },
  ...
  t09: {
    ...
    skill: { name: '悲歌壁壘：攔截雷幕', fx: 'buff', target: 'self', intercept: true, r: 16, spRestore: [50, 75, 100], shieldDefBoost: [0.6, 0.5, 0.4],
      dur: [6, 7, 8], cd: [20, 18, 16], mp: [35, 40, 45], desc: '自律僚機攔截防線：粉碎近身來襲彈道，補充磁力並在防守時大幅提高護盾減傷' },
  ...
  t10: {
    ...
    skill: { name: '幾何神碑：法老立方石板', fx: 'cube', spRestore: [60, 90, 120], shieldExpand: true, shieldDefBoost: [0.55, 0.45, 0.35],
      dur: [6, 7, 8], cd: [22, 20, 18], mp: [40, 45, 50], desc: '法老立方力場結界：直接補充磁力，防守時大幅擴大護盾面積並強化傷害減免' },
  ...
  t11: {
    ...
    skill: { name: '百戰心訣：陣地堅守結界', fx: 'buff', target: 'self', shieldBash: true, shieldDefBoost: [0.55, 0.45, 0.35], spRestore: [50, 75, 100],
      dur: [6, 7, 8], cd: [20, 18, 16], mp: [35, 40, 45], desc: '老兵陣地堅守心訣：持盾衝撞擊退敵機，直接補充磁力並提高護盾傷害減免效果' },
  ...
  t12: {
    ...
    skill: { name: '量子命運：同調共振護盾', fx: 'buff', target: 'self', spRegenHit: true, spRestore: [50, 75, 100], shieldDefBoost: [0.6, 0.5, 0.4],
      dur: [6, 7, 8], cd: [20, 18, 16], mp: [35, 40, 45], desc: '量子同調力場：直接補充磁力，受擊仍可持續回充磁力，且在防守時提高護盾減傷' },
...
//       m05 jet↔wolf(趾行深屈・鬃刺肩尖)
  ...
  m01: {
    ...
    skill: { name: '狂暴之怒：受擊磁力狂湧', fx: 'buff', target: 'self', spRegenHit: true, spRestore: [40, 60, 80], shieldBash: true,
      mul: { speed: [1.2, 1.3, 1.4] }, dur: [6, 7, 8], cd: [18, 16, 14], mp: [30, 35, 40], desc: '狂戰士受擊激發超頻充能：立即恢復磁力，受擊仍可恢復磁力，且護盾衝撞能擊退敵機' },
  ...
  m02: {
    ...
    skill: { name: '金湯之固：泰坦永恆護盾', fx: 'buff', target: 'self', spRestore: [60, 90, 120], shieldExpand: true, shieldDefBoost: [0.55, 0.45, 0.35],
      dur: [6, 7, 8], cd: [20, 18, 16], mp: [35, 40, 45], desc: '展開泰坦永恆防衛護盾：直接補充磁力，防守時大幅擴大護盾面積並強化傷害減免' },
  ...
  m03: {
    ...
    skill: { name: '雪線之澤：靈泉急救陣', fx: 'heal', target: 'self', heal: [120, 160, 200], sp: true, spRestore: [60, 90, 120], cleanse: true,
      dur: 6, cd: [22, 20, 18], mp: [40, 45, 50], desc: '喚醒高山靈泉聖域：解除自身異常狀態，大幅修復機體生命與磁力護盾' },
  ...
  m04: {
    ...
    skill: { name: '荒原天幕：神鷹迷霧界', fx: 'fog', r: 24, dur: [6, 7, 8], spRestore: [40, 60, 80], mul: { speed: [1.2, 1.3, 1.4] },
      cd: [18, 16, 14], mp: [35, 40, 45], desc: '投擲高濃度迷霧遮蔽彈幕：遮蔽敵方視線，進入隱蔽狀態並加速與補充磁力' },
  ...
  m05: {
    ...
    skill: { name: '幽夜斷路：滅靈防護結界', fx: 'emp', r: 16, dur: [3.5, 4.0, 4.5], spRestore: [40, 60, 80], shieldDefBoost: [0.6, 0.5, 0.4],
      cd: [18, 16, 14], mp: [35, 40, 45], desc: '展開幽夜滅靈力場：癱瘓半徑內敵軍武器，立即補充磁力並大幅提高護盾減傷' },
  ...
  m06: {
    ...
    skill: { name: '嘉年華巡遊：護盾擴張巡航', fx: 'buff', target: 'self', shieldExpand: true, spRestore: [50, 75, 100],
      mul: { speed: [1.25, 1.35, 1.45] }, dur: [6, 7, 8], cd: [20, 18, 16], mp: [35, 40, 45], desc: '嘉年華花車護衛力場：直接補充磁力並加速巡航，防守時大幅擴大護盾保護面積' },
  ...
  m07: {
    ...
    skill: { name: '極化偏轉：鏡面防衛矩陣', fx: 'reflect', dur: [4.0, 4.5, 5.0], spRestore: [40, 60, 80], shieldDefBoost: [0.55, 0.45, 0.35],
      cd: [20, 18, 16], mp: [35, 40, 45], desc: '啟動幾何鏡面偏轉矩陣：彈開直線來襲直擊光束與穿甲彈，補充磁力並強化護盾減免' },
  ...
  m08: {
    ...
    skill: { name: '相位穿梭：虛空避難', fx: 'phaseshift', dur: [2.5, 3.0, 3.5], spRestore: [40, 60, 80], mul: { speed: [1.3, 1.4, 1.5] },
      cd: [20, 18, 16], mp: [35, 40, 45], desc: '遁入超維虛空避難：短暫免疫一切傷害與負面狀態，立即補充磁力並高速脫離戰場' },
...
export const SEASON_WEATHER_WEIGHTS = {
  spring: {
    clear: 32, cloudy: 26, heavy_rain: 22, storm: 5, fog: 8, windy: 5, snow: 2, sandstorm: 0,
  },
  summer: {
    clear: 38, storm: 22, heavy_rain: 20, cloudy: 12, windy: 6, sandstorm: 1, fog: 0, snow: 1.0,
  },
  autumn: {
    clear: 40, cloudy: 20, heavy_rain: 15, windy: 12, fog: 7, sandstorm: 3, storm: 2, snow: 1.0,
  },
  winter: {
    snow: 42, cloudy: 24, clear: 18, windy: 8, fog: 5, heavy_rain: 3, storm: 0, sandstorm: 0,
  },
};

// 四季 × 四時段 (16種時空狀態) 之 7 維氣候均值傾向目標 (μ)
// 體現清晨輻射冷卻/逆溫晨霧、日間熱力升溫散霧與午後強熱對流、黃昏/夜間大氣穩定特徵
export const SEASON_TIME_BIAS = {
  spring: {
    dawn:  { clouds: 45, fog: 82, wind: 12, rain: 0,  sand: 0,  snow: 0,  thunder: 0 },
    day:   { clouds: 35, fog: 8,  wind: 28, rain: 15, sand: 0,  snow: 0,  thunder: 0 },
    dusk:  { clouds: 40, fog: 18, wind: 32, rain: 10, sand: 0,  snow: 0,  thunder: 0 },
    night: { clouds: 35, fog: 35, wind: 15, rain: 5,  sand: 0,  snow: 0,  thunder: 0 },
  },
  summer: {
    dawn:  { clouds: 25, fog: 8,  wind: 12, rain: 0,  sand: 0,  snow: 0,  thunder: 0 },
    day:   { clouds: 85, fog: 5,  wind: 58, rain: 88, sand: 0,  snow: 0,  thunder: 88 },
    dusk:  { clouds: 55, fog: 15, wind: 45, rain: 20, sand: 0,  snow: 0,  thunder: 15 },
    night: { clouds: 25, fog: 12, wind: 20, rain: 0,  sand: 0,  snow: 0,  thunder: 0 },
  },
  autumn: {
    dawn:  { clouds: 28, fog: 78, wind: 20, rain: 0,  sand: 0,  snow: 0,  thunder: 0 },
    day:   { clouds: 25, fog: 6,  wind: 68, rain: 0,  sand: 15, snow: 0,  thunder: 0 },
    dusk:  { clouds: 35, fog: 16, wind: 55, rain: 0,  sand: 10, snow: 0,  thunder: 0 },
    night: { clouds: 20, fog: 30, wind: 35, rain: 0,  sand: 0,  snow: 0,  thunder: 0 },
  },
  winter: {
    dawn:  { clouds: 68, fog: 76, wind: 32, rain: 0,  sand: 0,  snow: 82, thunder: 0 },
    day:   { clouds: 72, fog: 20, wind: 52, rain: 0,  sand: 0,  snow: 88, thunder: 0 },
    dusk:  { clouds: 82, fog: 35, wind: 65, rain: 0,  sand: 0,  snow: 92, thunder: 0 },
    night: { clouds: 78, fog: 42, wind: 50, rain: 0,  sand: 0,  snow: 95, thunder: 0 },
  },
};
...
export const WEATHER_DYNAMICS = {
  ...
  fog:        { windAmp: 0.5,  windFreq: 0.7,  waveAmp: 0.5,  waveSpeed: 0.6 },
};
 ...
 * 同時引入極值停留反彈動態: 當某一維度處於高數值區間 (>55%) 或低數值區間 (<45%) 過久時，
 * 會依停留時間累積反向恢復力，平滑提高向另一側擺盪的動態機率。
 *
 * @returns {{
...
export function weatherVectorAt(season = 'summer', startTime = 'day', startWeather = 'clear', elapsedS = 0, seed = 0, latDeg = 25.0) {
  ...
  for (let idx = 0; idx < WEATHER_ATTRS.length; idx++) {
    ...
    // 基礎合成值
    const vBase = initialVal + (targetMean - initialVal) * relax + noise * Math.min(1.0, relax + 0.15);

    // 高數值區間 / 低數值區間停留過久之反向機率與恢復力動態 (Dwell-time polarity counter-force)
    // 週期約 150s ~ 320s 之多尺度動態擺盪波，在某傾向持續過久時提供平滑的反向拉力
    const dwellPeriod = 160.0 + idx * 26.0;
    const pDwell1 = (seedNum * 0.191 + idx * 5.37) % 6.28318;
    const pDwell2 = (seedNum * 0.383 + idx * 11.23) % 6.28318;
    const dwellWave = Math.sin(elapsedS * (6.28318 / dwellPeriod) + pDwell1) * 0.70
                    + Math.sin(elapsedS * (6.28318 / (dwellPeriod * 0.47)) + pDwell2) * 0.30;

    let rebound = 0;
    if (vBase > 55) {
      // 處於高數值區間 (>55%)，停留越久越增加往低數值轉移的機率與反向拉力
      const excess = (vBase - 50) / 50; // [0.1, 1.0]
      rebound = -Math.pow(excess, 1.5) * 16.0 * Math.max(0, dwellWave + 0.35);
    } else if (vBase < 45) {
      // 處於低數值區間 (<45%)，停留越久越增加往高數值轉移的機率與正向推力
      const deficit = (50 - vBase) / 50; // [0.1, 1.0]
      rebound = Math.pow(deficit, 1.5) * 16.0 * Math.max(0, -dwellWave + 0.35);
    }

    let v = vBase + rebound * Math.min(1.0, relax + 0.25);
...
export function resolveWeatherDynamics(weatherVec, prevDyn = null, dt = 0) {
  ...
  const fogFar = Math.max(0.25, 1.90 - effectiveFog * 1.60);

  // 9. 照度係數 (受雲量、有效霧量、雨、沙綜合影響)
  const light = Math.max(0.18, 1.0 - (clouds / 100) * 0.50 - (effectiveFog * 0.20) - (effectiveRain * 0.15) - (effectiveSand * 0.15));
  ...
  else if (effectiveFog > 0.4) dominant = 'fog';
  ...
  for (let d = startDay; d <= endDay; d++) {
    for (const a of anchors) {
      ...
      if (anchorH > h0 && anchorH <= endHour) {
        ...
        // 當前天氣持續停留越久 (stepIdx 累積)，提升往另一種天氣切換的機率
        const keepProb = Math.max(0.20, 0.50 - (stepIdx - 1) * 0.10);
        if (r1 >= keepProb) {
