...
// ---- 物件出現 / 消失:溶入溶出(2026-08-16~17;序 8 ④-2)----
// `discard` 抖動而**不是** alpha 淡入:賽璐璐件淡入會連自己的輪廓一起淡掉(而輪廓正是這個
// 畫風的全部)。消失端的 mesh MUST 先從 `this.ents` 與所有戰鬥消費端摘掉,只把純渲染殘影
// 放進 `_dissolveGhosts`;否則延後移除 = 客戶端把準星解到一個伺服器已經沒有的目標上。
// 快照缺席可能只是迷霧過濾,只有同份快照的權威 `die` 事件可以開溶出;否則殘影會洩漏視野外資訊。
export const DISSOLVE = {
  IN_S: 0.55,       // 溶入時長(秒)
  OUT_S: 0.45,      // 權威死亡溶出時長(秒;迷霧消失不適用)
...
/** 溶出進度:1 = 完全實體、0 = 全部 discard。反轉同一條 smoothstep,不開第二條形狀 */
export const dissolveOutAt = (t) => 1 - (DISSOLVE.OUT_S > 0 ? _sstep(t / DISSOLVE.OUT_S) : 1);
...
export const skillPower = (ch, lvl = 1) => {
  ...
  if (u.fx === 'dash') {
    p = t(u.imp || 30) * 2;
  } else if (u.fx === 'stealth') {
    p = t(u.dur || 5) * 25;
  } else if (u.fx === 'vision') {
    p = t(u.vision || 7) * 20;
  } else if (u.fx === 'intercept') {
    p = t(u.r || 200) * 0.7;
  } else if (u.fx === 'heal') {
    const h = t(u.heal || 0);
    p = (u.target === 'team' ? h * 1.8 : h * 0.6);
  } else if (u.fx === 'emp') {
    const dur = t(u.dur || 3);
    const r = t(u.r || 130);
    p = dur * (r / 10) * 7;
  } else if (u.fx === 'summon') {
    const cnt = t(u.count || 3);
    p = cnt * 100;
  } else if (u.fx === 'buff') {
    const dur = t(u.dur || 6);
    let mag = 0;
    if (u.mul?.dmg) mag += (t(u.mul.dmg) - 1) * 300;
    if (u.mul?.dmgTaken) mag += (1 - t(u.mul.dmgTaken)) * 300;
    if (u.mul?.reload) mag += (1 - t(u.mul.reload)) * 200;
    if (u.add?.f) mag += (t(u.add.f) - 1) * 150;
...
export const skillPowerBand = () => {
  ...
  for (const id of Object.keys(CHARACTERS)) {
    for (let lvl = 1; lvl <= 3; lvl++) {
      const p = skillPower(id, lvl);
      if (p < lo) lo = p;
      if (p > hi) hi = p;
    }
  }
  _skillPowerBand = { lo, hi };
...
export const CHARACTERS = {
  ...
  s01: {
    ...
    light: { name: '「弱起拍」六足分鍵機槍', rw: '六足分鍵點射・FN Minimi 5.56 改・初速 915m/s', type: 'gun', mv: 915,
      dmg: [11, 14, 17], rate: 10, mag: [40, 50, 60], reload: 2.0, range: 190, crit: 0.06,
      vs: { flesh: 1.2, armor: 0.6, air: 1.3, building: 0.5 } },
    heavy: { name: '「定音符」雷導火箭巢', rw: '「定音符」雷射制導火箭巢・Hydra 70/APKWS・初速 700m/s', type: 'launcher', mv: 700, guide: 1,
      dmg: [49, 75, 109], r: [12, 14, 16], mag: 3, reload: 12, range: 300, pen: 6,
      vs: { flesh: 1.1, armor: 1.4, air: 0.5, building: 1.2 } },
    skill: { name: '巴哈無伴奏', fx: 'buff', target: 'team', r: 180, mul: { dmg: [1.15, 1.23, 1.3] },
      add: { fx: 'vamp', f: [0.1, 0.13, 0.16] },
      dur: [6, 8, 10], cd: 20, mp: [35, 40, 45], desc: '起手輕哼無伴奏樂章：協調全隊節奏，友軍火力提升，蜂群同步回收戰果（吸血）' },
    ult: { name: '總譜：終章大合奏', fx: 'summon', unit: 'heli', count: [2, 3, 4],
      cd: [80, 70, 60], mp: [80, 90, 100], desc: '揮落終曲總譜：呼叫重裝攻擊直升機編隊沿最近兵線壓上交響破陣' },
  },
  s02: {
    ...
    light: { name: '「老拖拉機」12.7mm 重機槍', rw: '車庫加裝重機槍・DShK 12.7mm・初速 850m/s', type: 'gun', mv: 850,
    ...
    heavy: { name: '「巴巴雅嘎」7V 溫壓破壁彈', rw: '土造「巴巴雅嘎」溫壓破壁彈・TBG-7V 彈頭・初速 120m/s', type: 'launcher', mv: 120,
    ...
    skill: { name: '哈爾科夫補焊術', fx: 'heal', target: 'self', heal: [180, 260, 340],
      cd: [24, 21, 18], mp: [35, 40, 45], desc: '焊槍出手電弧飛濺：哈爾科夫老技工手法，立即大幅搶修自身裝甲' },
    ult: { name: '「鐵匠鋪」爐火重鑄', fx: 'heal', target: 'team', r: 200, heal: [220, 300, 380], sp: true,
      cd: [80, 70, 60], mp: [85, 95, 105], desc: '「鐵匠鋪」爐火全開重鑄：全隊裝甲回火大修，護盾力場同步充滿' },
  },
  s03: {
    ...
    light: { name: '「半羽」相位波束槍', rw: '耳廓相控陣高頻波束・光速直擊', type: 'beam',
    ...
    heavy: { name: '額隆 HPM 燒盾微波門', rw: '額隆 HPM 燒盾微波門・高功率定向微波・光速直擊', type: 'beam',
      dmg: [21, 31, 44], mag: 5, reload: 8, range: 320, emp: [0.8, 1.0, 1.2],
      vsSp: 1.7, vsHp: 0.7,
      vs: { flesh: 0.7, armor: 0.8, air: 2.0, building: 0.4 } },
    skill: { name: '「KPI」定向雜訊阻斷', fx: 'emp', r: 120, dur: [2.5, 3, 3.5], range: 260,
      cd: [18, 16, 14], mp: [40, 45, 50], desc: '雙耳同相位對準：指定區域敵軍武器與感測器徹底離線（建築免疫）' },
    ult: { name: '利維坦長波鯨歌', fx: 'emp', r: 260, dur: [4, 5, 6],
      cd: [70, 62, 54], mp: [90, 100, 110], desc: '全身十六道隔艙稜線共形起振：大範圍低頻電子鯨歌，全頻段通訊壓制' },
  },
  s04: {
    ...
    light: { name: '「普通科」九式貼臉霰彈莢', rw: '陸自普通科九式霰彈莢・Benelli M4 霰彈・初速 400m/s', type: 'gun', mv: 400, fan: true, arc: [16, 14, 12],
      dmg: [34, 42, 52], rate: 2.2, mag: [7, 8, 10], reload: 2.6, range: 170, crit: 0.10, critX: 1.5,
      vs: { flesh: 1.6, armor: 0.5, air: 1.2, building: 0.4 } },
    heavy: { name: '「零式」熱浪電漿噴湧口', rw: '「零式」熱浪電漿噴湧口・磁化電漿短程扇形噴焰', type: 'plasma', arc: [13, 15, 17],
      dmg: [46, 75, 117], mag: 3, reload: 7, range: 264, pen: 8,
      vs: { flesh: 1.5, armor: 1.0, air: 0.5, building: 1.2 } },
    skill: { name: '教官的鐵鍬衝鋒', fx: 'dash', imp: [28, 34, 40],
      cd: [12, 10, 8], mp: [25, 30, 35], desc: '教官親授鐵鍬突刺衝鋒：沿視線方向爆發加速，拉近至白刃戰距離' },
    ult: { name: '「遙控器沒電之時」', fx: 'buff', target: 'self', mul: { dmg: [1.35, 1.45, 1.55], dmgTaken: [0.85, 0.8, 0.75] },
      add: { fx: 'haste', f: [1.25, 1.3, 1.35] },
      dur: [8, 10, 12], cd: [70, 60, 50], mp: [75, 85, 95], desc: '「遙控器沒電之時，你就是步兵」：進入白刃戰反射狀態，承傷降低並高速突臉破陣' },
  },
  s05: {
    ...
    light: { name: '「天梯榜首」超導轉輪機關槍', rw: '天梯第一 APM 超導轉輪機槍・線性感應馬達・初速 1400m/s', type: 'rail', mv: 1400,
      dmg: [9, 11, 14], rate: [14, 16, 18], mag: [70, 90, 110], reload: 2.8, range: 180, crit: 0.05,
      vs: { flesh: 1.2, armor: 0.6, air: 1.4, building: 0.4 } },
    heavy: { name: '「粉紅刺客」巡飛彈掛架', rw: '粉紅塗裝巡飛彈掛架・Lancet 縮裝・巡飛 90m/s', type: 'missile', mv: 90,
      dmg: [44, 63, 88], r: [13, 15, 17], mag: 4, reload: 11, range: 320, pen: 12,
      vs: { flesh: 1.0, armor: 1.6, air: 0.6, building: 1.1 } },
    skill: { name: 'APM 極限超頻', fx: 'buff', target: 'self', mul: { dmg: [1.05, 1.1, 1.15], reload: [0.65, 0.6, 0.55] },
      add: { fx: 'dodge' },
      dur: [6, 7, 8], cd: [18, 16, 14], mp: [30, 35, 40], desc: 'APM 極限超頻：換彈大幅加速，神級競速走位完美迴避敵方直射彈道' },
    ult: { name: '「直播高光」飽和自爆蜂群', fx: 'strike', count: [6, 8, 10], dmg: [60, 77, 94], r: 10, scatter: 30,
      add: { fx: 'confuse', dur: [1.5, 2, 2.5] },
      range: 320, pen: 8, cd: [70, 62, 54], mp: [85, 95, 105], vs: { armor: 1.3, building: 1.1 },
      desc: '直播高光時刻：呼叫粉紅 FPV 蜂群對指定區域飽和俯衝自爆，纏擾致盲（混亂）' },
  },
  s06: {
    ...
    light: { name: '「冷爵士」磁軌點射步槍', rw: '冷爵士點射步槍・實驗性磁軌砲・初速 1600m/s', type: 'rail', mv: 1600,
    ...
    heavy: { name: '「救贖者」近程防空響尾蛇', rw: '「救贖者」近程防空響尾蛇・AIM-9X 縮裝・初速 1000m/s', type: 'missile', mv: 1000,
      dmg: [45, 68, 102], r: [8, 9, 10], mag: 4, reload: 11, range: 340, pen: 6,
      vs: { flesh: 0.6, armor: 0.6, air: 2.5, building: 0.3 } },
    skill: { name: '「只擋不殺」攔截天網', fx: 'intercept', r: [150, 190, 230],
      cd: [16, 14, 12], mp: [30, 35, 40], desc: '「這雙手只用來擋子彈」：展開攔截天網，擊落半徑內所有來襲敵方飛彈' },
    ult: { name: '誓約：空白布章', fx: 'buff', target: 'team', r: 220, mul: { dmgTaken: [0.6, 0.5, 0.4] },
      dur: [6, 7, 8], cd: [75, 65, 55], mp: [85, 95, 105], desc: '守護誓約・空白布章：展開同盟護航陣列，半徑內全體友軍承受傷害大幅減免' },
  },
  s07: {
    ...
    light: { name: '「Q.E.D.」25mm 程控空爆砲', rw: '「Q.E.D.」程控空爆砲・XM25 派生 25mm・初速 760m/s', type: 'gun', mv: 760,
      dmg: [20, 25, 31], rate: 6, mag: [24, 30, 36], reload: 2.3, range: 210, crit: 0.06,
      vs: { flesh: 1.3, armor: 0.7, air: 1.6, building: 0.5 } },
    heavy: { name: '非歐幾何防空電漿網', rw: '非歐幾何電漿防空矩陣・磁化電漿扇形散布', type: 'plasma', arc: [20, 23, 26],
      dmg: [50, 77, 113], mag: 3, reload: 7, range: 264, pen: 4,
      vs: { flesh: 0.9, armor: 0.6, air: 2.0, building: 0.4 } },
    skill: { name: '沙哈最佳分配解', fx: 'intercept', r: [170, 210, 250],
      cd: [15, 13, 11], mp: [30, 35, 40], desc: '「攔截永遠該比打擊便宜」：沙哈最佳分配解，精準清空半徑內來襲飛彈' },
    ult: { name: '「證畢」破片反切矩陣', fx: 'strike', count: [5, 7, 9], dmg: [68, 85, 106], r: 11, scatter: 35,
      add: { fx: 'slow', f: 0.6, dur: [2, 2.5, 3] },
      range: 340, pen: 6, cd: [72, 64, 56], mp: [85, 95, 105], vs: { air: 1.5, armor: 1.1 },
      desc: '「一道證明完畢」：破片攔截網反向齊射，撕裂敵機氣動操縱面（緩速）' },
  },
  s08: {
    ...
    light: { name: '修道院守望 7.62 機槍', rw: '修道院守望機槍・PKM 7.62mm・初速 825m/s', type: 'gun', mv: 825,
      dmg: [14, 18, 22], rate: 7, mag: [36, 44, 52], reload: 2.1, range: 190, crit: 0.06,
      vs: { flesh: 1.3, armor: 0.7, air: 1.1, building: 0.5 } },
    heavy: { name: '「聖燭」電磁獵魔狙擊管', rw: '「聖燭」電磁獵魔狙擊管・同步軌道電磁狙擊・初速 2200m/s', type: 'rail', mv: 2200,
      dmg: [66, 99, 149], mag: 2, reload: 8, range: 360, crit: 0.25, critX: 2.0, pen: [16, 20, 24],
      vs: { flesh: 1.4, armor: 0.8, air: 1.4, building: 0.4 } },
    skill: { name: '天使血袋空投箱', fx: 'heal', target: 'team', r: 140, heal: [150, 210, 270],
      cd: [20, 18, 16], mp: [40, 45, 50], desc: '空中血庫開倉：投送天使血袋箱，半徑內友軍機體裝甲迅速修復' },
    ult: { name: '克拉科夫的晨鐘', fx: 'heal', target: 'team', r: 240, heal: [280, 380, 480], sp: true,
      cd: [85, 75, 65], mp: [90, 100, 110], desc: '克拉科夫的晨鐘響徹：大範圍野戰天使醫療，裝甲大量回復、護盾同步充滿' },
  },
  s09: {
    ...
    light: { name: '「帕迪客製」12號雙管防空霰彈', rw: '帕迪客製雙管獵槍・Purdey 12 鉛徑防空改・初速 420m/s', type: 'gun', mv: 420, fan: true, arc: [18, 16, 14],
      dmg: [34, 43, 53], rate: 2.6, mag: [8, 10, 12], reload: 2.4, range: 170, crit: 0.10, critX: 1.5,
      vs: { flesh: 1.3, armor: 0.4, air: 2.0, building: 0.3 } },
    heavy: { name: '「星光」星芒追獵飛彈', rw: '「星光」星芒追獵飛彈・Starstreak 縮裝雷導・初速 300m/s', type: 'launcher', mv: 300, guide: 1,
    ...
    skill: { name: '「好球！」紳士開獵', fx: 'buff', target: 'self', mul: { dmg: [1.25, 1.35, 1.45] },
      add: { fx: 'haste', f: [1.2, 1.25, 1.3] },
      dur: 6, cd: [18, 16, 14], mp: [30, 35, 40], desc: '紳士開獵宣告：「好球！」火力全面增幅，躍步底盤健步追獵' },
    ult: { name: '新南威爾斯圍獵網', fx: 'strike', count: [8, 10, 12], dmg: [51, 64, 77], r: 9, scatter: 40,
      add: { fx: 'pull', imp: [16, 20, 24] },
      range: 300, cd: [70, 62, 54], mp: [80, 90, 100], vs: { air: 2.0, flesh: 1.2 },
      desc: '新南威爾斯圍獵網：封鎖指定空域，獵網彈幕將獵物向彈著中心收攏（拉近）' },
  },
  s10: {
    ...
    light: { name: '「白噪音」擾頻脈衝雷射', rw: '白噪音擾頻雷射槍・低功率脈衝光束・光速直擊', type: 'beam',
      dmg: [16, 20, 24], rate: 9, mag: [30, 36, 42], reload: 1.8, range: 220, crit: 0.08,
      vs: { flesh: 1.4, armor: 0.5, air: 1.1, building: 0.4 } },
    heavy: { name: '「DoS」電磁穿顱長矛', rw: '「DoS」電磁穿顱長矛・EMP 穿甲狙擊彈・初速 900m/s', type: 'gun', mv: 900,
      dmg: [42, 64, 94], mag: 3, reload: 9, range: 340, emp: [1.5, 2, 2.5],
      vs: { flesh: 0.8, armor: 1.0, air: 1.8, building: 0.5 } },
    skill: { name: '塔林民防頻譜解密', fx: 'vision', vision: [6, 8, 10],
      cd: [26, 23, 20], mp: [35, 40, 45], desc: '塔林民防頻譜解密：破解敵方戰術遙測數據，全隊獲取限時無霧全局視野' },
    ult: { name: '「聽起來就很假」全域斷網', fx: 'emp', r: 300, dur: [4, 5, 6],
      cd: [75, 65, 55], mp: [90, 100, 110], desc: '「聽起來就很假」全域斷網：發動超大範圍低頻鏈路壓制，敵方感測徹底中斷' },
  },
  s11: {
    ...
    light: { name: '「鐘錶軸心」聚焦雷射標槍', rw: '精密鐘錶聚焦雷射槍・低散射固態雷射・光速直擊', type: 'beam',
    ...
    heavy: { name: '「擒縱器」膝關節碎裂磁軌砲', rw: '「擒縱器」關節碎裂磁軌砲・實驗性 EM 穿甲・初速 2000m/s', type: 'rail', mv: 2000,
      dmg: [48, 70, 99], mag: 2, reload: 8, range: 380, crit: 0.15, critX: 2.0, pen: [25, 30, 35],
      vs: { flesh: 0.8, armor: 2.2, air: 1.2, building: 0.7 } },
    skill: { name: '「我造了那個膝蓋」', fx: 'buff', target: 'self', mul: { dmg: [1.3, 1.4, 1.5] },
      add: { fx: 'mark', dur: [4, 5, 6] },
      dur: [5, 6, 7], cd: [18, 16, 14], mp: [35, 40, 45], desc: '「我造了那個膝蓋，我最知道怎麼打斷它」：精準解析弱點，下一擊必中必爆' },
    ult: { name: '格拉蘇蒂精密再校準', fx: 'heal', target: 'self', heal: [400, 550, 700], sp: true,
      cd: [80, 70, 60], mp: [80, 90, 100], desc: '格拉蘇蒂鐘錶匠之手：自身裝甲結構精密再校準大修，護盾力場同步充滿' },
  },
  s12: {
    ...
    light: { name: '「歸鄉路」74U 短卡賓', rw: '「歸鄉路」偵察短卡賓・AKS-74U・初速 735m/s', type: 'gun', mv: 735,
      dmg: [16, 20, 25], rate: 8, mag: [30, 36, 42], reload: 1.9, range: 180, crit: 0.08,
      vs: { flesh: 1.3, armor: 0.6, air: 1.1, building: 0.4 } },
    heavy: { name: '星象儀光電測距砲', rw: '星象儀高能測距雷射砲・光電一體定向束・光速直擊', type: 'beam',
    ...
    skill: { name: '巴赫奇薩賴薰衣草斗篷', fx: 'stealth', dur: [4, 5, 6],
    ...
    ult: { name: '「星星永遠不會落幕」', fx: 'rally', target: 'team', regen: [2.5, 3, 3.5],
      cleanse: true, revive: [0.5, 0.5, 0.5], dur: [8, 10, 12],
      cd: [70, 62, 54], mp: [80, 90, 100], desc: '「衛星會被打下來，星星不會」：全隊回復加速、解除並免疫異常，倒下的戰友原地復甦站起' },
  },
  ...
  t01: {
    ...
    light: { name: '「科爾德」同軸守護重機槍', rw: '科爾德守護重機槍・Kord 12.7mm 同軸・初速 860m/s', type: 'gun', mv: 860,
    ...
    heavy: { name: '「2A65」152mm 破障加農砲', rw: '「2A65」152mm 破障加農砲・加農榴彈縮裝・初速 650m/s', type: 'launcher', mv: 650,
    ...
    skill: { name: '「鋼鐵不屈」全軍突擊令', fx: 'buff', target: 'team', r: 200, mul: { dmg: [1.15, 1.25, 1.35] },
      add: { fx: 'haste', f: [1.2, 1.25, 1.3] },
      dur: [6, 8, 10], cd: [22, 20, 18], mp: [35, 40, 45], desc: '「我不再送沒有裝甲的孩子上戰場！」：全軍突擊令，友軍火力與推進速度齊升' },
    ult: { name: '烏拉爾十二號雪崩齊射', fx: 'strike', count: [6, 8, 10], dmg: [77, 98, 119], r: 12, scatter: 40,
      add: { fx: 'stun', dur: [0.8, 1, 1.2] },
      range: 340, pen: 10, cd: [80, 70, 60], mp: [90, 100, 110], vs: { building: 1.4, armor: 1.2 },
      desc: '烏拉爾十二號雪崩齊射：全營重型砲兵行進間齊射覆蓋，砲火撼地震撼（麻痺）' },
  },
  t02: {
    ...
    light: { name: '「加拉泰亞」高斯脈衝衝鋒槍', rw: '「加拉泰亞」高斯衝鋒槍・實驗性 EM 線圈・初速 1100m/s', type: 'rail', mv: 1100,
      dmg: [14, 18, 21], rate: 8, mag: [32, 40, 48], reload: 1.9, range: 200, crit: 0.08,
      vs: { flesh: 1.2, armor: 0.9, air: 1.1, building: 0.5 } },
    heavy: { name: '「第七實驗體」高能磁軌穿甲矛', rw: '「第七實驗體」穿甲磁軌矛・EM 加速穿甲彈・初速 1500m/s', type: 'rail', mv: 1500,
    ...
    skill: { name: '神經突觸瞬影閃', fx: 'dash', imp: [26, 32, 38],
    ...
    ult: { name: '臨界突破：同步率 100%', fx: 'buff', target: 'self',
      mul: { dmgTaken: [0.75, 0.7, 0.65] },
      add: { fx: 'overdrive', evade: [0.25, 0.32, 0.4] }, brk: true,
      dur: [8, 10, 12], cd: [80, 70, 60], mp: [85, 95, 105], desc: '臨界突破：同步率 100%——再無介面延遲：彈藥全滿、期間免裝填且閃避大增（挨一發即結束）' },
  },
  t03: {
    ...
    light: { name: '「主廚特調」12號連發碎肉霰彈', rw: '「主廚特調」連發霰彈槍・Saiga-12 彈鼓・初速 400m/s', type: 'gun', mv: 400, fan: true, arc: [17, 15, 13],
      dmg: [36, 45, 56], rate: 2.4, mag: [8, 10, 12], reload: 2.6, range: 170, crit: 0.10, critX: 1.5,
      vs: { flesh: 1.6, armor: 0.6, air: 0.9, building: 0.5 } },
    heavy: { name: '「爐膛出火」重型電漿烈焰', rw: '「爐膛出火」重型電漿烈焰・磁化電漿重型扇形噴焰', type: 'plasma', arc: [15, 17, 19],
    ...
    skill: { name: '主廚的防彈鑄鐵鍋', fx: 'buff', target: 'self', mul: { dmgTaken: [0.55, 0.5, 0.45] },
      add: { fx: 'haste', f: [1.35, 1.45, 1.55] },
      dur: [5, 6, 7], cd: [16, 14, 12], mp: [30, 35, 40], desc: '左臂鑄鐵鍋盾一橫：頂著敵方彈幕加速衝撞破陣（承傷大幅減免 + 衝鋒）' },
    ult: { name: '「今晚加菜！」掀鍋熱浪', fx: 'strike', count: [2, 3, 4], dmg: [88, 110, 138], r: 14, scatter: 18,
      add: { fx: 'pull', imp: [24, 30, 36] },
      range: 160, pen: 10, cd: [75, 65, 55], mp: [80, 90, 100], vs: { flesh: 1.5, armor: 1.1 },
      desc: '「今晚加菜！掀鍋了！」：滾燙高溫熱浪炸開，將周圍敵軍全數捲進甜蜜點（拉近）' },
  },
  t04: {
    ...
    light: { name: '「微聲微光」9×39 特戰步槍', rw: '「微聲微光」特戰步槍・VSS Vintorez 9×39mm・初速 295m/s', type: 'gun', mv: 295,
      dmg: [21, 27, 33], rate: 3.2, mag: [20, 24, 28], reload: 2.0, range: 210, crit: 0.15, critX: 1.8,
      vs: { flesh: 1.4, armor: 0.8, air: 1.0, building: 0.5 } },
    heavy: { name: '「格魯烏」14.5mm 穿甲反器材砲', rw: '「格魯烏」反器材重砲・KPV 14.5mm 穿甲彈・初速 1000m/s', type: 'gun', mv: 1000,
    ...
    skill: { name: '暗影「灰雁」迷彩', fx: 'stealth', dur: [4, 5, 6],
      cd: [20, 18, 16], mp: [35, 40, 45], desc: '暗影「灰雁」迷彩：啟動光學與熱成像匿蹤，從所有感測器上消失（開火即現形）' },
    ult: { name: '名單翻頁：「下一個，蜂后」', fx: 'buff', target: 'self', mul: { dmg: [1.25, 1.35, 1.45] }, vision: [8, 10, 12],
      add: { fx: 'mark', dur: [5, 6, 7] },
      dur: [8, 10, 12], cd: [75, 65, 55], mp: [85, 95, 105], desc: '名單翻頁：「下一個，蜂后」——格魯烏獵殺鎖定：全圖視野，下一發直擊必中必爆' },
  },
  t05: {
    ...
    light: { name: '「鶴羽一號」實驗型線圈步槍', rw: '「鶴羽一號」線圈步槍・實驗型線圈砲・初速 1500m/s', type: 'rail', mv: 1500,
      dmg: [16, 20, 25], rate: 7, mag: [36, 44, 52], reload: 2.1, range: 200, crit: 0.06,
      vs: { flesh: 1.3, armor: 0.8, air: 1.0, building: 0.5 } },
    heavy: { name: '「長空」仿生聚能雷射長矛', rw: '「長空」仿生聚能雷射長矛・關節整合雷射・光速直擊', type: 'beam',
      dmg: [37, 56, 83], mag: 5, reload: 8, range: 320, pen: [18, 22, 26],
      vs: { flesh: 0.8, armor: 1.7, air: 0.6, building: 0.6 } },
    skill: { name: '「掉漆的才是我的」應力自修', fx: 'heal', target: 'self', heal: [200, 280, 360],
      cd: [22, 19, 16], mp: [35, 40, 45], desc: '「掉漆的才是我的」：仿生關節應力自我檢修，迅速回復機體裝甲' },
    ult: { name: '瀋陽重工緊急生產線', fx: 'summon', unit: 'tank', count: [1, 2, 3],
      cd: [90, 80, 70], mp: [90, 100, 110], desc: '瀋陽重工緊急生產線：總設計師調度後勤，重型主戰坦克沿最近兵線出廠壓進' },
  },
  t06: {
    ...
    light: { name: '「山城特供」191 突擊步槍', rw: '「山城特供」突擊步槍・QBZ-191 5.8mm・初速 930m/s', type: 'gun', mv: 930,
      dmg: [15, 18, 23], rate: 9, mag: [34, 42, 50], reload: 1.8, range: 190, crit: 0.08,
      vs: { flesh: 1.3, armor: 0.7, air: 1.0, building: 0.5 } },
    heavy: { name: '「如意金箍」高溫熔核砲', rw: '「如意金箍」熔核砲・磁化電漿聚爆・多節長尾前捲直擊', type: 'plasma', arc: [10, 12, 14],
      dmg: [60, 93, 144], mag: 3, reload: 7, range: 264, pen: 10,
      vs: { flesh: 1.1, armor: 1.4, air: 0.3, building: 0.8 } },
    skill: { name: '九宮格麻辣飄移', fx: 'dash', imp: [28, 34, 40],
      cd: [11, 9, 7], mp: [25, 30, 35], desc: '九宮格麻辣飄移：省冠軍模擬器神級走位，機體像長在身上般靈活位移' },
    ult: { name: '「漫畫主角登場！」齊天大跳', fx: 'buff', target: 'self', mul: { dmg: [1.35, 1.45, 1.55], dmgTaken: [0.8, 0.75, 0.7] },
      add: { fx: 'leap', f: [2.0, 2.3, 2.6] },
      dur: [8, 10, 12], cd: [70, 60, 50], mp: [80, 90, 100], desc: '「漫畫主角登場！」：儲物櫃漫畫主角上場，齊天大跳躍跨越地形直擊' },
  },
  t07: {
    ...
    light: { name: '「無聲」88 式微聲短卡賓', rw: '「無聲」微聲短卡賓・88 式微聲縮裝・初速 720m/s', type: 'gun', mv: 720,
      dmg: [15, 19, 23], rate: 6, mag: [30, 36, 42], reload: 2.0, range: 190, crit: 0.10,
      vs: { flesh: 1.3, armor: 0.7, air: 1.0, building: 0.4 } },
    heavy: { name: '「白頭山鷹」14.5mm 栓動重狙', rw: '「白頭山鷹」反器材重狙・14.5mm 栓動重管・初速 1000m/s', type: 'gun', mv: 1000,
      dmg: [40, 57, 80], mag: 3, reload: 9, range: 400, crit: 0.25, critX: 2.2, pen: [18, 24, 30],
      vs: { flesh: 2.0, armor: 1.6, air: 1.2, building: 0.5 } },
    skill: { name: '「屏息」擬態膜翼滑翔', fx: 'stealth', dur: [5, 6, 7],
      cd: [20, 18, 16], mp: [35, 40, 45], desc: '「屏息」擬態膜翼滑翔：動作省到一毫米都不多，無聲無息隱形遁行（開火即現形）' },
    ult: { name: '「零點五度的微笑」一擊致命', fx: 'strike', count: 1, dmg: [340, 442, 553], r: 6,
      add: { fx: 'bleed', dps: [24, 30, 38], dur: 5, pen: 12 },
      range: 400, pen: 20, cd: [70, 62, 54], mp: [80, 90, 100], vs: { flesh: 1.5, armor: 1.2 },
      desc: '「零點五度的微笑」一擊致命：只需要一發，極限穿透創口持續失血（出血）' },
  },
  t08: {
    ...
    light: { name: '「宮商和聲」光電脈衝步槍', rw: '「宮商和聲」光電步槍・聲電複合雷射・光速直擊', type: 'beam',
    ...
    heavy: { name: '「龍吟」護盾諧振音波砲', rw: '「龍吟」諧振音波砲・定向聲電諧振・光速直擊', type: 'beam',
      dmg: [23, 33, 45], mag: 5, reload: 8, range: 320, emp: [1.0, 1.5, 2.0],
      vsSp: 1.55, vsHp: 0.78,
      vs: { flesh: 0.9, armor: 0.8, air: 1.8, building: 0.4 } },
    skill: { name: '撕碎通訊的搖籃曲', fx: 'emp', r: 140, dur: [2.5, 3, 3.5], range: 260,
      cd: [18, 16, 14], mp: [40, 45, 50], desc: '撕碎通訊的搖籃曲：精確音波共振干擾，指定區域敵軍武器全面離線' },
    ult: { name: '「平壤歌姬」絕對音感詠嘆調', fx: 'emp', r: 280, dur: [4, 5, 6],
      cd: [72, 64, 56], mp: [90, 100, 110], desc: '「平壤歌姬」絕對音感詠嘆調：全頻段諧振音波大肆宣洩，強行壓制全場通訊' },
  },
  t09: {
    ...
    light: { name: '「魯米之章」防衛機槍', rw: '「魯米之章」防衛機槍・MG3 7.62mm・初速 820m/s', type: 'gun', mv: 820,
      dmg: [14, 18, 22], rate: 8, mag: [40, 48, 56], reload: 2.2, range: 190, crit: 0.06,
      vs: { flesh: 1.3, armor: 0.7, air: 1.0, building: 0.5 } },
    heavy: { name: '「見證者」136 巡飛彈發射槽', rw: '「見證者」巡飛彈槽・Shahed-136 縮裝・巡飛 100m/s', type: 'missile', mv: 100,
      dmg: [38, 55, 77], r: [15, 17, 19], mag: 4, reload: 11, range: 360, pen: 10,
      vs: { flesh: 1.1, armor: 1.3, air: 0.3, building: 1.6 } },
    skill: { name: '寫給未歸者的哀悼詩', fx: 'summon', unit: 'rocketeer', count: [2, 3, 4],
      cd: [26, 23, 20], mp: [40, 45, 50], desc: '寫給未歸者的哀悼詩：「為敵我雙方各寫一行」，呼叫火箭兵支援壓陣' },
    ult: { name: '「無盡戰爭」巡飛彈暴風雨', fx: 'strike', count: [7, 9, 11], dmg: [72, 89, 111], r: 11, scatter: 45,
      add: { fx: 'confuse', dur: [1.5, 2, 2.5] },
      range: 360, pen: 8, cd: [80, 70, 60], mp: [90, 100, 110], vs: { building: 1.3, armor: 1.2 },
      desc: '「無盡戰爭」巡飛彈暴風雨：漫天廉價自爆無人機暴風雨式傾瀉，引發極度恐慌（混亂）' },
  },
  t10: {
    ...
    light: { name: '「伊斯法罕」30mm 截擊速射砲', rw: '「伊斯法罕」截擊速射砲・2A42 30mm 縮裝・初速 960m/s', type: 'gun', mv: 960,
      dmg: [18, 22, 27], rate: 5.5, mag: [28, 34, 40], reload: 2.3, range: 210, pen: 6,
      vs: { flesh: 1.1, armor: 1.0, air: 1.5, building: 0.5 } },
    heavy: { name: '「道爾」垂直冷發射防空飛彈', rw: '「道爾」垂直冷發射防空彈・9M330 縮裝・初速 800m/s', type: 'missile', mv: 800,
      dmg: [50, 75, 113], r: [11, 13, 15], mag: 4, reload: 11, range: 340, pen: 6,
      vs: { flesh: 0.7, armor: 0.7, air: 2.4, building: 0.4 } },
    skill: { name: '「對偶解」反向彈道預測', fx: 'intercept', r: [160, 200, 240],
      cd: [15, 13, 11], mp: [30, 35, 40], desc: '「對偶解」反向彈道預測：攔截永遠該比打擊便宜，瞬間清空半徑內來襲飛彈' },
    ult: { name: '頭巾內襯的「不可攔截區」', fx: 'buff', target: 'team', r: 220, mul: { dmgTaken: [0.55, 0.45, 0.35] },
      dur: [6, 7, 8], cd: [80, 70, 60], mp: [90, 100, 110], desc: '頭巾內襯的「不可攔截區」：展開不可突破之陣，半徑內友軍承傷大幅減免' },
  },
  t11: {
    ...
    light: { name: '「哈瓦那」老式 12.7mm 重機槍', rw: '「哈瓦那」老式重機槍・DShKM 12.7mm・初速 850m/s', type: 'gun', mv: 850,
      dmg: [20, 25, 31], rate: 4.8, mag: [34, 40, 48], reload: 2.3, range: 200, pen: 4,
      vs: { flesh: 1.3, armor: 1.0, air: 0.8, building: 0.6 } },
    heavy: { name: '「老雪茄」SPG-9 破甲無後座力砲', rw: '「老雪茄」無後座力砲・SPG-9 破甲彈・初速 435m/s', type: 'launcher', mv: 435,
    ...
    skill: { name: '「鐵罐頭最薄的地方」', fx: 'buff', target: 'team', r: 160, mul: { dmgTaken: [0.7, 0.65, 0.6] },
      dur: [4, 5, 6], cd: [20, 18, 16], mp: [35, 40, 45], desc: '「鐵罐頭哪裡最薄，老子都教過」：老兵口傳心授，半徑內友軍承受傷害降低' },
    ult: { name: '安哥拉老兵步兵班', fx: 'summon', unit: 'squad', count: [4, 6, 8],
      cd: [85, 75, 65], mp: [85, 95, 105], desc: '安哥拉老兵步兵班：老戰友聽得懂的戰術黑話，呼叫全副武裝步兵班沿兵線投入' },
  },
  t12: {
    ...
    light: { name: '「螢光尋跡」相位雷射槍', rw: '「螢光尋跡」雷射槍・低功率相位雷射・光速直擊', type: 'beam',
      dmg: [13, 16, 20], rate: 10, mag: [40, 50, 60], reload: 1.8, range: 220, crit: 0.08,
      vs: { flesh: 1.3, armor: 0.5, air: 1.0, building: 0.4 } },
    heavy: { name: '「深層日誌」EM 標定電磁砲', rw: '「深層日誌」標定電磁砲・EM 標定彈・初速 2500m/s', type: 'rail', mv: 2500,
      dmg: [53, 82, 122], mag: 2, reload: 8, range: 340, emp: [0.8, 1.0, 1.2],
      vs: { flesh: 0.8, armor: 1.0, air: 1.6, building: 0.5 } },
    skill: { name: '明斯克夜間頻譜掃描', fx: 'vision', vision: [5, 7, 9],
      cd: [24, 21, 18], mp: [35, 40, 45], desc: '明斯克夜間頻譜掃描：在頻譜深處尋找蜂群心跳，全隊獲得限時無霧視野' },
    ult: { name: '「那也是一個人」全頻靜默', fx: 'emp', r: 240, dur: [3, 4, 5], vision: [4, 5, 6],
      cd: [75, 65, 55], mp: [90, 100, 110], desc: '「那也是一個人」全頻靜默：標記過的所有敵方訊號瞬間強制靜默，並回傳精確座標' },
  },
...
//       m05 jet↔wolf(趾行深屈・鬃刺肩尖)
  ...
  m01: {
    ...
    light: { name: '「火神」7.62mm 六管轉輪機槍', rw: '「火神」轉輪速射機槍・M134 Minigun 7.62mm・初速 850m/s', type: 'gun', mv: 850,
    ...
    heavy: { name: '「地獄火」穿盾雷射導引彈', rw: '「地獄火」穿盾雷射導引彈・AGM-114 縮裝・初速 450m/s', type: 'missile', mv: 450,
    ...
    skill: { name: '「三十秒屋頂條款」', fx: 'dash', imp: [27, 33, 39],
      cd: [12, 10, 8], mp: [25, 30, 35], desc: '「三十秒屋頂條款」：哪邊付錢都一樣快，沿視線瞬間爆發脫離戰場' },
    ult: { name: '「合約外加班費三倍」', fx: 'buff', target: 'self', mul: { dmg: [1.3, 1.4, 1.5], reload: [0.8, 0.75, 0.7] },
      add: { fx: 'vamp', f: [0.12, 0.16, 0.2] },
      dur: [8, 10, 12], cd: [75, 65, 55], mp: [80, 90, 100], desc: '「合約外加班費三倍」：合約外時段機體全面超載，暗夜渡鴉嗜血汲命（吸血）' },
  },
  m02: {
    ...
    light: { name: '「憲兵隊」通用機關槍', rw: '「憲兵隊」通用機槍・FN MAG 7.62mm・初速 840m/s', type: 'gun', mv: 840,
      dmg: [16, 20, 25], rate: 7, mag: [40, 48, 56], reload: 2.2, range: 195, crit: 0.05,
      vs: { flesh: 1.3, armor: 0.8, air: 0.9, building: 0.5 } },
    heavy: { name: '「巨顎」口藏重型線圈加農砲', rw: '「巨顎」重型線圈加農砲・大口徑線性馬達砲・初速 1800m/s', type: 'rail', mv: 1800,
      dmg: [74, 109, 154], mag: 2, reload: 8, range: 360, crit: 0.1, critX: 1.8, pen: [20, 25, 30],
      vs: { flesh: 0.9, armor: 1.7, air: 0.4, building: 1.0 } },
    skill: { name: '「雇主的貨第一」防禦姿態', fx: 'buff', target: 'self', mul: { dmgTaken: [0.6, 0.55, 0.5] },
      dur: [4, 5, 6], cd: [16, 14, 12], mp: [30, 35, 40], desc: '「雇主的貨第一」防禦姿態：二十年押運本能，機體重裝防禦承傷大幅減免' },
    ult: { name: '二十年零損失押運合約', fx: 'buff', target: 'team', r: 200, mul: { dmgTaken: [0.7, 0.62, 0.55] },
      dur: [6, 8, 10], cd: [80, 70, 60], mp: [85, 95, 105], desc: '二十年零損失押運合約：這一單絕對保到底，半徑內全體友軍承受傷害顯著降低' },
  },
  m03: {
    ...
    light: { name: '「雪線」防衛脈衝雷射', rw: '「雪線」防衛脈衝雷射・低功率防禦光束・光速直擊', type: 'beam',
      dmg: [15, 19, 23], rate: 8, mag: [30, 36, 42], reload: 1.9, range: 220, crit: 0.07,
      vs: { flesh: 1.4, armor: 0.5, air: 1.1, building: 0.4 } },
    heavy: { name: '「精算師」雷射導引空投彈', rw: '「精算師」雷射導引空投彈・APKWS 縮裝・初速 700m/s', type: 'launcher', mv: 700, guide: 1,
      dmg: [62, 95, 137], r: [11, 13, 15], mag: 3, reload: 12, range: 300, pen: 8,
      vs: { flesh: 1.1, armor: 1.2, air: 1.2, building: 1.0 } },
    skill: { name: '阿爾卑斯空中救援保單', fx: 'heal', target: 'team', r: 150, heal: [140, 200, 260],
      cd: [20, 18, 16], mp: [40, 45, 50], desc: '阿爾卑斯空中救援保單：先修好再收帳單，半徑內友軍機體裝甲迅速修復' },
    ult: { name: '「全額理賠」年度大結算', fx: 'heal', target: 'team', r: 220, heal: [260, 350, 440], sp: true,
      cd: [85, 75, 65], mp: [90, 100, 110], desc: '「全額理賠」年度大結算：大帳一次全額結清，全隊裝甲大修、護盾同步充滿' },
  },
  m04: {
    ...
    ult: { name: '阿爾泰雄鷹全境巡狩', fx: 'recon', target: 'team', vision: [18, 24, 30],
      mul: { range: [1.2, 1.25, 1.3], speed: [1.2, 1.25, 1.3] },
      add: { fx: 'evade', evade: [0.12, 0.16, 0.2] }, dur: [8, 10, 12],
      cd: [72, 64, 56], mp: [85, 95, 105], desc: '阿爾泰雄鷹全境巡狩：受雇前先查清一切：全隊無霧視野翻倍，射程/跑速/閃避同步拉高' },
  },
  m05: {
    ...
    light: { name: '「鎖喉」12.7mm 三管電磁機砲', rw: '「鎖喉」三管電磁機砲・GAU-19 線圈改裝・初速 1300m/s', type: 'rail', mv: 1300,
      dmg: [19, 24, 30], rate: 6, mag: [36, 44, 52], reload: 2.4, range: 200, crit: 0.06,
      vs: { flesh: 1.2, armor: 1.0, air: 0.9, building: 0.6 } },
    heavy: { name: '「債務催收」自導飛彈', rw: '「債務催收」自導飛彈・射後鎖定制導彈・初速 400m/s', type: 'missile', mv: 400,
      dmg: [50, 72, 103], r: [13, 15, 17], mag: 4, reload: 11, range: 330, pen: [12, 15, 18],
      vs: { flesh: 1.0, armor: 1.5, air: 0.5, building: 1.2 } },
    skill: { name: '「先斷電，再談利息」', fx: 'emp', r: 130, dur: [2.5, 3, 3.5], range: 250,
      cd: [18, 16, 14], mp: [40, 45, 50], desc: '「先斷電，再談利息」：欠債不還就斷電，指定區域敵方機體武器強制離線' },
    ult: { name: '「連本帶利」強制清算', fx: 'strike', count: [6, 8, 10], dmg: [68, 85, 106], r: 11, scatter: 38,
      add: { fx: 'stun', dur: [0.8, 1, 1.2] },
      range: 330, pen: 10, cd: [78, 68, 58], mp: [88, 98, 108], vs: { armor: 1.3, building: 1.2 },
      desc: '「連本帶利」強制清算：逾期利滾利，飽和清算打擊瞬間斷電扣押敵機（麻痺）' },
  },
  m06: {
    ...
    light: { name: '「里約放克」雙聯掛架機槍', rw: '「里約放克」雙聯掛架機槍・PKP 7.62mm 縮裝・初速 825m/s', type: 'gun', mv: 825,
      dmg: [13, 16, 20], rate: 9, mag: [45, 54, 63], reload: 2.2, range: 180, crit: 0.05,
      vs: { flesh: 1.3, armor: 0.6, air: 1.2, building: 0.5 } },
    heavy: { name: '「嘉年華彩花」集束子母彈', rw: '「嘉年華彩花」集束子母彈・CBU 縮裝拋撒・初速 400m/s', type: 'launcher', mv: 400,
    ...
    skill: { name: '「這單我轉出去了」外包契約', fx: 'summon', unit: 'rocketeer', count: [2, 3, 4],
      cd: [26, 23, 20], mp: [40, 45, 50], desc: '「這單我轉出去了」外包契約：臨時轉分包，僱傭火箭兵沿最近兵線加入戰鬥' },
    ult: { name: '「狂歡節旺季」直升機大擴編', fx: 'summon', unit: 'heli', count: [1, 2, 3],
      cd: [85, 75, 65], mp: [90, 100, 110], desc: '「狂歡節旺季」直升機大擴編：訂單瞬間爆量，呼叫攻擊直升機編隊全面壓上' },
  },
  m07: {
    ...
    light: { name: '「界碑」雙聯 35mm 厄利孔高射砲', rw: '「界碑」雙聯 35mm 厄利孔高射砲・Oerlikon 縮裝・初速 1100m/s', type: 'gun', mv: 1100,
      dmg: [18, 23, 28], rate: 6.5, mag: [32, 40, 48], reload: 2.6, range: 210, crit: 0.05,
      vs: { flesh: 1.0, armor: 0.9, air: 1.6, building: 0.5 } },
    heavy: { name: '「落閘」扇面防衛電漿幕', rw: '「落閘」區域拒止電漿幕・近迫電漿扇形散射攔截', type: 'plasma', arc: [22, 25, 28],
      dmg: [46, 73, 111], mag: 3, reload: 7, range: 264, pen: 8,
      vs: { flesh: 0.8, armor: 1.0, air: 2.2, building: 0.3 } },
    skill: { name: '「血償條款」防禦穹頂', fx: 'intercept', r: [160, 200, 240],
      cd: [16, 14, 12], mp: [30, 35, 40], desc: '「血償條款」防禦穹頂：一手交錢一手交貨，瞬間清空半徑內所有來襲飛彈' },
    ult: { name: '「禁止通行」全域火力覆蓋', fx: 'strike', count: [7, 9, 11], dmg: [55, 68, 85], r: 9, scatter: 40,
      add: { fx: 'slow', f: 0.6, dur: [2, 2.5, 3] },
      range: 320, cd: [74, 66, 58], mp: [85, 95, 105], vs: { air: 2.0, flesh: 1.2 },
      desc: '「禁止通行」全域火力覆蓋：把整片天空劃進責任區，密集彈幕強勢壓制（緩速）' },
  },
  m08: {
    ...
    ult: { name: '「尾款結清，查無此人」', fx: 'stealth', dur: [4, 5, 6], add: { fx: 'alpha' },
      cd: [70, 62, 54], mp: [80, 90, 100], desc: '「尾款結清，查無此人」：尾款結清便人間蒸發（開火現形，但現形第一秒爆發傷害暴增）' },
  },
};
...
export const SIEGE = {
  ...
  NAMES: ['前線砲塔', '中段砲塔', '主堡'],
  // ---- 區域 BOSS 關卡(2026-08-14 使用者定案:「敵方砲塔主堡會鎖住 HP1 直到區域 BOSS 被
  // 擊敗且對話結束」)。**這是鎖血之外的第二道閘,不是取代它** —— 階段順序(前線 → 中段 →
  // 主堡)照舊由 `siegeOpenStage` 管,本閘管的是「同一階之內,BOSS 先死才拆得掉建築」。
  // 兩者的結算語意刻意不同:
  //   ・階段未到    = **完全免傷 + 排除索敵**(`siegeLocked`;打不動的東西留在索敵名單裡會把兵線卡死)
  //   ・BOSS 未解決 = **打得掉血,但夾在 `BLD_HP_FLOOR` 不死**(使用者選的語意)。建築照樣被索敵、
  //     照樣掉血 ⇒ 玩家看得到進度條,只是收不了尾;`_kill` 因此不會被呼叫,階段也不會提前推進。
  // 「對話結束」= 伺服器自己倒數 `TALK_S` 秒(使用者選的做法)。對白內容住客戶端 `storytalk.js`、
  // 伺服器不知道這是第幾章 ⇒ 這裡放的是**固定秒數**而不是推導值:一場對白是 8~10 句、每句
  // 1.7~4.2 秒(`dialogue.js lineMs`),典型全長約 25~30 秒。改對白長度 MUST 回頭看這一格。
  // ⚠ **主堡那一階刻意不倒數**:它的對白在**結算畫面**播(主堡一倒就 gameOver,戰鬥中沒有
  // 那一場可以播完)⇒ 主堡的 BOSS 一死就解鎖,否則玩家會對著一座 HP1 的主堡空等 TALK_S 秒。
  TALK_S: 26,
  BLD_HP_FLOOR: 1,   // 「鎖住 HP1」的那個 1(HP 下限,不是免傷)
};
...
/**
 * 這一階的區域 BOSS 倒下之後,建築還要鎖住幾秒(= 那一場對白的長度)。**唯一縫**。
 * 主堡那一階回 0 —— 它的對白在結算畫面播,見 SIEGE.TALK_S 的 ⚠。
 */
export const siegeTalkS = (stage) => (stage < SIEGE.BASE ? SIEGE.TALK_S : 0);
...
// ⑥**防禦面也隨段位升級 + 進段補滿護盾**(2026-08-14 使用者:「敵方 BOSS 防禦面也會隨著 HP
//   階段升級,HP 階段改變時護盾會補滿」)——「防禦面與小兵永不升級」那一句只剩後半段成立
//   (`buy` 對 BOSS 照樣直接拒絕,小兵強化恆 Lv0)。三條:
//   ㋐ 八軌**全部**由 `_bossEnrage` 推進(名冊走 `ECON.UPGRADES` 全表,MUST NOT 手寫「防禦四軌」
//     —— 加第九軌時手寫的那一份會靜默過期);
//   ㋑ **裝甲上限(`hp` 軌)MUST 等比放大當下 HP**:段位量的是 `Σhp / Σmaxhp`(①),而 `_applyUpg`
//     的 hp 軌是「上限與當下血量同時 +Δ」= 補滿那一截,直接違反 ④「補血不得回到上一階」;
//     反過來只放大上限的話,比例當場掉下去 ⇒ `_bossSync` 連鎖狂暴到頂。等比放大兩邊都躲掉:
//     比例逐位元不變,拿到的是純粹的 EHP;
//   ㋒ **護盾補滿排在升級之後**(`sp` 軌會加大 `maxSp`,先補後升 = 補到舊上限)。
//   護盾補滿刻意**不受 ④ 的天花板約束** —— 那條講的是裝甲(HP),而使用者這一輪講的就是護盾。
export const BOSS = {
