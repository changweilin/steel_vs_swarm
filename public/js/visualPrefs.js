...
export const VISUAL_KNOBS = {
  ...
  // 預設**關**:它要的是「場景裡每一個材質都宣告了第二個輸出」這條契約,而漏掉一個的症狀
  // 是那一批物件整批不畫(WebGL2 的 INVALID_OPERATION,console 一個字都沒有)。做成開關 =
  // 風險變成 opt-in:關著的時候那條契約根本沒有被行使,畫面逐位元同舊制。
  inkMrt: {
  ...
  // 紀律① 不變:需美術方向確認的一律 def = 「這一項不生效」。
  inkBreak: {
    label: '墨線斷筆', def: 0, min: 0, max: 1, step: 0.05, unit: '%',
    hint: '沿著線的方向以低頻噪聲把墨線斷開,讓它讀起來像**畫**出來的而不是**算**出來的。0% = 連續實線(舊制)。',
  },
  inkGroup: {
    label: '群組剪影', def: 'off', choices: ['off', 'on'],
    choiceLabels: { off: '關', on: '開' },
    hint: '整株樹 / 整顆巨岩 / 一堆石頭當成**一個東西**看:群組內部的折邊線收掉,只留剪影。關掉就是逐塊零件各自描邊(舊制)。WebGL1 裝置上這個開關沒有作用。',
  },
  leafCard: {
    label: '葉片卡冠層', def: 'off', choices: ['off', 'auto', 'all'],
  ...
  reflect: {
    label: '水面倒影', def: 0, min: 0, max: 1.5, step: 0.05, unit: '%',
    hint: '岸邊高物件在水面上拉出斷口倒影塊(不是真的鏡面反射:那要多跑一趟全場)。**倒影是亮的還是暗的**需要美術方向,故預設 0% = 不畫。',
  },
  // ⚠ 這一格的 def 是 **§0-b 的使用者定案**(2026-08-15「改 —— 走 School B」)而不是
  // 一般的「需美術方向 ⇒ 預設不生效」。翻成 'b' 的前提有兩個,兩個都已經成立:
