// ============ s05 part file (airframe; dev-only) ============
// s05 overclock race FPV: X carbon main plate + four exposed motors + tilted camera mount
// 2D approved art: public/assets/cyberpunk_art/mechs/s05_static.png(/ _moving / _heavy)
// Authority = mecha.js gen.sil: one X carbon plate, four exposed motors, one forward-raked camera
// mount; no fairing, the frame is everything. gen.note: no fairing or armor plates -- the design
// language is "nothing but the parts that fly."
//   -> deliberately never calls tboxF as a shell: the main frame is a true-outline X plate (prismF
//     extrusion) with all electronics stacked bare. One extra cover breaks this craft's reason to exist.
...
export default {
  ...
  body(c, t) {
    const { PAL, accent } = c;
    // X carbon plate (thick lower, thin upper): true-X outline -- narrowed arm roots, center electronics deck
    const X = [
    ...
    // Bare electronics stack (FC + 4-in-1 ESC + status LED)
    ...
    // Top-mounted battery + dual straps + kill tally marks (pilot culture; 5th diagonal slash)
    ...
    // XT60 pigtail (trailing down-back; bare)
    ...
    // Bunny-ear antennas x2 (tail V) + lollipop head
    for (const sx of [-1, 1]) {
  ...
  lift(c, t) {
    const { PAL, K } = c;
    const spin = [];
    const R = 0.42, Q = 0.72;                   // true-X: tight wheelbase amplifies prop-to-frame ratio
    for (const [sx, sz] of [[-1, -1], [-1, 1], [1, -1], [1, 1]]) {
      cylF(t, 0.085, 0.095, 0.16, 8, sx * Q, -0.06, sz * Q, GUNMETAL, { metalness: 0.9 });   // Exposed motor
      ...
      spin.push(r.prop);
      // Under-arm team color LED strip (night-race identification; no glows)
  ...
  mount(c, F) {
    ...
    const t = F.tilt;
    // 35-deg forward-raked camera mount (wedge prism) + main lens + crash standoff -- attitude anchor
    ...
    // Armament: race frame has no armor; weapons are minimum-bulk bare pods (light left, heavy right)
