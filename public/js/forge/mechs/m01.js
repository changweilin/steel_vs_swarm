...
// 舊版 = legacy_models.js buildMorphMech() 的 `G === 'vampire'` 那一支;它的主裝甲吃 `armor = PAL.main`
// ,而本檔七片主殼(頭殼/切面楔胸/腹甲/大腿/小腿/上臂/前臂)全壓在 `mid` 上 ——
...
export default {
  ...
  chest(c, ch, d) {
    ...
    // 收起滑翔翼變成披風」)—— 地面態 = 垂下貼背的斗篷,飛行態由 m01_flight **就地改擺位**
    // 轉成水平三角翼(2026-08-15 使用者:「不要用透明顯現,要與地面形態的左右半翼整合」)。
    // ⇒ 翼**只在這裡生一次**,飛行檔 MUST NOT 再叫一次 `glider`:多叫的那兩片拿到的是
    //   `glider#2/#3` 的標籤、在地面型沒有對手 ⇒ 整組走淡入(實測 18 件),而畫面上是
    //   「披風還垂著,另外兩片翼憑空浮出來」。交出 pivot 給飛行檔重擺 = 同一片零件的兩個姿態。
    c.gliders = {};
    for (const sx of [-1, 1]) {
