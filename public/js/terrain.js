...
export async function buildTerrain(cfg, onProgress) {
  ...
  // 「看得到」:把地形往外再鋪 `edgeBufferM()`，但只容納邊界障礙與其腳下地面。
  // 遠景假山／假林／假城／島礁已收入有碰撞的障礙環，不再靠巨大外裙延伸到地平線。
  // 五條紀律:
  //  ①**深度推導不手寫** = `edgeBufferM()`(← `edgeWallDeepM()`):障礙型錄變深時裙自動跟著變；
  //    緩衝介於最深障礙與其少量安全餘裕之間，不再被射程／地平線距離擴大。
  ...
  //    整個平面),拼圖天生成立。
  //  ⑦**對外的高度查詢**:`bufferHeightAt` 仍是裙的外推高度**唯一縫**，
  //    供地貌底毯與開發量測使用；正式建圖已不再發射圖界外無碰撞布景。
