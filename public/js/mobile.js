...
// 設定列的說明一律改成 ⓘ 懸浮提示(2026-07-31);文字由各自的 sync 函式寫進 `data-tip`,
// 標記 DOM 只有 tip.js 這一份 —— MUST NOT 在這裡自己拼一顆長得像的 span。
...
export function renderCtrlModeRow(mount, opts = {}) {
  ...
  // `opts.bare`:呼叫端(戰區畫面)的分頁標題已經寫著「操作方式」⇒ 不再重複一次列名。
  // 說明文字改掛 ⓘ 懸浮提示(tip.js);常駐一長串會把陣營卡推出首屏。
...
export function syncCtrlSettings() {
  ...
  // 提示文字一律寫進 `data-tip`(ⓘ 懸浮提示),MUST NOT 退回常駐 textContent
  const tip = (el, text) => { if (el) el.dataset.tip = text; };
  ...
  for (const mount of _modeMounts) {
    ...
    tip(mount.querySelector('.tset-ctrl-hint'),
      (_modeHost ? '整房一致,由你決定 ・ ' : '整房一致,由房主決定 ・ ')
...
export function syncTouchSettings() {
  ...
  for (const mount of _mounts) {
    for (const d of TOUCH_SETTINGS) {
      ...
      if (!el) continue;
      const dot = mount.querySelector(`.tset-hint-${d.key}`);   // 說明改掛 ⓘ 懸浮提示
