...
// ============ 邊界緩衝區單一物件組合類障礙物填滿 ============
// 人造物件如：風機／太陽能板／摩天樓／連排透天等，以整齊排列（矩陣排列或交錯排列）填滿邊界緩衝區；
// 自然物件如：大小神木林／大小巨岩／大小山頭／冰山浮冰等，以隨機排列（多尺度大小變化）填滿邊界緩衝區。
// 全程使用世界座標雜湊起種，零全域共享隨機序列消耗，嚴格決定性。
export const BOUNDARY_BUFFER_LAYOUTS = Object.freeze({
  // ---- 人造物件 (Artificial): 整齊排列 (矩陣或交錯) ----
  ...
  // 摩天樓: 交錯排列 (staggered) 豐富立體天際線
  skyscrapers: { type: 'artificial', mode: 'staggered', pitchX: 16, pitchZ: 18, object: 'skyscraper', scaleRange: [0.85, 1.15] },
  alpinecity:  { type: 'artificial', mode: 'staggered', pitchX: 16, pitchZ: 18, object: 'skyscraper', scaleRange: [0.85, 1.1] },
  skyfall:     { type: 'artificial', mode: 'staggered', pitchX: 20, pitchZ: 20, object: 'skyfall',    scaleRange: [0.9, 1.1] },
  // 連排透天 / 民房: 矩陣排列 (grid) 正交街廓
  rowhouse:    { type: 'artificial', mode: 'grid',      pitchX: 10, pitchZ: 11, object: 'house', scaleRange: [0.9, 1.05] },
  edgehamlet:  { type: 'artificial', mode: 'grid',      pitchX: 11, pitchZ: 12, object: 'house', scaleRange: [0.9, 1.1] },
  cliffvillage:{ type: 'artificial', mode: 'grid',      pitchX: 10, pitchZ: 11, object: 'house', scaleRange: [0.9, 1.05] },
  house:       { type: 'artificial', mode: 'grid',      pitchX: 12, pitchZ: 12, object: 'house', scaleRange: [0.9, 1.05] },
  // 工業廠房 / 倉儲 / 電廠 / 焚化爐: 矩陣排列 (grid)
  warehousebelt:    { type: 'artificial', mode: 'grid', pitchX: 16, pitchZ: 16, object: 'factory', scaleRange: [0.9, 1.05] },
  harborwarehouses: { type: 'artificial', mode: 'grid', pitchX: 16, pitchZ: 16, object: 'factory', scaleRange: [0.9, 1.05] },
  factory:          { type: 'artificial', mode: 'grid', pitchX: 18, pitchZ: 18, object: 'factory', scaleRange: [0.9, 1.1] },
  powerplant:       { type: 'artificial', mode: 'grid', pitchX: 18, pitchZ: 18, object: 'powerplant', scaleRange: [0.9, 1.1] },
  incinerator:      { type: 'artificial', mode: 'grid', pitchX: 18, pitchZ: 18, object: 'incinerator', scaleRange: [0.9, 1.1] },
  // 油槽儲運: 交錯排列 (staggered)
  tankfarm:         { type: 'artificial', mode: 'staggered', pitchX: 12, pitchZ: 12, object: 'tank', scaleRange: [0.9, 1.1] },
  // 溫室大棚: 矩陣排列 (grid)
  greenhouse:         { type: 'artificial', mode: 'grid', pitchX: 12, pitchZ: 12, object: 'greenhouse', scaleRange: [0.95, 1.05] },
  hillsidegreenhouses:{ type: 'artificial', mode: 'grid', pitchX: 12, pitchZ: 12, object: 'greenhouse', scaleRange: [0.95, 1.05] },
  // 農牧場: 矩陣排列 (grid)
  ranch:            { type: 'artificial', mode: 'grid', pitchX: 16, pitchZ: 16, object: 'ranch', scaleRange: [0.9, 1.1] },
  terracedfarms:    { type: 'artificial', mode: 'grid', pitchX: 16, pitchZ: 16, object: 'ranch', scaleRange: [0.9, 1.1] },
  // 水產養殖 / 蚵棚: 矩陣排列 (grid)
  ...
  mine:             { type: 'artificial', mode: 'staggered', pitchX: 18, pitchZ: 18, object: 'mine', scaleRange: [0.9, 1.1] },
  oilfield:         { type: 'artificial', mode: 'staggered', pitchX: 16, pitchZ: 16, object: 'oilfield', scaleRange: [0.9, 1.1] },
  deeprig:          { type: 'artificial', mode: 'staggered', pitchX: 18, pitchZ: 18, object: 'oilfield', scaleRange: [0.9, 1.1] },
  trucks:           { type: 'artificial', mode: 'grid', pitchX: 8, pitchZ: 10, object: 'truck', scaleRange: [0.95, 1.05] },

  // ---- 自然物件 (Natural): 隨機排列 (random) 帶多尺度大小變化 ----
  // 大小神木林: 隨機散布，尺度 0.65x ~ 1.55x
  gianttree:   { type: 'natural', mode: 'random', pitchX: 10, pitchZ: 11, object: 'gianttree', scaleRange: [0.65, 1.55], randomYaw: true },
  giantforest: { type: 'natural', mode: 'random', pitchX: 10, pitchZ: 11, object: 'gianttree', scaleRange: [0.65, 1.50], randomYaw: true },
  densegiants: { type: 'natural', mode: 'random', pitchX: 9,  pitchZ: 10, object: 'gianttree', scaleRange: [0.70, 1.55], randomYaw: true },
  foresthills: { type: 'natural', mode: 'random', pitchX: 11, pitchZ: 12, object: 'gianttree', scaleRange: [0.65, 1.45], randomYaw: true },
  fallentree:  { type: 'natural', mode: 'random', pitchX: 12, pitchZ: 10, object: 'fallentree',scaleRange: [0.60, 1.40], randomYaw: true },
  // 大小巨岩: 隨機散布，尺度 0.55x ~ 1.65x
  boulder:     { type: 'natural', mode: 'random', pitchX: 11, pitchZ: 11, object: 'boulder',   scaleRange: [0.55, 1.65], randomYaw: true },
  rockery:     { type: 'natural', mode: 'random', pitchX: 10, pitchZ: 11, object: 'boulder',   scaleRange: [0.60, 1.60], randomYaw: true },
  basaltspine: { type: 'natural', mode: 'random', pitchX: 11, pitchZ: 12, object: 'boulder',   scaleRange: [0.60, 1.50], randomYaw: true },
  reefchain:   { type: 'natural', mode: 'random', pitchX: 11, pitchZ: 11, object: 'boulder',   scaleRange: [0.60, 1.50], randomYaw: true },
  isletbarrier:{ type: 'natural', mode: 'random', pitchX: 12, pitchZ: 12, object: 'boulder',   scaleRange: [0.55, 1.50], randomYaw: true },
  // 大小山頭: 隨機散布，尺度 0.70x ~ 1.45x
  rollinghills:{ type: 'natural', mode: 'random', pitchX: 14, pitchZ: 14, object: 'boulder',   scaleRange: [0.70, 1.45], randomYaw: true },
  // 冰山浮冰: 隨機散布，尺度 0.50x ~ 1.45x
  icefloe:     { type: 'natural', mode: 'random', pitchX: 14, pitchZ: 13, object: 'icefloe',   scaleRange: [0.50, 1.40], randomYaw: true },
  iceberg:     { type: 'natural', mode: 'random', pitchX: 18, pitchZ: 16, object: 'iceberg',   scaleRange: [0.55, 1.45], randomYaw: true },
  seaice:      { type: 'natural', mode: 'random', pitchX: 13, pitchZ: 12, object: 'icefloe',   scaleRange: [0.50, 1.35], randomYaw: true },
});
 ...
 */
function generateBoundaryUnit(kind, { w, d, h, seed, season, water, layout }) {
  const def = WALL_KINDS[kind];
  if (layout.object === 'wind' || kind.startsWith('wind')) {
    return linearEnvironmentParts(kind, { len: w, depth: d, h, seed, season });
  }
  if (layout.object === 'solar' || kind.includes('solar')) {
    return linearEnvironmentParts(kind, { len: w, depth: d, h, seed, season });
  }
  if (layout.object === 'aquaculture' || ['searanch', 'oysterracks'].includes(kind)) {
    return linearEnvironmentParts(kind, { len: w, depth: d, h, seed, season });
  }
  if (layout.object === 'truck' || kind === 'trucks') {
    return linearEnvironmentParts('trucks', { len: w, depth: d, h, seed, season });
  }
  if (kind === 'deeprig') {
    return linearEnvironmentParts('deeprig', { len: w, depth: d, h, seed, season });
  }
  if (layout.object === 'tank' || kind === 'tankfarm') {
  ...
  if (def?.object && ENVIRONMENT_OBJECTS[def.object]) {
    return environmentParts(def.object, { size: [w, h, d], seed, season });
  }
  if (layout.object && ENVIRONMENT_OBJECTS[layout.object]) {
    return environmentParts(layout.object, { size: [w, h, d], seed, season });
  }
  return linearEnvironmentParts(kind, { len: w, depth: d, h, seed, season });
}
...
export function buildBoundaryRunParts(kind, {
  ...
  if (mode === 'random') {
    // 自然物件：全區（邊界 + 緩衝區）一體化隨機散布，尺度變化與旋轉姿態跨區連續
    for (let r = 0; r < totalRows; r++) {
      ...
      for (let c = 0; c < numCols; c++) {
        ...
        const jitU = (rnd() - 0.5) * colStep * 0.65;
        const jitV = isBuffer ? (rnd() - 0.5) * rowStep * 0.55 : 0;
        let u = baseU + jitU;
        let v = baseV + jitV;

        let scale = 1.0;
        if (layout.scaleRange) {
          const [sMin, sMax] = layout.scaleRange;
          scale = isBuffer ? (sMin + rnd() * (sMax - sMin)) : (sMin + rnd() * (Math.min(1.0, sMax) - sMin));
        }

        const objW = Math.min(unitW * scale, colStep * 0.95);
        const objD = Math.min(unitD * scale, (isBuffer ? rowStep : depth) * 0.92);
        const objH = targetH * scale;

        if (u - objW / 2 < -len / 2 + 0.2 || u + objW / 2 > len / 2 - 0.2) continue;
        if (!isBuffer) {
          if (v + objD / 2 > depth / 2 || v - objD / 2 < -depth / 2) continue;
        } else {
          if (v + objD / 2 > -depth / 2 - 0.2 || v - objD / 2 < -depth / 2 - bufferDepth + 0.5) continue;
        }

        const modelParts = generateBoundaryUnit(kind, {
          w: objW, d: objD, h: objH, seed: ptSeed, season, water, layout,
        });
    ...
    // 人造物件：整齊排列（矩陣 grid 或交錯 staggered），邊界（Row 0）與緩衝區（Row 1..N）鎖入同一網格
    for (let r = 0; r < totalRows; r++) {
      const isBuffer = r > 0;
      ...
      for (let c = 0; c < numCols; c++) {
        ...
        if (u - unitW / 2 < -len / 2 + 0.2 || u + unitW / 2 > len / 2 - 0.2) continue;
        if (isBuffer && (v - unitD / 2 < -depth / 2 - bufferDepth + 0.5)) continue;

        const modelParts = generateBoundaryUnit(kind, {
          w: unitW, d: unitD, h: targetH, seed: ptSeed, season, water, layout,
        });
