...
import { heritageRuinType } from './heritageSites.js';
 ...
 */
export function buildSunkenRelics(parentGroup, terrain, seed, blockers = [], mappedSites = []) {
  const wy = terrain?.waterY;
  if (wy == null) return;
  ...
  const occupied = [];
  // 圖資明確的水下遺址先於隨機散布，尺寸使用同一水深／半徑防線。
  for (const site of mappedSites) {
    if (relicCount >= maxRelics) break;
    const {x,z} = site, radius=RELIC_KINDS.ruins.colR;
    const floorY=terrain.heightAt(x,z), depth=wy-floorY;
    if (!Number.isFinite(floorY) || depth<3 || terrainEnvCode(terrain,x,z)!==1
      || x-radius<minX || x+radius>maxX || z-radius<minZ || z+radius>maxZ
      || occupied.some(p=>Math.hypot(p.x-x,p.z-z)<p.radius+radius)) continue;
    const objectSeed=(Math.imul(Math.round(x*16),73856093)^Math.imul(Math.round(z*16),19349663)^seed)>>>0;
    const relic=buildRelicObject('ruins',parentGroup,x,floorY,z,mulberry32(objectSeed),{
      state:'underwater',seed:objectSeed,maxHeight:depth-.3,ruinType:heritageRuinType(site.tags),
    });
    const col=relicCollider(relic,'mapped_underwater_heritage',wy);
    if(col) blockers.push(col);
    occupied.push({x,z,radius});relicCount++;
  }
  ...
  for (let i = 0; i < 220 && relicCount < maxRelics; i++) {
    ...
    if (code === 1 && wy - floorY >= 3.0) {
      ...
      const radius=RELIC_KINDS[kind].colR;
      if (x-radius<minX || x+radius>maxX || z-radius<minZ || z+radius>maxZ
        || occupied.some(p=>Math.hypot(p.x-x,p.z-z)<p.radius+radius)) continue;
      const objectSeed = (Math.imul(Math.round(x*16),73856093) ^ Math.imul(Math.round(z*16),19349663) ^ seed) >>> 0;
      const relic = buildRelicObject(kind, parentGroup, x, floorY, z, mulberry32(objectSeed), {
        isLand: false, state: 'underwater', seed: objectSeed, maxHeight: wy-floorY-.3,
      });
...
export function buildSunkenRuins(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('ruins', group, x, y, z, { radius: RELIC_KINDS.ruins.colR, ...opts });
}
...
export function buildObeliskAltarRing(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('obelisk', group, x, y, z, { radius: RELIC_KINDS.obelisk.colR, ...opts });
}

/** 6. 建造沈沒/廢棄哥德尖塔與鐘樓 (Sunken Spire & Gothic Watchtower) */
export function buildSunkenSpire(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('spire', group, x, y, z, { radius: RELIC_KINDS.spire.colR, ...opts });
}

/** 7. 建造巨神雕像與守護者石手 (Colossal Titan Visage & Guardian Relic) */
export function buildColossalTitanVisage(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('titan', group, x, y, z, { radius: RELIC_KINDS.titan.colR, ...opts });
}
...
export function buildSunkenShrineTorii(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('shrineTorii', group, x, y, z, { radius: RELIC_KINDS.shrineTorii.colR, ...opts });
}

/** 13. 建造失落古代佛塔舍利殿 (Sunken Ancient Stupa Sanctuary) */
export function buildSunkenStupaRuin(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('stupaRuin', group, x, y, z, { radius: RELIC_KINDS.stupaRuin.colR, ...opts });
}

/** 14. 建造失落階梯金字塔巨石神廟 (Sunken Stepped Ziggurat Pyramid) */
export function buildSunkenPyramidZiggurat(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('pyramidZiggurat', group, x, y, z, { radius: RELIC_KINDS.pyramidZiggurat.colR, ...opts });
}

/** 15. 建造台灣原住民石板屋遺址 (Sunken Indigenous Slate House Relic) */
export function buildSunkenSlateRuin(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('sunkenSlateRuin', group, x, y, z, { radius: RELIC_KINDS.sunkenSlateRuin.colR, ...opts });
}

/** 16. 建造古埃及沉沒塔門與紙莎草柱廳 (Sunken Egyptian Pylon & Hypostyle Hall) */
export function buildSunkenEgyptianPylon(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('sunkenEgyptianPylon', group, x, y, z, { radius: RELIC_KINDS.sunkenEgyptianPylon.colR, ...opts });
}

/** 17. 建造南島語族失落巨舟形屋架 (Sunken Austronesian Tongkonan Frame) */
export function buildSunkenTongkonan(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('sunkenTongkonan', group, x, y, z, { radius: RELIC_KINDS.sunkenTongkonan.colR, ...opts });
}

/** 18. 建造極地/荒野守護石偶石陣 (Inuksuk Sentinel Cairns Site) */
export function buildInuksukSite(group, x, y, z, rnd, opts = {}) {
  return buildHeritageSite('inuksukSite', group, x, y, z, { radius: RELIC_KINDS.inuksukSite.colR, ...opts });
}
...
export function buildAquaticWorld(scene, terrain, env = {}) {
  ...
  buildSunkenRelics(rootGroup, terrain, seed, env.blockers, env.heritageSites);
